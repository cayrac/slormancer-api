import { GameDataReaper } from '../../../../model/content/game/data/game-data-reaper';

export const GAME_DATA_REAPER: Array<GameDataReaper> = [
  {
    "REF": 0,
    "EN_NAME": "Adam Nostrus' $",
    "EN_DESC": "*+@ £*+@ £*+@ £|/\n*+@ £|/\n|*Bryan is constantly taunting you.",
    "EN_LORE": "This is the Slorm Reaper Adam Nostrus gave you.",
    "EN": "Adam Nostrus' $//*+@ £*+@ £*+@ £|/\n*+@ £|/\n|*Bryan is constantly taunting you.//This is the Slorm Reaper Adam Nostrus gave you.",
    "FR": "$ d'Adam Nostrus/$ d'Adam Nostrus//*+@ £*+@ £*+@ £|/\n*+@ £|/\n|*Bryan se moque continuellement de vous.//C'est le Slorm Reaper qu'Adam Nostrus vous a confié.",
    "FR_NAME": "$ d'Adam Nostrus/$ d'Adam Nostrus",
    "FR_DESC": "*+@ £*+@ £*+@ £|/\n*+@ £|/\n|*Bryan se moque continuellement de vous.",
    "FR_LORE": "C'est le Slorm Reaper qu'Adam Nostrus vous a confié.",
    "CH": "亚当·诺斯图斯的$//*+@ £*+@ £*+@ £|/\n*+@£|/\n|*布莱恩总是嘲笑你。//这是亚当·诺斯图斯给你的猎魂武装。",
    "CH_NAME": "亚当·诺斯图斯的$",
    "CH_DESCRIPTION": "*+@ £*+@ £*+@ £|/\n*+@£|/\n|*布莱恩总是嘲笑你。",
    "CH_LORE": "这是亚当·诺斯图斯给你的猎魂武装。",
    "TW": "亞當·諾斯圖斯的$//*+@ £*+@ £*+@ £|/\n*+@£|/\n|*布萊恩總是嘲笑你。//這是亞當·諾斯圖斯給你的獵魂武裝。",
    "TW_NAME": "亞當·諾斯圖斯的$",
    "TW_DESCRIPTION": "*+@ £*+@ £*+@ £|/\n*+@£|/\n|*布萊恩總是嘲笑你。",
    "TW_LORE": "這是亞當·諾斯圖斯給你的獵魂武裝。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 0,
    "SPRITE": 0,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3|15|1|\n3|",
    "VALUE_TYPE": "|%||\n|",
    "VALUE_LEVEL": "rl|||\nrl|",
    "VALUE_STAT": "min_basic_damage_add|attack_speed_global_mult|health_on_hit_add|\nmin_elemental_damage_add|",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 22,
    "BASE_DMG_MAX": 27,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 5,
    "EVOLVE_IN": 1,
    "100": "1148 - 1666"
  },
  {
    "REF": 1,
    "EN_NAME": "Adam Nostrus' Mighty $",
    "EN_DESC": "|*Whenever you open a Breach, you gain <Adam's Blessing>, granting you +@ Attack Speed for the next @ seconds.#Adam's Blessing lasts @ second longer for every enemy inside the Breach.",
    "EN_LORE": "Not very dazzling but rather effective.",
    "EN": "Adam Nostrus' Mighty $//|*Whenever you open a Breach, you gain <Adam's Blessing>, granting you +@ Attack Speed for the next @ seconds.#Adam's Blessing lasts @ second longer for every enemy inside the Breach.//Not very dazzling but rather effective.",
    "FR": "$ Puissant d'Adam Nostrus/$ Puissante d'Adam Nostrus//|*Chaque fois que vous ouvrez une brèche, vous gagnez la Bénédiction d'Adam, vous conférant +@ de Vitesse d'Attaque pour les @ secondes suivantes.#La bénédiction d'Adam dure @ seconde de plus pour chaque ennemi à l'intérieur de la Brèche.//Pas très éblouissant mais plutôt efficace.",
    "FR_NAME": "$ Puissant d'Adam Nostrus/$ Puissante d'Adam Nostrus",
    "FR_DESC": "|*Chaque fois que vous ouvrez une brèche, vous gagnez la Bénédiction d'Adam, vous conférant +@ de Vitesse d'Attaque pour les @ secondes suivantes.#La bénédiction d'Adam dure @ seconde de plus pour chaque ennemi à l'intérieur de la Brèche.",
    "FR_LORE": "Pas très éblouissant mais plutôt efficace.",
    "CH": "亚当·诺斯图斯的强力$//|*开启裂隙时，获得<Adam's Blessing>，为你赋予+@攻击速度，持续@秒。#裂隙内的每个敌人使亚当的祝福延长@秒。//不算多厉害，但很有用。",
    "CH_NAME": "亚当·诺斯图斯的强力$",
    "CH_DESCRIPTION": "|*开启裂隙时，获得<Adam's Blessing>，为你赋予+@攻击速度，持续@秒。#裂隙内的每个敌人使亚当的祝福延长@秒。",
    "CH_LORE": "不算多厉害，但很有用。",
    "TW": "亞當·諾斯圖斯的強力$//|*開啟裂隙時，獲得<Adam's Blessing>，為你賦予+@攻擊速度，持續@秒。 #裂隙內的每個敵人使亞當的祝福延長@秒。//不算多厲害，但很有用。",
    "TW_NAME": "亞當·諾斯圖斯的強力$",
    "TW_DESCRIPTION": "|*開啟裂隙時，獲得<Adam's Blessing>，為你賦予+@攻擊速度，持續@秒。 #裂隙內的每個敵人使亞當的祝福延長@秒。",
    "TW_LORE": "不算多厲害，但很有用。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 1,
    "SPRITE": 1,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "20|5|1",
    "VALUE_TYPE": "%||",
    "VALUE_LEVEL": "||",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 22,
    "BASE_DMG_MAX": 27,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 12,
    "EVOLVE_IN": 2,
    "100": "1655 - 2392"
  },
  {
    "REF": 2,
    "EN_NAME": "Adam Nostrus' Perfect $",
    "EN_DESC": "|*Whenever you close a Breach, Bryan heals you for @ Life.",
    "EN_LORE": "There is a small label sticking out, it is written: \"Property of Adam N.\"",
    "EN": "Adam Nostrus' Perfect $//|*Whenever you close a Breach, Bryan heals you for @ Life.//There is a small label sticking out, it is written: \"Property of Adam N.\"",
    "FR": "$ Parfait d'Adam Nostrus/$ Parfaite d'Adam Nostrus//|*Chaque fois que vous fermez une Brèche, Bryan vous soigne @ de Vie.//Il y a une petite étiquette qui dépasse avec écrit : \"Propriété d'Adam N.\"",
    "FR_NAME": "$ Parfait d'Adam Nostrus/$ Parfaite d'Adam Nostrus",
    "FR_DESC": "|*Chaque fois que vous fermez une Brèche, Bryan vous soigne @ de Vie.",
    "FR_LORE": "Il y a une petite étiquette qui dépasse avec écrit : \"Propriété d'Adam N.\"",
    "CH": "亚当·诺斯图斯的完美$//|*关闭裂隙时，布莱恩会为你治疗@生命值。//有一个小标签，写着：“亚当的财产”",
    "CH_NAME": "亚当·诺斯图斯的完美$",
    "CH_DESCRIPTION": "|*关闭裂隙时，布莱恩会为你治疗@生命值。",
    "CH_LORE": "有一个小标签，写着：“亚当的财产”",
    "TW": "亞當·諾斯圖斯的完美$//|*關閉裂隙時，布萊恩會為你治療@生命值。//有一個小標籤，寫著：“亞當的財產”",
    "TW_NAME": "亞當·諾斯圖斯的完美$",
    "TW_DESCRIPTION": "|*關閉裂隙時，布萊恩會為你治療@生命值。",
    "TW_LORE": "有一個小標籤，寫著：“亞當的財產”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 2,
    "SPRITE": 2,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "rl",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 22,
    "BASE_DMG_MAX": 27,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1999 - 2898"
  },
  {
    "REF": 3,
    "EN_NAME": "$ of the Trainee",
    "EN_DESC": "*+@ £|*Whenever you deal damage to an enemy, you have _% £ to deal _ £.#Chance is equal to your current $.#Additional Damage is equal to twice your total $./\n*+@ £|*You have +_ £.#Reaper Damage is equal to ten times the combined Level of all your Characters./\n*You can no longer gain Mastery.",
    "EN_LORE": "Ideal for Champions in a hurry.",
    "EN": "$ of the Trainee//*+@ £|*Whenever you deal damage to an enemy, you have _% £ to deal _ £.#Chance is equal to your current $.#Additional Damage is equal to twice your total $./\n*+@ £|*You have +_ £.#Reaper Damage is equal to ten times the combined Level of all your Characters./\n*You can no longer gain Mastery.//Ideal for Champions in a hurry.",
    "FR": "$ de la Recrue//*+@ £|*Lorsque vous infligez des dégâts à un ennemi, vous avez _% de £ d'infliger _ de £.#Cette Chance est équivalente à votre $.#Les Dégâts Additionnels sont équivalents à deux fois votre $./\n*+@ £|*Vous avez +_ de £.#Les Dégats de Reaper sont équivalents à dix fois le niveau combiné de tous vos Personnages./\n*Vous ne pouvez plus gagner de Maîtrise.//Convient très bien aux Champions pressés.",
    "FR_NAME": "$ de la Recrue",
    "FR_DESC": "*+@ £|*Lorsque vous infligez des dégâts à un ennemi, vous avez _% de £ d'infliger _ de £.#Cette Chance est équivalente à votre $.#Les Dégâts Additionnels sont équivalents à deux fois votre $./\n*+@ £|*Vous avez +_ de £.#Les Dégats de Reaper sont équivalents à dix fois le niveau combiné de tous vos Personnages./\n*Vous ne pouvez plus gagner de Maîtrise.",
    "FR_LORE": "Convient très bien aux Champions pressés.",
    "CH": "受训者的$//*+@ £|*对敌人造成伤害时，有_%£造成_£。#概率等于你当前的$。#额外伤害等于你的总$的十倍。/\n*+@ £|*你获得+_£。#猎魂伤害等于所有角色等级之和。\n*无法再获得精通。//不算多厉害，但很有用。",
    "CH_NAME": "受训者的$",
    "CH_DESCRIPTION": "*+@ £|*对敌人造成伤害时，有_%£造成_£。#概率等于你当前的$。#额外伤害等于你的总$的十倍。/\n*+@ £|*你获得+_£。#猎魂伤害等于所有角色等级之和。\n*无法再获得精通。",
    "CH_LORE": "不算多厉害，但很有用。",
    "TW": "受訓者的$//*+@ £|*對敵人造成傷害時，有_%£造成_£。#概率等於你當前的$。#額外傷害等於你的總$的十倍。/\n*+@ £|*你獲得+_£。 #獵魂傷害等於所有角色等級之和。\n*無法再獲得精通。//不算多厲害，但很有用。",
    "TW_NAME": "受訓者的$",
    "TW_DESCRIPTION": "*+@ £|*對敵人造成傷害時，有_%£造成_£。#概率等於你當前的$。#額外傷害等於你的總$的十倍。/\n*+@ £|*你獲得+_£。 #獵魂傷害等於所有角色等級之和。\n*無法再獲得精通。",
    "TW_LORE": "不算多厲害，但很有用。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 3,
    "SPRITE": 3,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|\n1|",
    "VALUE_TYPE": "%|\n%|",
    "VALUE_LEVEL": "rl|\nrl|\n",
    "VALUE_STAT": "xp_find_percent|chance|additional_damage|\nxp_find_global_mult|min_weapon_damage_add|",
    "VALUE_REAL": "synergy:level:1|synergy:xp_find_percent:2|\nsynergy:all_level:10|",
    "BASE_DMG_MIN": 77,
    "BASE_DMG_MAX": 143,
    "MIN_DMG_LVL": 0,
    "MAX_DMG_LVL": 0,
    "DMG_MULTIPLIER": 0,
    "LOOTABLE": 1,
    "MAX_LVL": 1000,
    "EVOLVE_IN": null,
    "100": "77 - 143"
  },
  {
    "REF": 4,
    "EN_NAME": "$ of Detonating Force",
    "EN_DESC": "*+@ £*+@ £|*Whenever you deal Damage over Time to an Enemy, you apply a Time Weakness stack, up to @.#Each stack of Time Weakness increases Damage over Time Damage received by @.*act:229/\n|*Each stack of Time Weakness increases damage dealt by Detonation by @./\n|*Damage over Time ticks now only applies Time Weakness stacks but no longer deal damage.",
    "EN_LORE": "Let time do its work.",
    "EN": "$ of Detonating Force//*+@ £*+@ £|*Whenever you deal Damage over Time to an Enemy, you apply a Time Weakness stack, up to @.#Each stack of Time Weakness increases Damage over Time Damage received by @.*act:229/\n|*Each stack of Time Weakness increases damage dealt by Detonation by @./\n|*Damage over Time ticks now only applies Time Weakness stacks but no longer deal damage.//Let time do its work.",
    "FR": "$ de Force Détonante//*@ £*+@ £|*Chaque fois que vous infligez des Dégâts sur la Durée à un ennemi, vous appliquez un cumul de Faiblesse Temporelle, jusqu'à @.#Chaque cumul de Faiblesse Temporelle augmente les Dégâts sur la Durée reçus de @.*act:229/\n|*Chaque cumul de Faiblesse Temporelle augmente les dégâts infligés par Détonation de @./\n|*Les ticks de Dégâts sur la Durée n'appliquent désormais que des cumuls de Faiblesse Temporelle mais n'infligent plus de dégâts.//Laisser le temps faire son oeuvre.",
    "FR_NAME": "$ de Force Détonante",
    "FR_DESC": "*@ £*+@ £|*Chaque fois que vous infligez des Dégâts sur la Durée à un ennemi, vous appliquez un cumul de Faiblesse Temporelle, jusqu'à @.#Chaque cumul de Faiblesse Temporelle augmente les Dégâts sur la Durée reçus de @.*act:229/\n|*Chaque cumul de Faiblesse Temporelle augmente les dégâts infligés par Détonation de @./\n|*Les ticks de Dégâts sur la Durée n'appliquent désormais que des cumuls de Faiblesse Temporelle mais n'infligent plus de dégâts.",
    "FR_LORE": "Laisser le temps faire son oeuvre.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 4,
    "SPRITE": 51,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|50|3|\n4|",
    "VALUE_TYPE": "%|%||%|\n%|",
    "VALUE_LEVEL": "rl|rl|||\n|",
    "VALUE_STAT": "dot_increased_damage_percent|dot_increased_damage_global_mult",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 16,
    "BASE_DMG_MAX": 18,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 5,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1150 - 1396"
  },
  {
    "REF": 5,
    "EN_NAME": "$ of Relentless Transferance",
    "EN_DESC": "|*Indirect Damage have:#+@ £#+@ £*Indirect Damage deal _ Additional Critical Strike Damage. Additional Critical Strike Damage is equal to your $.*Overkill Indirect Damage is stored and dealt as Additional Damage the next time you deal Indirect Damage.*Indirect Damage can now Leech Life and Mana.*Indirect Damage is every damage that hasn't been dealt by your Primary or Secondary Skill./\n|*Whenever you kill an enemy with Indirect Damage, you gain an Indirect Defense stack, up to @, that lasts @ seconds.*Whenever you're dealt damage, that Damage cannot exceed _.#Damage being @ of your Max Life minus @ for each of your Indirect Defense stacks.*Whenever Indirect Defense reduces Damage this way by more than @ of its base value, you lose @ stacks of Indirect Defense./\n|*Your Primary and Secondary Skill deal @ Decreased Damage.",
    "EN_LORE": "A passive aggressive Slorm Reaper.",
    "EN": "$ of Relentless Transferance//|*Indirect Damage have:#+@ £#+@ £*Indirect Damage deal _ Additional Critical Strike Damage. Additional Critical Strike Damage is equal to your $.*Overkill Indirect Damage is stored and dealt as Additional Damage the next time you deal Indirect Damage.*Indirect Damage can now Leech Life and Mana.*Indirect Damage is every damage that hasn't been dealt by your Primary or Secondary Skill./\n|*Whenever you kill an enemy with Indirect Damage, you gain an Indirect Defense stack, up to @, that lasts @ seconds.*Whenever you're dealt damage, that Damage cannot exceed _.#Damage being @ of your Max Life minus @ for each of your Indirect Defense stacks.*Whenever Indirect Defense reduces Damage this way by more than @ of its base value, you lose @ stacks of Indirect Defense./\n|*Your Primary and Secondary Skill deal @ Decreased Damage.//A passive aggressive Slorm Reaper.",
    "FR": "$ de Transfert Implacable//|*Les Dégâts Indirects ont :#+@ de £#+@ de £*Les Dégâts Indirects infligent _ Dégâts Critiques Additionnels. Les Dégâts Critiques Additionnels sont équivalents à votre $.*Les Dégâts Excédentaires des Dégâts Indirects sont stockés et infligés comme Dégâts Additionnels la prochaine fois que vous infligez des Dégâts Indirects.*Les Dégâts Indirects peuvent désormais Voler de la Vie et du Mana.*Les Dégâts Indirects englobent tous les dégâts qui n'ont pas été infligés par votre Compétence Primaire ou Secondaire./\n|*Chaque fois que vous tuez un ennemi avec des Dégâts Indirects, vous gagnez un cumul de Défense Indirecte, jusqu'à @, qui dure @ secondes.*Chaque fois que vous subissez des dégâts, ces dégâts ne peuvent pas dépasser _.#Ces dégâts sont équivalents à @ de votre Vie Max moins @ pour chaque cumul de Défense Indirecte.*Chaque fois que Défense Indirecte réduit les dégâts de plus de @ de la valeur de base, vous perdez @ cumuls de Défense Indirecte./\n|*Vos Compétences Primaires et Secondaires infligent des Dégâts Réduits de @.//Un Slorm Reaper passif agressif.",
    "FR_NAME": "$ de Transfert Implacable",
    "FR_DESC": "|*Les Dégâts Indirects ont :#+@ de £#+@ de £*Les Dégâts Indirects infligent _ Dégâts Critiques Additionnels. Les Dégâts Critiques Additionnels sont équivalents à votre $.*Les Dégâts Excédentaires des Dégâts Indirects sont stockés et infligés comme Dégâts Additionnels la prochaine fois que vous infligez des Dégâts Indirects.*Les Dégâts Indirects peuvent désormais Voler de la Vie et du Mana.*Les Dégâts Indirects englobent tous les dégâts qui n'ont pas été infligés par votre Compétence Primaire ou Secondaire./\n|*Chaque fois que vous tuez un ennemi avec des Dégâts Indirects, vous gagnez un cumul de Défense Indirecte, jusqu'à @, qui dure @ secondes.*Chaque fois que vous subissez des dégâts, ces dégâts ne peuvent pas dépasser _.#Ces dégâts sont équivalents à @ de votre Vie Max moins @ pour chaque cumul de Défense Indirecte.*Chaque fois que Défense Indirecte réduit les dégâts de plus de @ de la valeur de base, vous perdez @ cumuls de Défense Indirecte./\n|*Vos Compétences Primaires et Secondaires infligent des Dégâts Réduits de @.",
    "FR_LORE": "Un Slorm Reaper passif agressif.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 6,
    "SPRITE": 18,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.1|\n90|15|100|1|50|10|\n50|",
    "VALUE_TYPE": "%|%|\n||%|%|%||\n%|",
    "VALUE_LEVEL": "rl|rl|\n||||||\n|",
    "VALUE_STAT": "increased_damage|crit_chance_percent",
    "VALUE_REAL": "synergy:physical_damage:100|\nsynergy:indirect_defense:1|",
    "BASE_DMG_MIN": 11,
    "BASE_DMG_MAX": 15,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1943 - 2535"
  },
  {
    "REF": 6,
    "EN_NAME": "Big Fury $",
    "EN_DESC": "*+@ £*+@ £*+@ £|*Your Skill Damage are <Lucky>./\n*+@ £|*You have +_% £.#Elemental Penetration is equal to your $./\n|*Skills cannot be Imbued.",
    "EN_LORE": "\"That's my secret Captain...#\nI'm always Angry !\"",
    "EN": "Big Fury $//*+@ £*+@ £*+@ £|*Your Skill Damage are <Lucky>./\n*+@ £|*You have +_% £.#Elemental Penetration is equal to your $./\n|*Skills cannot be Imbued.//\"That's my secret Captain...#\nI'm always Angry !\"",
    "FR": "$ d'Immense Fureur//*+@ £*+@ £*+@ £|*Les Dégâts Physiques sont <Chanceux>./\n*+@ £|*Vous avez +_% de £.#Cette Pénétration Élémentaire est équivalente à votre $./\n|*Vos Compétences ne peuvent plus être Imprégnées.//\"Le voilà mon secret Captain...#\nJe n'ai jamais cessé d'être en Colère !\"",
    "FR_NAME": "$ d'Immense Fureur",
    "FR_DESC": "*+@ £*+@ £*+@ £|*Les Dégâts Physiques sont <Chanceux>./\n*+@ £|*Vous avez +_% de £.#Cette Pénétration Élémentaire est équivalente à votre $./\n|*Vos Compétences ne peuvent plus être Imprégnées.",
    "FR_LORE": "\"Le voilà mon secret Captain...#\nJe n'ai jamais cessé d'être en Colère !\"",
    "CH": "愤怒的$//*+@£*+@£*+@£|*你的技能伤害变为<Lucky>。/\n*+@£|*你获得+_%£。#元素穿透等于你的$。/\n|*技能无法被附魔。//“那是我的秘密，船长……#\n我总是很愤怒！”",
    "CH_NAME": "愤怒的$",
    "CH_DESCRIPTION": "*+@£*+@£*+@£|*你的技能伤害变为<Lucky>。/\n*+@£|*你获得+_%£。#元素穿透等于你的$。/\n|*技能无法被附魔。",
    "CH_LORE": "“那是我的秘密，船长……#\n我总是很愤怒！”",
    "TW": "憤怒的$//*+@£*+@£*+@£|*你的技能傷害變為<Lucky>。 /\n*+@£|*你獲得+_%£。 #元素穿透等於你的$。 /\n|*技能無法被附魔。//“那是我的秘密，船長……#\n我總是很憤怒！ ”",
    "TW_NAME": "憤怒的$",
    "TW_DESCRIPTION": "*+@£*+@£*+@£|*你的技能傷害變為<Lucky>。 /\n*+@£|*你獲得+_%£。 #元素穿透等於你的$。 /\n|*技能無法被附魔。",
    "TW_LORE": "“那是我的秘密，船長……#\n我總是很憤怒！ ”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 5,
    "SPRITE": 6,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|4|0.5|\n3|",
    "VALUE_TYPE": "||%|\n%|",
    "VALUE_LEVEL": "rl|rl|rl|\nrl|",
    "VALUE_STAT": "min_basic_damage_add|max_basic_damage_add|basic_damage_global_mult|\nmax_basic_damage_percent|elemental_penetration_percent",
    "VALUE_REAL": "synergy:armor_penetration:1",
    "BASE_DMG_MIN": 26,
    "BASE_DMG_MAX": 32,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2032 - 2920"
  },
  {
    "REF": 7,
    "EN_NAME": "$ of the Master",
    "EN_DESC": "*+@ £|*Your Primary Skill has +@ Mastery Level./\n|*Effects from this Slorm Reaper synergyzing on Mastery now also count the Mastery of every other Character./\n|*You can no longer gain Mastery.",
    "EN_LORE": "If you want to be the very best!",
    "EN": "$ of the Master//*+@ £|*Your Primary Skill has +@ Mastery Level./\n|*Effects from this Slorm Reaper synergyzing on Mastery now also count the Mastery of every other Character./\n|*You can no longer gain Mastery.//If you want to be the very best!",
    "FR": "$ du Maître//*+@ £|*Votre Compétence Primaire a +@ de Niveau de Maitrise./\n|*Les effets de ce Slorm Reaper synergisant sur la Maîtrise comptent désormais également la Maîtrise de chaque autre Personnage./\n|*Vous ne pouvez plus gagner de Maîtrise.//Si vous voulez devenir le meilleur dresseur !",
    "FR_NAME": "$ du Maître",
    "FR_DESC": "*+@ £|*Votre Compétence Primaire a +@ de Niveau de Maitrise./\n|*Les effets de ce Slorm Reaper synergisant sur la Maîtrise comptent désormais également la Maîtrise de chaque autre Personnage./\n|*Vous ne pouvez plus gagner de Maîtrise.",
    "FR_LORE": "Si vous voulez devenir le meilleur dresseur !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 7,
    "SPRITE": 19,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|5",
    "VALUE_TYPE": "%|",
    "VALUE_LEVEL": "|",
    "VALUE_STAT": "skill_mastery_gain_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 5,
    "BASE_DMG_MAX": 12,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 24,
    "EVOLVE_IN": 8,
    "100": "771 - 1290"
  },
  {
    "REF": 8,
    "EN_NAME": "$ of the Masterful Master",
    "EN_DESC": "|*You have +_% £.#Armor is equal to @ of the total of all your Masteries on this Character.*You have +_% £.#Elemental Resistance is equal to @ of the total of all your Masteries on this Character.",
    "EN_LORE": "Like no one ever was!",
    "EN": "$ of the Masterful Master//|*You have +_% £.#Armor is equal to @ of the total of all your Masteries on this Character.*You have +_% £.#Elemental Resistance is equal to @ of the total of all your Masteries on this Character.//Like no one ever was!",
    "FR": "$ du Maître Magistral//|*Vous avez +_% de £.#Cette Armure est équivalente à @ du niveau total de vos Maîtrises sur ce Personnage.*Vous avez +_% de £.#Cette Résistance Élémentaire est équivalente à @ du niveau total de vos Maîtrises sur ce Personnage.//Vous battre sans répit !",
    "FR_NAME": "$ du Maître Magistral",
    "FR_DESC": "|*Vous avez +_% de £.#Cette Armure est équivalente à @ du niveau total de vos Maîtrises sur ce Personnage.*Vous avez +_% de £.#Cette Résistance Élémentaire est équivalente à @ du niveau total de vos Maîtrises sur ce Personnage.",
    "FR_LORE": "Vous battre sans répit !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 8,
    "SPRITE": 20,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|0.5",
    "VALUE_TYPE": "%|%",
    "VALUE_LEVEL": "rl|rl",
    "VALUE_STAT": "res_phy_global_mult|res_mag_global_mult",
    "VALUE_REAL": "synergy:all_masteries:0.5*rl|synergy:all_masteries:0.5*rl",
    "BASE_DMG_MIN": 5,
    "BASE_DMG_MAX": 12,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 40,
    "EVOLVE_IN": 9,
    "100": "1094 - 1847"
  },
  {
    "REF": 9,
    "EN_NAME": "$ of the Masterful Great Grandmaster",
    "EN_DESC": "|*Every @ Primary Casts, your Primary Skill deals _% £.#Increased Damage is equal to @ of the total of all your Masteries on this character.",
    "EN_LORE": "To catch them is your real test! To train them is your cause!",
    "EN": "$ of the Masterful Great Grandmaster//|*Every @ Primary Casts, your Primary Skill deals _% £.#Increased Damage is equal to @ of the total of all your Masteries on this character.//To catch them is your real test! To train them is your cause!",
    "FR": "$ du Grand Maître Magistral//|*Toutes les @ Compétences Primaires lancées, votre Compétence Primaire inflige _% de £.#Ces Dégâts Supplémentaires sont équivalents à @ du niveau total de vos Maîtrises sur ce Personnage.//Tout faire pour être vainqueur, et gagner les défis !",
    "FR_NAME": "$ du Grand Maître Magistral",
    "FR_DESC": "|*Toutes les @ Compétences Primaires lancées, votre Compétence Primaire inflige _% de £.#Ces Dégâts Supplémentaires sont équivalents à @ du niveau total de vos Maîtrises sur ce Personnage.",
    "FR_LORE": "Tout faire pour être vainqueur, et gagner les défis !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 9,
    "SPRITE": 21,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3|0.5",
    "VALUE_TYPE": "|%",
    "VALUE_LEVEL": "|rl",
    "VALUE_STAT": "increased_damage",
    "VALUE_REAL": "synergy:all_masteries:1*rl",
    "BASE_DMG_MIN": 5,
    "BASE_DMG_MAX": 12,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1329 - 2220"
  },
  {
    "REF": 10,
    "EN_NAME": "$ of The Wicked Snare",
    "EN_DESC": "*+@ £#*+@ £*+@ £|*Whenever you cast an Area of Effect or an Ancestral Seal, you also trigger multiplie Area Projectiles from the border of the Area and moving toward its center, piercing and dealing _ $ to enemies hit.#The number of Area Projectiles is based on the size of the Area, up to @.##The Duration of Area Projectiles is increased by @.##Area Projectiles deal _% £.#Increased Damage is equal to your Area Increased Effect./\n*+@ £|/\n|*Damage dealt by Areas of Effect cast from your Primary, Secondary or Support Skill is reduced to 0.",
    "EN_LORE": "With this weapon, we become masters of deceit...\"",
    "EN": "$ of The Wicked Snare//*+@ £#*+@ £*+@ £|*Whenever you cast an Area of Effect or an Ancestral Seal, you also trigger multiplie Area Projectiles from the border of the Area and moving toward its center, piercing and dealing _ $ to enemies hit.#The number of Area Projectiles is based on the size of the Area, up to @.##The Duration of Area Projectiles is increased by @.##Area Projectiles deal _% £.#Increased Damage is equal to your Area Increased Effect./\n*+@ £|/\n|*Damage dealt by Areas of Effect cast from your Primary, Secondary or Support Skill is reduced to 0.//With this weapon, we become masters of deceit...\"",
    "FR": "$ du Piège Malicieux//*+@ £#*+@ £*+@ £|*Chaque fois que vous lancez une Zone d'Effet ou un Sceau Ancestral, vous déclenchez également plusieurs Projectiles de Zone depuis le bord de la zone et se déplaçant vers son centre, perçant et infligeant _ de $ aux ennemis touchés.#Le nombre de Projectiles de Zone est basé sur votre Taille de Zone d'Effet, jusqu'à @.##La Durée des Projectiles de Zone est augmentée de @.##Les Projectiles de Zone infligent _% de £.#Ces Dégâts Supplémentaires sont équivalents à votre Effet de Zone./\n*+@ £|/\n|*Les dégâts infligés par les Zones d'Effet de vos Compétences Primaires, Secondaires ou de Support sont réduits à 0.//\"Avec cette arme, nous devenons les maîtres de la tromperie...\"",
    "FR_NAME": "$ du Piège Malicieux",
    "FR_DESC": "*+@ £#*+@ £*+@ £|*Chaque fois que vous lancez une Zone d'Effet ou un Sceau Ancestral, vous déclenchez également plusieurs Projectiles de Zone depuis le bord de la zone et se déplaçant vers son centre, perçant et infligeant _ de $ aux ennemis touchés.#Le nombre de Projectiles de Zone est basé sur votre Taille de Zone d'Effet, jusqu'à @.##La Durée des Projectiles de Zone est augmentée de @.##Les Projectiles de Zone infligent _% de £.#Ces Dégâts Supplémentaires sont équivalents à votre Effet de Zone./\n*+@ £|/\n|*Les dégâts infligés par les Zones d'Effet de vos Compétences Primaires, Secondaires ou de Support sont réduits à 0.",
    "FR_LORE": "\"Avec cette arme, nous devenons les maîtres de la tromperie...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 10,
    "SPRITE": 10,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.25|1|0.15|16|1|\n1|\n0",
    "VALUE_TYPE": "%|%|%||%|\n%|",
    "VALUE_LEVEL": "rl|rl|rl||rl|\nrl|",
    "VALUE_STAT": "aoe_increased_effect_percent|additional_projectile_percent|rebound_chance_percent|increased_damage|\naoe_increased_effect_global_mult",
    "VALUE_REAL": "damage:physical_damage:75|synergy:area_of_effect:1",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 20,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 46,
    "EVOLVE_IN": 11,
    "100": "956 - 1478"
  },
  {
    "REF": 11,
    "EN_NAME": "$ of The Surrounding Wicked Snare",
    "EN_DESC": "|*Whenever you cast an Area of Effect, you also trigger the same number of Area Projectiles from around you.",
    "EN_LORE": "\"... leading our enemies to their own downfall with each strike.\"#- An inspired Champion.",
    "EN": "$ of The Surrounding Wicked Snare//|*Whenever you cast an Area of Effect, you also trigger the same number of Area Projectiles from around you.//\"... leading our enemies to their own downfall with each strike.\"#- An inspired Champion.",
    "FR": "$ du Piège Malicieux Ambiant//|*Chaque fois que vous lancez une Zone d'Effet, vous déclenchez également le même nombre de Projectiles de Zone autour de vous.//\"... dirigeant nos ennemis vers leur propre chute avec chaque coup.\"#- Un Champion inspiré.",
    "FR_NAME": "$ du Piège Malicieux Ambiant",
    "FR_DESC": "|*Chaque fois que vous lancez une Zone d'Effet, vous déclenchez également le même nombre de Projectiles de Zone autour de vous.",
    "FR_LORE": "\"... dirigeant nos ennemis vers leur propre chute avec chaque coup.\"#- Un Champion inspiré.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 11,
    "SPRITE": 11,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 20,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1369 - 2118"
  },
  {
    "REF": 12,
    "EN_NAME": "$ of Support",
    "EN_DESC": "|*If applicable, your Support Skill gains the following Support Effects:#+@ £#+@ Increased Radius#+@ Increased Duration*Whenever you cast your Support Skill, you also cast a Support Eruption, dealing _ £ to enemies within a ~ yard radius.#Support Eruption benefits from Support Effects.*Whenever you cast your Support Skill, you gain a Support Streak stack, up to @.#Support Streak lasts @ seconds but its Cooldown starts once the Cooldown of your Support Skill has ended.#Each stack of Support Streak increases the effect of Support Effects by @.*Above @ Support Streak stacks, Support Streak only lasts @ second./\n|*You gain the Passive Effects of all your other Non-Equipped Specializations./\n|*You no longer benefit from Passive Effects from your current Specialization (Upgrades still work).",
    "EN_LORE": "Wrought by Cory, while he was Gafonnon's apprentice,...",
    "EN": "$ of Support//|*If applicable, your Support Skill gains the following Support Effects:#+@ £#+@ Increased Radius#+@ Increased Duration*Whenever you cast your Support Skill, you also cast a Support Eruption, dealing _ £ to enemies within a ~ yard radius.#Support Eruption benefits from Support Effects.*Whenever you cast your Support Skill, you gain a Support Streak stack, up to @.#Support Streak lasts @ seconds but its Cooldown starts once the Cooldown of your Support Skill has ended.#Each stack of Support Streak increases the effect of Support Effects by @.*Above @ Support Streak stacks, Support Streak only lasts @ second./\n|*You gain the Passive Effects of all your other Non-Equipped Specializations./\n|*You no longer benefit from Passive Effects from your current Specialization (Upgrades still work).//Wrought by Cory, while he was Gafonnon's apprentice,...",
    "FR": "$ de Soutien//|*Si applicable, votre Compétence de Soutien obtient les Effets de Soutien suivants :#+@ de £#+@ de Rayon Augmenté#+@ de Durée Augmentée*Chaque fois que vous lancez votre Compétence de Soutien, vous lancez également une Éruption de Soutien, infligeant _ de £ aux ennemis dans un rayon d'environ ~ mètres.#L'Éruption de Soutien bénéficie des Effets de Soutien.*Chaque fois que vous lancez votre Compétence de Soutien, vous gagnez un cumul de Soutien en Série, jusqu'à @.#Soutien en Série dure @ secondes, mais son Temps de Recharge commence une fois le temps de recharge de votre Compétence de Soutien terminé.#Chaque cumul de Soutien en Série augmente l'effet des Effets de Soutien de @.*Au-delà de @ cumuls de Soutien en Série, Soutien en Série ne dure que @ secondes./\n|*Vous bénéficiez des Effets Passifs de toutes vos Spécialisations non-équipées./\n|*Vous ne bénéficiez plus des Effets Passifs de votre Spécialisation actuelle (les Améliorations fonctionnent toujours).//Forgé par Smaloron, alors qu'il était l'apprenti de Gafonnon...",
    "FR_NAME": "$ de Soutien",
    "FR_DESC": "|*Si applicable, votre Compétence de Soutien obtient les Effets de Soutien suivants :#+@ de £#+@ de Rayon Augmenté#+@ de Durée Augmentée*Chaque fois que vous lancez votre Compétence de Soutien, vous lancez également une Éruption de Soutien, infligeant _ de £ aux ennemis dans un rayon d'environ ~ mètres.#L'Éruption de Soutien bénéficie des Effets de Soutien.*Chaque fois que vous lancez votre Compétence de Soutien, vous gagnez un cumul de Soutien en Série, jusqu'à @.#Soutien en Série dure @ secondes, mais son Temps de Recharge commence une fois le temps de recharge de votre Compétence de Soutien terminé.#Chaque cumul de Soutien en Série augmente l'effet des Effets de Soutien de @.*Au-delà de @ cumuls de Soutien en Série, Soutien en Série ne dure que @ secondes./\n|*Vous bénéficiez des Effets Passifs de toutes vos Spécialisations non-équipées./\n|*Vous ne bénéficiez plus des Effets Passifs de votre Spécialisation actuelle (les Améliorations fonctionnent toujours).",
    "FR_LORE": "Forgé par Smaloron, alors qu'il était l'apprenti de Gafonnon...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 12,
    "SPRITE": 7,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.5|0.3|50|3|3|30|1",
    "VALUE_TYPE": "%|%|%|||%|||",
    "VALUE_LEVEL": "rl|rl|rl||||||",
    "VALUE_STAT": "increased_damage|physical_damage",
    "VALUE_REAL": "damage:physical_damage:100",
    "BASE_DMG_MIN": 21,
    "BASE_DMG_MAX": 31,
    "MIN_DMG_LVL": 9,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 1,
    "MAX_LVL": 16,
    "EVOLVE_IN": 13,
    "100": "1390 - 1694"
  },
  {
    "REF": 13,
    "EN_NAME": "Great $ of Support",
    "EN_DESC": "|*Whenever the Cooldown of your Support Skill should start, it has a _% Chance to be reduced to @.#This Chance is equal to your current number of Support Streak stacks.",
    "EN_LORE": "... this Reaper was supposed to help the wearer to fill obvious flaws...",
    "EN": "Great $ of Support//|*Whenever the Cooldown of your Support Skill should start, it has a _% Chance to be reduced to @.#This Chance is equal to your current number of Support Streak stacks.//... this Reaper was supposed to help the wearer to fill obvious flaws...",
    "FR": "$ Supérieur de Soutien/$ Supérieure de Soutien//|*Chaque fois que le Temps de Recharge de votre Compétence de Soutien devrait commencer, il a une Chance de _% d'être réduit à @.#Cette chance est équivalente à votre cumul de Soutien en Série.//... Ce Reaper devait permettre à celui qui le porte de combler des lacunes évidentes...",
    "FR_NAME": "$ Supérieur de Soutien/$ Supérieure de Soutien",
    "FR_DESC": "|*Chaque fois que le Temps de Recharge de votre Compétence de Soutien devrait commencer, il a une Chance de _% d'être réduit à @.#Cette chance est équivalente à votre cumul de Soutien en Série.",
    "FR_LORE": "... Ce Reaper devait permettre à celui qui le porte de combler des lacunes évidentes...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 13,
    "SPRITE": 8,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:support_streak:1",
    "BASE_DMG_MIN": 21,
    "BASE_DMG_MAX": 31,
    "MIN_DMG_LVL": 10,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 32,
    "EVOLVE_IN": 14,
    "100": "1534 - 1838"
  },
  {
    "REF": 14,
    "EN_NAME": "Precise Great $ of Support",
    "EN_DESC": "|*Casting your Support Skill while Support Streak has less than @ second Cooldown grants @ Support Streak stacks instead.",
    "EN_LORE": "... The Reaper of the Master is the first iteration of this concept. - Reapersmiths through the ages, by Professor Geranium.",
    "EN": "Precise Great $ of Support//|*Casting your Support Skill while Support Streak has less than @ second Cooldown grants @ Support Streak stacks instead.//... The Reaper of the Master is the first iteration of this concept. - Reapersmiths through the ages, by Professor Geranium.",
    "FR": "$ Précis Supérieur de Soutien/$ Précise Supérieure de Soutien//|*Lancer votre Compétence de Soutien alors que Soutien en Série a un Temps de Recharge inférieur à @ seconde vous confère @ cumuls de Soutien en Série à la place.//... le Reaper du Maître est la première itération de ce concept. - Les Reapergerons à travers les âges, par le Professeur Geranium.",
    "FR_NAME": "$ Précis Supérieur de Soutien/$ Précise Supérieure de Soutien",
    "FR_DESC": "|*Lancer votre Compétence de Soutien alors que Soutien en Série a un Temps de Recharge inférieur à @ seconde vous confère @ cumuls de Soutien en Série à la place.",
    "FR_LORE": "... le Reaper du Maître est la première itération de ce concept. - Les Reapergerons à travers les âges, par le Professeur Geranium.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 14,
    "SPRITE": 9,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|5",
    "VALUE_TYPE": "|",
    "VALUE_LEVEL": "|",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 21,
    "BASE_DMG_MAX": 31,
    "MIN_DMG_LVL": 11,
    "MAX_DMG_LVL": 13,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1675 - 1982"
  },
  {
    "REF": 15,
    "EN_NAME": "Tyler’s Favorite $",
    "EN_DESC": "|*Tyler, Bryan's Bunny now fights alongside you.*The first Projectile Skill of your Class now throws Vegetables at enemies.*Whenever you throw a Vegetable, it has a @ £ to then drop on the floor after hitting an enemy. Tyler immediately gets there and eats the Vegetable, dealing _ £ to enemies within a ~ yard radius while doing so in a Messy Explosion.*Whenever Tyler eats a Vegetable, you also gain a Well-Fed stack, up to @. Tyler deals @ £ for every Well-Fed stack you have.*act:230/\n|*Whenever Go Tyler, Go! fails, you can pick up the...\"waste\" to restore your Well-Fed stacks.*Whenever Tyler eats a Vegetable, he now also eats all Vegetables within a @ yard radius, increasing the Damage dealt by Messy Explosion by @ for every eaten Vegetable./\n|*When casting Go, Tyler Go!, its Chance to \"fail\" is doubled.",
    "EN_LORE": "- \"Is that...a vegetable?\"#-\"Yes...yes it is\"",
    "EN": "Tyler’s Favorite $//|*Tyler, Bryan's Bunny now fights alongside you.*The first Projectile Skill of your Class now throws Vegetables at enemies.*Whenever you throw a Vegetable, it has a @ £ to then drop on the floor after hitting an enemy. Tyler immediately gets there and eats the Vegetable, dealing _ £ to enemies within a ~ yard radius while doing so in a Messy Explosion.*Whenever Tyler eats a Vegetable, you also gain a Well-Fed stack, up to @. Tyler deals @ £ for every Well-Fed stack you have.*act:230/\n|*Whenever Go Tyler, Go! fails, you can pick up the...\"waste\" to restore your Well-Fed stacks.*Whenever Tyler eats a Vegetable, he now also eats all Vegetables within a @ yard radius, increasing the Damage dealt by Messy Explosion by @ for every eaten Vegetable./\n|*When casting Go, Tyler Go!, its Chance to \"fail\" is doubled.//- \"Is that...a vegetable?\"#-\"Yes...yes it is\"",
    "FR": "$ Favori de Tyler/$ Favorie de Tyler//|*Tyler, le lapin de Bryan, se bat maintenant à vos côtés.*La première Compétence de Projectile de votre Classe lance désormais des Légumes sur les ennemist.*Chaque fois que vous lancez un Légume, il a @ de £ de tomber au sol après avoir touché quelque chose. Tyler arrive immédiatement et mange le Légume, infligeant _ de £ aux ennemis dans un rayon de ~ mètres en provoquant une Pagaille Désordonnée.*À chaque fois que Tyler mange un Légume, vous gagnez également un cumul de Bien Nourri, jusqu'à @. Tyler inflige @ de £ pour chaque cumul de Bien Nourri.*act:230/\n|*Chaque fois que En Avant Tyler ! échoue, vous pouvez ramasser le...\"déchet\" pour restaurer vos cumuls de Bien Nourri.*Chaque fois que Tyler mange un Légume, il mange maintenant aussi tous les Légumes dans un rayon de @ mètres, augmentant les dégâts infligés par Pagaille Désordonnée de @ pour chaque Légume mangé./\n|*Lors du lancement d'En Avant Tyler, sa chance d'échouer\" est doublée.//- \"Est-ce que c'est... un légume ?\"#-\"Oui... Oui c'en est un.\"",
    "FR_NAME": "$ Favori de Tyler/$ Favorie de Tyler",
    "FR_DESC": "|*Tyler, le lapin de Bryan, se bat maintenant à vos côtés.*La première Compétence de Projectile de votre Classe lance désormais des Légumes sur les ennemist.*Chaque fois que vous lancez un Légume, il a @ de £ de tomber au sol après avoir touché quelque chose. Tyler arrive immédiatement et mange le Légume, infligeant _ de £ aux ennemis dans un rayon de ~ mètres en provoquant une Pagaille Désordonnée.*À chaque fois que Tyler mange un Légume, vous gagnez également un cumul de Bien Nourri, jusqu'à @. Tyler inflige @ de £ pour chaque cumul de Bien Nourri.*act:230/\n|*Chaque fois que En Avant Tyler ! échoue, vous pouvez ramasser le...\"déchet\" pour restaurer vos cumuls de Bien Nourri.*Chaque fois que Tyler mange un Légume, il mange maintenant aussi tous les Légumes dans un rayon de @ mètres, augmentant les dégâts infligés par Pagaille Désordonnée de @ pour chaque Légume mangé./\n|*Lors du lancement d'En Avant Tyler, sa chance d'échouer\" est doublée.",
    "FR_LORE": "- \"Est-ce que c'est... un légume ?\"#-\"Oui... Oui c'en est un.\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 15,
    "SPRITE": 15,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "20|1|3|\n1|100",
    "VALUE_TYPE": "%||%|\n|%",
    "VALUE_LEVEL": "|rl||\n|",
    "VALUE_STAT": "chance|physical_damage|increased_damage",
    "VALUE_REAL": "damage:physical_damage:40",
    "BASE_DMG_MIN": 14,
    "BASE_DMG_MAX": 17,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": 2,
    "MAX_LVL": 38,
    "EVOLVE_IN": 16,
    "100": "1379 - 2544"
  },
  {
    "REF": 16,
    "EN_NAME": "Amazing Tyler's Favorite $",
    "EN_DESC": "|*Tyler now eats Vegetables @ faster.*Whenever Tyler eats a Vegetable, it has a @ Chance to not consume the Vegetable.",
    "EN_LORE": "Tyler is Bryan's bunny, he lives in the vegetable gardens up north and has his own Slorm Reaper.",
    "EN": "Amazing Tyler's Favorite $//|*Tyler now eats Vegetables @ faster.*Whenever Tyler eats a Vegetable, it has a @ Chance to not consume the Vegetable.//Tyler is Bryan's bunny, he lives in the vegetable gardens up north and has his own Slorm Reaper.",
    "FR": "$ Génialissime Favori de Tyler/$ Génialissime Favorie de Tyler//|*Tyler mange maintenant les Légumes @ plus rapidement.*Chaque fois que Tyler mange un Légume, il a @ de Chance de ne pas consommer le Légume.//Tyler est le lapin de Bryan, il vit dans les potagers au nord et possède son propre Reaper.",
    "FR_NAME": "$ Génialissime Favori de Tyler/$ Génialissime Favorie de Tyler",
    "FR_DESC": "|*Tyler mange maintenant les Légumes @ plus rapidement.*Chaque fois que Tyler mange un Légume, il a @ de Chance de ne pas consommer le Légume.",
    "FR_LORE": "Tyler est le lapin de Bryan, il vit dans les potagers au nord et possède son propre Reaper.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 16,
    "SPRITE": 16,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|25",
    "VALUE_TYPE": "%|%",
    "VALUE_LEVEL": "rl|",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 14,
    "BASE_DMG_MAX": 17,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1661 - 2835"
  },
  {
    "REF": 17,
    "EN_NAME": "Neverending Massacre $",
    "EN_DESC": "|*While on a Combo Streak, you have:#+@ £#+@ £*Slorm Find granted by the Combo Counter is doubled.*Disintegrate deals _% £.#Increased Damage is equal to the number of Victims of your Combo Counter above @.*The duration of Massacre is increased by @.*act:231/\n|*Whenever Massacre ends, you trigger Final Disintegration, dealing _ £ to enemies within a ~ yard radius.#Skill Damage is equal to your current Damage Per Second./\n|*The Cooldown Time of Massacre is paused while on a Combo Streak.",
    "EN_LORE": "The Monster Hunt is on!",
    "EN": "Neverending Massacre $//|*While on a Combo Streak, you have:#+@ £#+@ £*Slorm Find granted by the Combo Counter is doubled.*Disintegrate deals _% £.#Increased Damage is equal to the number of Victims of your Combo Counter above @.*The duration of Massacre is increased by @.*act:231/\n|*Whenever Massacre ends, you trigger Final Disintegration, dealing _ £ to enemies within a ~ yard radius.#Skill Damage is equal to your current Damage Per Second./\n|*The Cooldown Time of Massacre is paused while on a Combo Streak.//The Monster Hunt is on!",
    "FR": "$ de Massacre sans Fin//|*Lors d'un Combo, vous avez :#+@ £#+@ £*La Découverte de Slorm accordée par le Compteur de Combo est doublée.*Désintégration inflige _% de £.#Ces Dégâts Supplémentaires sont équivalents au nombre de Victimes de votre Compteur de Combo au-dessus de @.*La durée du Massacre est augmentée de @.*act:231/\n|*À la fin de Massacre, vous déclenchez la Désintégration Finale, infligeant _ de £ aux ennemis dans un rayon d'environ ~ mètres.#Ces Dégats sont équivalents à votre Dégâts par Seconde actuel./\n|*Le Temps de Recharge de Massacre est suspendu lors d'une Combo.//La Chasse aux Monstres est ouverte !",
    "FR_NAME": "$ de Massacre sans Fin",
    "FR_DESC": "|*Lors d'un Combo, vous avez :#+@ £#+@ £*La Découverte de Slorm accordée par le Compteur de Combo est doublée.*Désintégration inflige _% de £.#Ces Dégâts Supplémentaires sont équivalents au nombre de Victimes de votre Compteur de Combo au-dessus de @.*La durée du Massacre est augmentée de @.*act:231/\n|*À la fin de Massacre, vous déclenchez la Désintégration Finale, infligeant _ de £ aux ennemis dans un rayon d'environ ~ mètres.#Ces Dégats sont équivalents à votre Dégâts par Seconde actuel./\n|*Le Temps de Recharge de Massacre est suspendu lors d'une Combo.",
    "FR_LORE": "La Chasse aux Monstres est ouverte !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 17,
    "SPRITE": 109,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "15|25|100|1|",
    "VALUE_TYPE": "%|%||%|\n",
    "VALUE_LEVEL": "|||rl|",
    "VALUE_STAT": "cooldown_reduction_global_mult|the_speed_percent|increased_damage|\nphysical_damage",
    "VALUE_REAL": "synergy:victims_combo_100:1|\nsynergy:current_dps:1",
    "BASE_DMG_MIN": 26,
    "BASE_DMG_MAX": 32,
    "MIN_DMG_LVL": 9,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1665 - 2012"
  },
  {
    "REF": 18,
    "EN_NAME": "Energy Overflowing $",
    "EN_DESC": "*+@ £*+@ £*+@ £*+@ £|*Mana you Restore over your Max Mana is stored as Overflowing Mana.#Every @ seconds, Overflowing Mana is evenly distributed as Elemental Damage and increased by @ to up to @ random enemies within a @ yard radius.*Overflowing Mana has _% £.#Critical Strike Chance is equal to yours./\n|*Max Mana Modifiers are added to your Life instead of your Mana./\n|*Your Max Mana is @.",
    "EN_LORE": "Reap Different",
    "EN": "Energy Overflowing $//*+@ £*+@ £*+@ £*+@ £|*Mana you Restore over your Max Mana is stored as Overflowing Mana.#Every @ seconds, Overflowing Mana is evenly distributed as Elemental Damage and increased by @ to up to @ random enemies within a @ yard radius.*Overflowing Mana has _% £.#Critical Strike Chance is equal to yours./\n|*Max Mana Modifiers are added to your Life instead of your Mana./\n|*Your Max Mana is @.//Reap Different",
    "FR": "$ de Surcharge d'Énergie//*+@ £*+@ £*+@ £*+@ £|*Le Mana que vous restaurez au-delà de votre Mana Max est stocké comme Mana Débordant.#Toutes les @ secondes, le Mana Débordant est réparti équitablement sous forme de Dégâts Élémentaires et augmenté de @ jusqu'à @ ennemis aléatoires dans un rayon de @ mètres.*Le Mana Débordant a _% de £.#Cette Chance de Coup Critique est équivalente à la vôtre./\n|*Les Modificateurs de Mana Max affectent désormais votre Vie Max et plus votre Mana Max./\n|*Votre Mana Max est @.//Reapez Différent",
    "FR_NAME": "$ de Surcharge d'Énergie",
    "FR_DESC": "*+@ £*+@ £*+@ £*+@ £|*Le Mana que vous restaurez au-delà de votre Mana Max est stocké comme Mana Débordant.#Toutes les @ secondes, le Mana Débordant est réparti équitablement sous forme de Dégâts Élémentaires et augmenté de @ jusqu'à @ ennemis aléatoires dans un rayon de @ mètres.*Le Mana Débordant a _% de £.#Cette Chance de Coup Critique est équivalente à la vôtre./\n|*Les Modificateurs de Mana Max affectent désormais votre Vie Max et plus votre Mana Max./\n|*Votre Mana Max est @.",
    "FR_LORE": "Reapez Différent",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 18,
    "SPRITE": 17,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|1|1|1|2|10|5|7|\n\n1",
    "VALUE_TYPE": "|%|%|%||%|||\n\n",
    "VALUE_LEVEL": "rl|rl|rl|rl||rl|||",
    "VALUE_STAT": "mana_on_hit_add|mana_on_hit_global_mult|mana_on_kill_global_mult|mana_regen_global_mult|crit_chance_percent",
    "VALUE_REAL": "synergy:crit_chance:1",
    "BASE_DMG_MIN": 17,
    "BASE_DMG_MAX": 29,
    "MIN_DMG_LVL": 2,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "466 - 1166"
  },
  {
    "REF": 19,
    "EN_NAME": "$ of The Spectral Shapeshifter",
    "EN_DESC": "*+@ £*+@ £|*act:232/\n|*Spectral Shape also deals _ £ every second to enemies within range.*The Cooldown Time of Spectral Shape is reduced to @ second./\n|*The Mana cost of Spectral Shape becomes @ Mana.",
    "EN_LORE": "The shadows writhe around us all.",
    "EN": "$ of The Spectral Shapeshifter//*+@ £*+@ £|*act:232/\n|*Spectral Shape also deals _ £ every second to enemies within range.*The Cooldown Time of Spectral Shape is reduced to @ second./\n|*The Mana cost of Spectral Shape becomes @ Mana.//The shadows writhe around us all.",
    "FR": "$ du Changeforme Spectral//*+@ £*+@ £|*act:232/\n|*Forme Spectrale inflige également _ de £ chaque seconde aux ennemis à portée.*Le Temps de Recharge de Forme Spectrale est réduit à @ seconde./\n|*Le coût en Mana de Forme Spectrale devient @ Mana.//Les ombres se tordent autour de nous.",
    "FR_NAME": "$ du Changeforme Spectral",
    "FR_DESC": "*+@ £*+@ £|*act:232/\n|*Forme Spectrale inflige également _ de £ chaque seconde aux ennemis à portée.*Le Temps de Recharge de Forme Spectrale est réduit à @ seconde./\n|*Le coût en Mana de Forme Spectrale devient @ Mana.",
    "FR_LORE": "Les ombres se tordent autour de nous.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 19,
    "SPRITE": 12,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|0.5|\n1|\n100",
    "VALUE_TYPE": "%|%|\n|\n%",
    "VALUE_LEVEL": "rl|rl|\n|\n",
    "VALUE_STAT": "aoe_increased_effect_percent|weapon_damage_mult|\nelemental_damage",
    "VALUE_REAL": "damage:elemental_damage:150",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 19,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": 1,
    "MAX_LVL": 20,
    "EVOLVE_IN": 20,
    "100": "1654 - 2268"
  },
  {
    "REF": 20,
    "EN_NAME": "$ of the Spectral Horrifying Shapeshifter",
    "EN_DESC": "|*Whenever Spectral Shape deals damage to an enemy, you apply Horrified to the enemy hit, up to @ stacks.#Spectral Shape deals @ £ for each stack of Horrified on enemies hit.",
    "EN_LORE": "Twilight falls upon us all.",
    "EN": "$ of the Spectral Horrifying Shapeshifter//|*Whenever Spectral Shape deals damage to an enemy, you apply Horrified to the enemy hit, up to @ stacks.#Spectral Shape deals @ £ for each stack of Horrified on enemies hit.//Twilight falls upon us all.",
    "FR": "$ du Changeforme Spectral Horrifiant//|*Chaque fois que Forme Spectrale inflige des dégâts à un ennemi, vous appliquez Horreur à l'ennemi touché, jusqu'à @ cumuls.#Forme Spectrale inflige @ de £ pour chaque cumul d'Horreur sur les ennemis touchés.//Le crépuscule s'abat sur nous tous.",
    "FR_NAME": "$ du Changeforme Spectral Horrifiant",
    "FR_DESC": "|*Chaque fois que Forme Spectrale inflige des dégâts à un ennemi, vous appliquez Horreur à l'ennemi touché, jusqu'à @ cumuls.#Forme Spectrale inflige @ de £ pour chaque cumul d'Horreur sur les ennemis touchés.",
    "FR_LORE": "Le crépuscule s'abat sur nous tous.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 20,
    "SPRITE": 13,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "10|15",
    "VALUE_TYPE": "|%",
    "VALUE_LEVEL": "|",
    "VALUE_STAT": "increased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 19,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": null,
    "MAX_LVL": 36,
    "EVOLVE_IN": 21,
    "100": "1945 - 2552"
  },
  {
    "REF": 21,
    "EN_NAME": "$ of the Spectral Horrifying Shapeshifter of Dread",
    "EN_DESC": "|*Whenever an Horrified enemy leaves the range of Spectral Shape, you deal Skill Damage to him equal to the Damage dealt to him by Spectral Shape.",
    "EN_LORE": "The darkness coils within us all.",
    "EN": "$ of the Spectral Horrifying Shapeshifter of Dread//|*Whenever an Horrified enemy leaves the range of Spectral Shape, you deal Skill Damage to him equal to the Damage dealt to him by Spectral Shape.//The darkness coils within us all.",
    "FR": "$ du Changeforme Spectral Horrifiant d'Effroi//|*Chaque fois qu'un ennemi affecté par Horreur quitte la portée de Forme Spectrale, vous lui infligez des Dégâts de Compétence équivalents aux dégâts qui lui ont été infligés par Forme Spectrale.//Les ténèbres s'enroulent en chacun de nous.",
    "FR_NAME": "$ du Changeforme Spectral Horrifiant d'Effroi",
    "FR_DESC": "|*Chaque fois qu'un ennemi affecté par Horreur quitte la portée de Forme Spectrale, vous lui infligez des Dégâts de Compétence équivalents aux dégâts qui lui ont été infligés par Forme Spectrale.",
    "FR_LORE": "Les ténèbres s'enroulent en chacun de nous.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 21,
    "SPRITE": 14,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 19,
    "MIN_DMG_LVL": 7,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2220 - 2843"
  },
  {
    "REF": 22,
    "EN_NAME": "Mana Harvesting $",
    "EN_DESC": "*+@ £*+@ £|*You can Leech Mana with Mana Harvest.#The size of Mana Harvest is increased by @.*act:213/\n|*The Mana Harvest area now lasts @ seconds.#While you stand inside the Harvest area, Mana Spark continuously focuses new targets.#When Mana Harvest ends, if you are inside the Harvest area, it triggers an explosion dealing _ £ to enemies inside the Harvest area./\n|*Mana Regeneration now consumes Mana instead of Regenerating it.",
    "EN_LORE": "Wishing you an excellent vintage.",
    "EN": "Mana Harvesting $//*+@ £*+@ £|*You can Leech Mana with Mana Harvest.#The size of Mana Harvest is increased by @.*act:213/\n|*The Mana Harvest area now lasts @ seconds.#While you stand inside the Harvest area, Mana Spark continuously focuses new targets.#When Mana Harvest ends, if you are inside the Harvest area, it triggers an explosion dealing _ £ to enemies inside the Harvest area./\n|*Mana Regeneration now consumes Mana instead of Regenerating it.//Wishing you an excellent vintage.",
    "FR": "$ de Récolte de Mana//*+@ £*+@ £|*Récolte de Mana bénéficie de Vol de Mana.#La taille de Récolte de Mana est augmentée de @.*act:213/\n|*La zone de Récolte de Mana dure désormais @ secondes.#Tant que vous êtes à l'intérieur de la zone de Récolte, l'Étincelle de Mana vise continuellement de nouvelles cibles.#Quand Récolte de Mana se termine, si vous êtes à l'intérieur de la zone de Récolte, vous déclenchez une explosion infligeant _ £ aux ennemis à l'intérieur de la zone de Récolte./\n|*Votre Régénération de Mana consomme désormais du Mana au lieu de le régénérer.//En vous souhaitant un excellent millésime.",
    "FR_NAME": "$ de Récolte de Mana",
    "FR_DESC": "*+@ £*+@ £|*Récolte de Mana bénéficie de Vol de Mana.#La taille de Récolte de Mana est augmentée de @.*act:213/\n|*La zone de Récolte de Mana dure désormais @ secondes.#Tant que vous êtes à l'intérieur de la zone de Récolte, l'Étincelle de Mana vise continuellement de nouvelles cibles.#Quand Récolte de Mana se termine, si vous êtes à l'intérieur de la zone de Récolte, vous déclenchez une explosion infligeant _ £ aux ennemis à l'intérieur de la zone de Récolte./\n|*Votre Régénération de Mana consomme désormais du Mana au lieu de le régénérer.",
    "FR_LORE": "En vous souhaitant un excellent millésime.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 22,
    "SPRITE": 22,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.1|1|0.5|\n4|2",
    "VALUE_TYPE": "%|%|%|\n|",
    "VALUE_LEVEL": "rl|rl|rl|\n|",
    "VALUE_STAT": "mana_leech_percent|mana_leech_global_mult|physical_damage",
    "VALUE_REAL": "damage:physical_damage:300",
    "BASE_DMG_MIN": 26,
    "BASE_DMG_MAX": 34,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "824 - 1847"
  },
  {
    "REF": 23,
    "EN_NAME": "$ of Concentrated Inner Strength",
    "EN_DESC": "*+@ £*|*You have +_ £.#Inner Fire Damage is equal to @ of your $.*You have +_ £.#Inner Fire Duration is equal to your $.*act:214*act:215/\n*+@ £|/\n|*Reaper Damage is now added to Inner Fire Damage, instead of Skill Damage.",
    "EN_LORE": "Juggling with Fire.",
    "EN": "$ of Concentrated Inner Strength//*+@ £*|*You have +_ £.#Inner Fire Damage is equal to @ of your $.*You have +_ £.#Inner Fire Duration is equal to your $.*act:214*act:215/\n*+@ £|/\n|*Reaper Damage is now added to Inner Fire Damage, instead of Skill Damage.//Juggling with Fire.",
    "FR": "$ de Force Intérieure Concentrée//*+@ £*|*Vous avez +_ £.#Ces Dégâts de Feu Intérieur sont équivalents à @ de vos $.*Vous avez +_ £.#La Durée de Feu Intérieur est équivalente à votre $.*act:214*act:215/\n*+@ £|/\n|*Les Dégâts de Reaper sont ajoutés aux Dégâts de Feu Intérieur au lieu d'être ajoutés aux Dégâts Physiques.//Jongler avec le Feu.",
    "FR_NAME": "$ de Force Intérieure Concentrée",
    "FR_DESC": "*+@ £*|*Vous avez +_ £.#Ces Dégâts de Feu Intérieur sont équivalents à @ de vos $.*Vous avez +_ £.#La Durée de Feu Intérieur est équivalente à votre $.*act:214*act:215/\n*+@ £|/\n|*Les Dégâts de Reaper sont ajoutés aux Dégâts de Feu Intérieur au lieu d'être ajoutés aux Dégâts Physiques.",
    "FR_LORE": "Jongler avec le Feu.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 23,
    "SPRITE": 23,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "2|1|\n1",
    "VALUE_TYPE": "%|%|\n%",
    "VALUE_LEVEL": "rl|rl|\nrl",
    "VALUE_STAT": "inner_fire_damage_percent|inner_fire_damage_add|inner_fire_duration_add|\ninner_fire_max_number_global_mult",
    "VALUE_REAL": "synergy:reaper_damage:1*rl|synergy:inner_fire_max_number:100",
    "BASE_DMG_MIN": 17,
    "BASE_DMG_MAX": 21,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1392 - 1649"
  },
  {
    "REF": 24,
    "EN_NAME": "$ of the Untouchable One",
    "EN_DESC": "*+@ £*+@ £*+@ £|*You have +_ £.#Evasion is equal to your $.*You have +@ Chance to Evade successfully.*Whenever you Evade successfully, Untouchable's Cooldown is reset.*act:216/\n|*You have +_ £.#Evasion is equal to your $.*Successfull Evasions grant you a Dash stack, allowing you dash an additional time during Untouchable, up to @ times./\n|*When Untouchable ends, incoming Damage is increased by @ for the next @ seconds.",
    "EN_LORE": "For those who want to play Tag in a very serious way.",
    "EN": "$ of the Untouchable One//*+@ £*+@ £*+@ £|*You have +_ £.#Evasion is equal to your $.*You have +@ Chance to Evade successfully.*Whenever you Evade successfully, Untouchable's Cooldown is reset.*act:216/\n|*You have +_ £.#Evasion is equal to your $.*Successfull Evasions grant you a Dash stack, allowing you dash an additional time during Untouchable, up to @ times./\n|*When Untouchable ends, incoming Damage is increased by @ for the next @ seconds.//For those who want to play Tag in a very serious way.",
    "FR": "$ de l'Intouchable//*+@ £*+@ £*+@ £|*Vous avez +_ £.#L'Evasion est équivalente à vos $.*Vous avez +@ de Chance d'Esquiver.*Lorsque vous Esquivez, le Temps de Recharge d'Intouchable est réinitialisé.*act:216/\n|*Vous avez +_ £.#L'Evasion est équivalente à vos $.*Les Esquives réussies vous confèrent un cumul d'Élan, vous permettant de vous élancer une fois supplémentaire durant Intouchable, jusqu'à @ fois./\n|*Quand Intouchable se termine, les Dégâts reçus sont augmentés de @ pour les @ prochaines secondes.//Pour ceux qui veulent jouer à Chat de manière très sérieuse.",
    "FR_NAME": "$ de l'Intouchable",
    "FR_DESC": "*+@ £*+@ £*+@ £|*Vous avez +_ £.#L'Evasion est équivalente à vos $.*Vous avez +@ de Chance d'Esquiver.*Lorsque vous Esquivez, le Temps de Recharge d'Intouchable est réinitialisé.*act:216/\n|*Vous avez +_ £.#L'Evasion est équivalente à vos $.*Les Esquives réussies vous confèrent un cumul d'Élan, vous permettant de vous élancer une fois supplémentaire durant Intouchable, jusqu'à @ fois./\n|*Quand Intouchable se termine, les Dégâts reçus sont augmentés de @ pour les @ prochaines secondes.",
    "FR_LORE": "Pour ceux qui veulent jouer à Chat de manière très sérieuse.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 24,
    "SPRITE": 24,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3|0.5|0.5|15|\n5|\n100|3",
    "VALUE_TYPE": "|%|%|%|\n|\n%|",
    "VALUE_LEVEL": "rl|rl|rl||\n||\n|",
    "VALUE_STAT": "dodge_add|dodge_percent|dodge_global_mult|dodge_add|\ndodge_add",
    "VALUE_REAL": "synergy:min_basic_damage:100|\nsynergy:elemental_damage:100",
    "BASE_DMG_MIN": 5,
    "BASE_DMG_MAX": 12,
    "MIN_DMG_LVL": 7,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1272 - 2133"
  },
  {
    "REF": 25,
    "EN_NAME": "$ of the Lambs",
    "EN_DESC": "|*Your Primary Skill has a @ £ to Silence enemies hit for the next @ seconds.*Whenever you Silence an enemy during its cast, you also deal _ £ to the enemy hit.*Whenever you kill a Silenced enemy, you have a @ £ to trigger an Explosion, silencing enemies within a ~ yard radius for the next @ seconds.*act:233/\n|*Non-Spellcasters Enemies that should be Silenced are Blinded instead.*Muting Area also deal its damage to Blinded enemies./\n|*Blinded enemies only deal Critical Strikes.",
    "EN_LORE": "Enthrall me with your acumen.",
    "EN": "$ of the Lambs//|*Your Primary Skill has a @ £ to Silence enemies hit for the next @ seconds.*Whenever you Silence an enemy during its cast, you also deal _ £ to the enemy hit.*Whenever you kill a Silenced enemy, you have a @ £ to trigger an Explosion, silencing enemies within a ~ yard radius for the next @ seconds.*act:233/\n|*Non-Spellcasters Enemies that should be Silenced are Blinded instead.*Muting Area also deal its damage to Blinded enemies./\n|*Blinded enemies only deal Critical Strikes.//Enthrall me with your acumen.",
    "FR": "$ des Agneaux//|*Votre Compétence Primaire a @ de £ de réduire au Silence les ennemis touchés pour les @ prochaines secondes.*Chaque fois que vous réduisez au Silence un ennemi alors qu'il est en train de lancer une Compétence, vous lui infligez _ de £.*Chaque fois que vous tuez un ennemi réduit au Silence, vous avez @ de £ de déclencher une Explosion, réduisant au Silence les ennemis dans un rayon d'environ ~ mètres pour les @ prochaines secondes.*act:233/\n|*Les ennemis qui ne sont pas des Lanceurs de Sort qui devraient être réduits au Silence sont Aveuglés à la place.*La Zone de Silence inflige également ses dégâts aux ennemis aveuglés./\n|*Les ennemis Aveuglés infligent uniquement des Coups Critiques.//Enthousiasmez-moi avec votre perspicacité.",
    "FR_NAME": "$ des Agneaux",
    "FR_DESC": "|*Votre Compétence Primaire a @ de £ de réduire au Silence les ennemis touchés pour les @ prochaines secondes.*Chaque fois que vous réduisez au Silence un ennemi alors qu'il est en train de lancer une Compétence, vous lui infligez _ de £.*Chaque fois que vous tuez un ennemi réduit au Silence, vous avez @ de £ de déclencher une Explosion, réduisant au Silence les ennemis dans un rayon d'environ ~ mètres pour les @ prochaines secondes.*act:233/\n|*Les ennemis qui ne sont pas des Lanceurs de Sort qui devraient être réduits au Silence sont Aveuglés à la place.*La Zone de Silence inflige également ses dégâts aux ennemis aveuglés./\n|*Les ennemis Aveuglés infligent uniquement des Coups Critiques.",
    "FR_LORE": "Enthousiasmez-moi avec votre perspicacité.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 25,
    "SPRITE": 35,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.25|3|0.5|3",
    "VALUE_TYPE": "%||%|",
    "VALUE_LEVEL": "rl||rl|",
    "VALUE_STAT": "chance|physical_damage|chance",
    "VALUE_REAL": "damage:physical_damage:150",
    "BASE_DMG_MIN": 24,
    "BASE_DMG_MAX": 32,
    "MIN_DMG_LVL": 10,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1825 - 2179"
  },
  {
    "REF": 26,
    "EN_NAME": "$ of the Unkillable Berzerker",
    "EN_DESC": "|*While below @ Max Life, incoming damage cannot exceed @ of your Max Life.*While below @ Max Life, you gain a +_ % £ and £.#This Multiplier is equal to your Percent of Missing Life.*Whenever you deal damage to an enemy, you have a @ £ to deal _ Additional Damage.#Additional Damage is equal to your Missing Life and increased by @. /\n|*Every @ seconds, you trigger an Explosion dealing possible Additional Damage from Unkillable Berzerker as Skill Damage to enemies within a ~ yard radius.#This Explosion deals @ £ for each enemy within its range./\n|*While above @ Max Life, Life Regeneration now consumes Life instead of Regenerating it.",
    "EN_LORE": "Hey, someone's reading the flavor text!#Hello you! You're breathtaking!",
    "EN": "$ of the Unkillable Berzerker//|*While below @ Max Life, incoming damage cannot exceed @ of your Max Life.*While below @ Max Life, you gain a +_ % £ and £.#This Multiplier is equal to your Percent of Missing Life.*Whenever you deal damage to an enemy, you have a @ £ to deal _ Additional Damage.#Additional Damage is equal to your Missing Life and increased by @. /\n|*Every @ seconds, you trigger an Explosion dealing possible Additional Damage from Unkillable Berzerker as Skill Damage to enemies within a ~ yard radius.#This Explosion deals @ £ for each enemy within its range./\n|*While above @ Max Life, Life Regeneration now consumes Life instead of Regenerating it.//Hey, someone's reading the flavor text!#Hello you! You're breathtaking!",
    "FR": "$ du Berzerker Infaillible//|*Tant que votre Vie est inférieure à @, les dégâts reçus ne peuvent pas dépasser @ de votre Vie Max.*Tant que votre Vie est inférieure à @, vous bénéficiez d'un multiplicateur de +_ % de £ et de £.#Ce multiplicateur est équivalent à votre Pourcentage de Vie Manquante.*Chaque fois que vous infligez des dégâts à un ennemi, vous avez @ de £ d'infliger _ de Dégâts Additionnels. Ces Dégâts Additionnels sont équivalents à votre Vie Manquante et augmentés de @. /\n|*Toutes les @ secondes, vous déclenchez une Explosion infligeant les Dégâts Additionnels possibles de Berzerker Infaillible en tant que Dégâts Physiques aux ennemis dans un rayon d'environ ~ mètres.#Cette Explosion inflige @ £ pour chaque ennemi à portée./\n|*Tant que votre Vie est supérieure à @, la Régénération de Vie consume désormais la Vie au lieu de la régénérer.//Hé, quelqu'un lit le petit texte d'ambiance !#Bonjour, vous ! Vous êtes à couper le souffle !",
    "FR_NAME": "$ du Berzerker Infaillible",
    "FR_DESC": "|*Tant que votre Vie est inférieure à @, les dégâts reçus ne peuvent pas dépasser @ de votre Vie Max.*Tant que votre Vie est inférieure à @, vous bénéficiez d'un multiplicateur de +_ % de £ et de £.#Ce multiplicateur est équivalent à votre Pourcentage de Vie Manquante.*Chaque fois que vous infligez des dégâts à un ennemi, vous avez @ de £ d'infliger _ de Dégâts Additionnels. Ces Dégâts Additionnels sont équivalents à votre Vie Manquante et augmentés de @. /\n|*Toutes les @ secondes, vous déclenchez une Explosion infligeant les Dégâts Additionnels possibles de Berzerker Infaillible en tant que Dégâts Physiques aux ennemis dans un rayon d'environ ~ mètres.#Cette Explosion inflige @ £ pour chaque ennemi à portée./\n|*Tant que votre Vie est supérieure à @, la Régénération de Vie consume désormais la Vie au lieu de la régénérer.",
    "FR_LORE": "Hé, quelqu'un lit le petit texte d'ambiance !#Bonjour, vous ! Vous êtes à couper le souffle !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 26,
    "SPRITE": 76,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "30|5|30|0.25|2|\n3|5|\n25",
    "VALUE_TYPE": "%|%|%|%|%|\n|%|\n%",
    "VALUE_LEVEL": "|||rl|rl|\n||\n",
    "VALUE_STAT": "res_phy_global_mult|res_mag_global_mult|chance|\nincreased_damage",
    "VALUE_REAL": "synergy:percent_missing_health:100|synergy:missing_health_unkillable:100|",
    "BASE_DMG_MIN": 17,
    "BASE_DMG_MAX": 25,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1137 - 1325"
  },
  {
    "REF": 27,
    "EN_NAME": "$ of Alpha and Omega",
    "EN_DESC": "*+@ £*+@ £|*@ of Reaper Damage is added to Elemental Damage instead of Skill Damage.*Two Black Holes are constantly rotating around you, dealing _ £ and _ £ to enemies hit.#The closer you are to @ Mana, the stronger they get. They deal up to @ £ and get up to @ larger and faster.*If you remain around @ Mana for @ seconds, you trigger Alpha, increasing the number of Black Holes to @./\n|*If you remain around @ Life for @ seconds, you trigger Omega: Black Holes create smaller copies decreasing in size while moving in a line away from you.*Black Holes deal _% £.#Increased Damage is equal to @ of the Mastery Level of your Secondary Skill./\n|*Your Secondary Skill is Locked.",
    "EN_LORE": "The last Reaper you'll ever use.",
    "EN": "$ of Alpha and Omega//*+@ £*+@ £|*@ of Reaper Damage is added to Elemental Damage instead of Skill Damage.*Two Black Holes are constantly rotating around you, dealing _ £ and _ £ to enemies hit.#The closer you are to @ Mana, the stronger they get. They deal up to @ £ and get up to @ larger and faster.*If you remain around @ Mana for @ seconds, you trigger Alpha, increasing the number of Black Holes to @./\n|*If you remain around @ Life for @ seconds, you trigger Omega: Black Holes create smaller copies decreasing in size while moving in a line away from you.*Black Holes deal _% £.#Increased Damage is equal to @ of the Mastery Level of your Secondary Skill./\n|*Your Secondary Skill is Locked.//The last Reaper you'll ever use.",
    "FR": "$ de l'Alpha et de l'Oméga//*+@ £*+@ £|*@ des Dégâts de Reaper sont ajoutés aux Dégâts Élémentaires au lieu d'être ajoutés aux Dégats Physiques.*Deux Trous Noirs orbitent continuellement autour de vous, infligeant _ £ et _ £ aux ennemis touchés.#Plus vous êtes proches de @ de Mana, plus ils deviennent puissants. Ils infligent jusqu'à @ £ et deviennent également jusqu'à @ plus larges et plus rapides.*Si vous vous maintenez autour de @ de Mana durant @ secondes, vous déclenchez Alpha, augmentant le nombre de Trous Noirs à @./\n|*Si vous vous maintenez autour de @ de Vie durant @ secondes, vous déclenchez Omega : Les Trous Noirs créent des copies qui se déplacent en ligne.*Les Trous Noirs infligent _% £.#Ces Dégâts Supplémentaires sont équivalents à @ du Niveau de Maîtrise de votre Compétence Secondaire./\n|*Votre Compétence Secondaire est Verrouillée.//Le dernier Reaper que vous utiliserez.",
    "FR_NAME": "$ de l'Alpha et de l'Oméga",
    "FR_DESC": "*+@ £*+@ £|*@ des Dégâts de Reaper sont ajoutés aux Dégâts Élémentaires au lieu d'être ajoutés aux Dégats Physiques.*Deux Trous Noirs orbitent continuellement autour de vous, infligeant _ £ et _ £ aux ennemis touchés.#Plus vous êtes proches de @ de Mana, plus ils deviennent puissants. Ils infligent jusqu'à @ £ et deviennent également jusqu'à @ plus larges et plus rapides.*Si vous vous maintenez autour de @ de Mana durant @ secondes, vous déclenchez Alpha, augmentant le nombre de Trous Noirs à @./\n|*Si vous vous maintenez autour de @ de Vie durant @ secondes, vous déclenchez Omega : Les Trous Noirs créent des copies qui se déplacent en ligne.*Les Trous Noirs infligent _% £.#Ces Dégâts Supplémentaires sont équivalents à @ du Niveau de Maîtrise de votre Compétence Secondaire./\n|*Votre Compétence Secondaire est Verrouillée.",
    "FR_LORE": "Le dernier Reaper que vous utiliserez.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 36,
    "SPRITE": 27,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|0.5|50|50|100|1|50|2|4|\n50|2|300",
    "VALUE_TYPE": "%|%|%|%|%|%|%|||\n%||%",
    "VALUE_LEVEL": "rl|rl||||rl||||\n||",
    "VALUE_STAT": "basic_damage_global_mult|elemental_damage_global_mult|physical_damage|elemental_damage|increased_damage|\nincreased_damage",
    "VALUE_REAL": "damage:physical_damage:50|damage:elemental_damage:50|\nsynergy:mastery_secondary:300",
    "BASE_DMG_MIN": 15,
    "BASE_DMG_MAX": 19,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 5,
    "DMG_MULTIPLIER": 5,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1430 - 2207"
  },
  {
    "REF": 28,
    "EN_NAME": "$ of Auracle-Maker",
    "EN_DESC": "|*You have +_% £.#Aura Increased Effect is equal to @ per Available Aura and increased by @ for every Active Aura in your Ancestral Skill Slot.*act:234/\n|*Auras with a range have +_ % Increased Size.#Increased Size is equal to @ of your Aura Increased Effect./\n|*Your Area Increased Size is reduced to 0.",
    "EN_LORE": "\"This Slorm Reaper belonged to Skywim, who became a founding member of the Elders Slormies Council. He loved Auras, he finds them very practical.\"",
    "EN": "$ of Auracle-Maker//|*You have +_% £.#Aura Increased Effect is equal to @ per Available Aura and increased by @ for every Active Aura in your Ancestral Skill Slot.*act:234/\n|*Auras with a range have +_ % Increased Size.#Increased Size is equal to @ of your Aura Increased Effect./\n|*Your Area Increased Size is reduced to 0.//\"This Slorm Reaper belonged to Skywim, who became a founding member of the Elders Slormies Council. He loved Auras, he finds them very practical.\"",
    "FR": "$ du Faiseur d'Auracle//|*Vous avez +_% £.#L'Effet d'Aura Amélioré est égal à @ par Aura Équipée et augmenté de @ pour chaque Aura Active dans vos Emplacements de Compétences Ancestrales.*act:234/\n|*Les Auras ayant une Taille augmentée de +_%.#Cette Taille est équivalente à @ de votre Effet d'Aura Amélioré./\n|*Votre Taille de Zone d'Effet est réduite à 0.//\"Ce Slorm Reaper appartenait à Skywim, qui est devenu un membre fondateur du Conseil des Elders Slorms. Il adorait les Auras, qu'il trouvait très pratiques.\"",
    "FR_NAME": "$ du Faiseur d'Auracle",
    "FR_DESC": "|*Vous avez +_% £.#L'Effet d'Aura Amélioré est égal à @ par Aura Équipée et augmenté de @ pour chaque Aura Active dans vos Emplacements de Compétences Ancestrales.*act:234/\n|*Les Auras ayant une Taille augmentée de +_%.#Cette Taille est équivalente à @ de votre Effet d'Aura Amélioré./\n|*Votre Taille de Zone d'Effet est réduite à 0.",
    "FR_LORE": "\"Ce Slorm Reaper appartenait à Skywim, qui est devenu un membre fondateur du Conseil des Elders Slorms. Il adorait les Auras, qu'il trouvait très pratiques.\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 28,
    "SPRITE": 30,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.1|25|\n50|\n-100",
    "VALUE_TYPE": "%|%|\n|",
    "VALUE_LEVEL": "rl||\n|",
    "VALUE_STAT": "aura_increased_effect_percent",
    "VALUE_REAL": "synergy:aura_equipped_per_aura_active:0.1*rl|\nsynergy:aura_increased_effect:0.5",
    "BASE_DMG_MIN": 31,
    "BASE_DMG_MAX": 42,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 1,
    "MAX_LVL": 44,
    "EVOLVE_IN": 29,
    "100": "1220 - 2458"
  },
  {
    "REF": 29,
    "EN_NAME": "$ of the Scheming Auracle-Maker",
    "EN_DESC": "|*Toggling On an Aura deals _ £ to enemies within a ~ yard radius.#Toggling Off an Aura Stuns enemies within a ~ yard radius for @ seconds.*The radius is increased by Aura Increased Effect and each effect can only occur every @ seconds.",
    "EN_LORE": "\"He was later known as: The Elder Slorm, Skywim.\"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "EN": "$ of the Scheming Auracle-Maker//|*Toggling On an Aura deals _ £ to enemies within a ~ yard radius.#Toggling Off an Aura Stuns enemies within a ~ yard radius for @ seconds.*The radius is increased by Aura Increased Effect and each effect can only occur every @ seconds.//\"He was later known as: The Elder Slorm, Skywim.\"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "FR": "$ du Faiseur d'Auracle Intrigant//|*Activer une Aura inflige _ de £ aux ennemis dans un rayon de ~ mètres.#Désactiver une Aura Étourdit les ennemis dans un rayon de ~ mètres pour les @ prochaines secondes.*Ce rayon est amélioré par votre Effet d'Aura Amélioré et chaque effet ne peut se produire qu'une fois toutes les @ secondes.//\"Il fut plus tard connu sous le nom de : The Elder Slorm, Skywim. \"#- Champions of Arah, Heroes or Impostors, par Adam Nostrus",
    "FR_NAME": "$ du Faiseur d'Auracle Intrigant",
    "FR_DESC": "|*Activer une Aura inflige _ de £ aux ennemis dans un rayon de ~ mètres.#Désactiver une Aura Étourdit les ennemis dans un rayon de ~ mètres pour les @ prochaines secondes.*Ce rayon est amélioré par votre Effet d'Aura Amélioré et chaque effet ne peut se produire qu'une fois toutes les @ secondes.",
    "FR_LORE": "\"Il fut plus tard connu sous le nom de : The Elder Slorm, Skywim. \"#- Champions of Arah, Heroes or Impostors, par Adam Nostrus",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 29,
    "SPRITE": 31,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "2|5|3",
    "VALUE_TYPE": "||",
    "VALUE_LEVEL": "||rl",
    "VALUE_STAT": "elemental_damage",
    "VALUE_REAL": "damage:elemental_damage:2*rl",
    "BASE_DMG_MIN": 31,
    "BASE_DMG_MAX": 42,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1453 - 2700"
  },
  {
    "REF": 30,
    "EN_NAME": "$ of the Unholy Sepulcher",
    "EN_DESC": "|*Whenever you kill an enemy, you now have a @ £ to Entomb them.#The Chance to Entomb is increased by @.*act:235/\n|*Whenever you trigger the Explosion of a Tomb, it also triggers the Explosion of another random Tomb within its range./\n|*Whenever you walk on a Tomb it triggers Tomb Explosion and deals _ £ to you.",
    "EN_LORE": "What is dead may die again!",
    "EN": "$ of the Unholy Sepulcher//|*Whenever you kill an enemy, you now have a @ £ to Entomb them.#The Chance to Entomb is increased by @.*act:235/\n|*Whenever you trigger the Explosion of a Tomb, it also triggers the Explosion of another random Tomb within its range./\n|*Whenever you walk on a Tomb it triggers Tomb Explosion and deals _ £ to you.//What is dead may die again!",
    "FR": "$ de Sépulture Impie//|*Chaque fois que vous tuez un ennemi, vous avez désormais une chance de @ £ de l'Enterrer.#La Chance d'Enterrer est augmentée de @.*act:235/\n|*Chaque fois que vous déclenchez l'Explosion d'une Tombe, elle déclenche également l'Explosion d'une autre Tombe aléatoire dans sa portée./\n|*Chaque fois que vous marchez sur une Tombe, elle déclenche une Explosion de Tombe et vous inflige _ de £.//Ce qui est mort peut encore mourir !",
    "FR_NAME": "$ de Sépulture Impie",
    "FR_DESC": "|*Chaque fois que vous tuez un ennemi, vous avez désormais une chance de @ £ de l'Enterrer.#La Chance d'Enterrer est augmentée de @.*act:235/\n|*Chaque fois que vous déclenchez l'Explosion d'une Tombe, elle déclenche également l'Explosion d'une autre Tombe aléatoire dans sa portée./\n|*Chaque fois que vous marchez sur une Tombe, elle déclenche une Explosion de Tombe et vous inflige _ de £.",
    "FR_LORE": "Ce qui est mort peut encore mourir !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 31,
    "SPRITE": 63,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "15|1|",
    "VALUE_TYPE": "%|%",
    "VALUE_LEVEL": "|rl",
    "VALUE_STAT": "chance|physical_damage",
    "VALUE_REAL": "synergy:remain_damage:1",
    "BASE_DMG_MIN": 17,
    "BASE_DMG_MAX": 34,
    "MIN_DMG_LVL": 11,
    "MAX_DMG_LVL": 14,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 24,
    "EVOLVE_IN": 31,
    "100": "1977 - 2523"
  },
  {
    "REF": 31,
    "EN_NAME": "$ of the Desecrated Unholy Sepulcher",
    "EN_DESC": "|*You have a +@ Increased Chance to Entomb enemies hit by Tomb Profanation.*act:236",
    "EN_LORE": "What Remain shall not Remain!",
    "EN": "$ of the Desecrated Unholy Sepulcher//|*You have a +@ Increased Chance to Entomb enemies hit by Tomb Profanation.*act:236//What Remain shall not Remain!",
    "FR": "$ de Sépulture Impie Profanée//|*Vous avez +@ de Chance d'Enterrer les ennemis touchés par Profanation de Tombe.*act:236//Ce qui reste ne restera pas !",
    "FR_NAME": "$ de Sépulture Impie Profanée",
    "FR_DESC": "|*Vous avez +@ de Chance d'Enterrer les ennemis touchés par Profanation de Tombe.*act:236",
    "FR_LORE": "Ce qui reste ne restera pas !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 32,
    "SPRITE": 64,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "rl",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 17,
    "BASE_DMG_MAX": 34,
    "MIN_DMG_LVL": 12,
    "MAX_DMG_LVL": 15,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2141 - 2692"
  },
  {
    "REF": 32,
    "EN_NAME": "$ of Elemental Overload",
    "EN_DESC": "|*Imbued Skills deal @ £.#You have +@ Resistance to the Element of your Imbued Element.*Whenever you deal Ancestral Strike Damage, you gain an Elemental Fury stack, up to @.#Imbued Skills deal @ £ for each stack of Elemental Fury.*Every @ seconds, you lose your Elemental Fury stacks and gain Elemental Overload for another random Element.*While Elemental Overload is active, the First Imbue Skill of the chosen Element becomes Equipped and at Max Rank./\n|*You gain Elemental Prolongation.*act:237/\n|*Your Ancestral Strike Damage becomes equal to your Critical Strike Damage.",
    "EN_LORE": "\"The Elemental Overload Reaper is a formidable weapon...\"",
    "EN": "$ of Elemental Overload//|*Imbued Skills deal @ £.#You have +@ Resistance to the Element of your Imbued Element.*Whenever you deal Ancestral Strike Damage, you gain an Elemental Fury stack, up to @.#Imbued Skills deal @ £ for each stack of Elemental Fury.*Every @ seconds, you lose your Elemental Fury stacks and gain Elemental Overload for another random Element.*While Elemental Overload is active, the First Imbue Skill of the chosen Element becomes Equipped and at Max Rank./\n|*You gain Elemental Prolongation.*act:237/\n|*Your Ancestral Strike Damage becomes equal to your Critical Strike Damage.//\"The Elemental Overload Reaper is a formidable weapon...\"",
    "FR": "$ de Charge Élémentaire//|*Les Compétences Imprégnées infligent @ de £.#Vous avez +@ de Résistance à l'Élément de votre Élément Imprégné.*Chaque fois que vous infligez des Dégâts Ancestraux, vous gagnez un cumul de Furie Élémentaire, jusqu'à @.#Les Compétences Imprégnées infligent @ de £ pour chaque cumul de Furie Élémentaire.*Toutes les @ secondes, vous perdez vos cumuls de Furie Élémentaire et obtenez Surcharge Élémentaire pour un autre Élément aléatoire.*Pendant Surcharge Élémentaire, la Première Compétence Imprégnée de l'Élément choisi est automatiquement Équipée et au Rang Max./\n|*Vous obtenez Prolongement Élémentaire.*act:237/\n|*Vos Dégâts de Coup Ancestral deviennent égaux à vos Dégâts de Coup Critique.//\"Le Reaper de Charge Élémentaire est une arme redoutable...\"",
    "FR_NAME": "$ de Charge Élémentaire",
    "FR_DESC": "|*Les Compétences Imprégnées infligent @ de £.#Vous avez +@ de Résistance à l'Élément de votre Élément Imprégné.*Chaque fois que vous infligez des Dégâts Ancestraux, vous gagnez un cumul de Furie Élémentaire, jusqu'à @.#Les Compétences Imprégnées infligent @ de £ pour chaque cumul de Furie Élémentaire.*Toutes les @ secondes, vous perdez vos cumuls de Furie Élémentaire et obtenez Surcharge Élémentaire pour un autre Élément aléatoire.*Pendant Surcharge Élémentaire, la Première Compétence Imprégnée de l'Élément choisi est automatiquement Équipée et au Rang Max./\n|*Vous obtenez Prolongement Élémentaire.*act:237/\n|*Vos Dégâts de Coup Ancestral deviennent égaux à vos Dégâts de Coup Critique.",
    "FR_LORE": "\"Le Reaper de Charge Élémentaire est une arme redoutable...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 33,
    "SPRITE": 93,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|1|5|9",
    "VALUE_TYPE": "%|%||%|",
    "VALUE_LEVEL": "rl|rl|rl||",
    "VALUE_STAT": "increased_damage|increased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 9,
    "BASE_DMG_MAX": 27,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 16,
    "EVOLVE_IN": 33,
    "100": "948 - 1496"
  },
  {
    "REF": 33,
    "EN_NAME": "$ of Charged Elemental Overload",
    "EN_DESC": "|*While Elemental Overload is active, the Second Imbue Skill of the chosen Element becomes Equipped and at Max Rank.",
    "EN_LORE": "\"... capable of manipulating the elements at will.\"",
    "EN": "$ of Charged Elemental Overload//|*While Elemental Overload is active, the Second Imbue Skill of the chosen Element becomes Equipped and at Max Rank.//\"... capable of manipulating the elements at will.\"",
    "FR": "$ de Surcharge Élémentaire Chargée//|*Lorsque Surcharge élémentaire est active, la seconde Compétence Imprégnée de l'Élément choisi est automatiquement Équipée et au Rang Max.//\"... capable de manipuler les éléments à volonté.\"",
    "FR_NAME": "$ de Surcharge Élémentaire Chargée",
    "FR_DESC": "|*Lorsque Surcharge élémentaire est active, la seconde Compétence Imprégnée de l'Élément choisi est automatiquement Équipée et au Rang Max.",
    "FR_LORE": "\"... capable de manipuler les éléments à volonté.\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 34,
    "SPRITE": 94,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 9,
    "BASE_DMG_MAX": 27,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 28,
    "EVOLVE_IN": 34,
    "100": "948 - 1496"
  },
  {
    "REF": 34,
    "EN_NAME": "$ of Supercharged Elemental Overload",
    "EN_DESC": "|*While Elemental Overload is active, the Third Imbue Skill of the chosen Element becomes Equipped and at Max Rank.",
    "EN_LORE": "\"Each strike is a display of mastery, breaking the resistance of enemies with a deluge of elemental energy.\"#- Slorm Reapers of Legend, by Pr. Geranium",
    "EN": "$ of Supercharged Elemental Overload//|*While Elemental Overload is active, the Third Imbue Skill of the chosen Element becomes Equipped and at Max Rank.//\"Each strike is a display of mastery, breaking the resistance of enemies with a deluge of elemental energy.\"#- Slorm Reapers of Legend, by Pr. Geranium",
    "FR": "$ Chargé de Surcharge Élémentaire Chargée/$ Chargée de Surcharge Élémentaire Chargée//|*Lorsque Surcharge élémentaire est active, la troisième Compétence Imprégnée de l'Élément choisi est automatiquement Équipée et au Rang Max.//\"Chaque coup est une démonstration de maîtrise, brisant la résistance des ennemis avec un déluge d'énergie élémentaire.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "$ Chargé de Surcharge Élémentaire Chargée/$ Chargée de Surcharge Élémentaire Chargée",
    "FR_DESC": "|*Lorsque Surcharge élémentaire est active, la troisième Compétence Imprégnée de l'Élément choisi est automatiquement Équipée et au Rang Max.",
    "FR_LORE": "\"Chaque coup est une démonstration de maîtrise, brisant la résistance des ennemis avec un déluge d'énergie élémentaire.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 35,
    "SPRITE": 95,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 9,
    "BASE_DMG_MAX": 27,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "948 - 1496"
  },
  {
    "REF": 35,
    "EN_NAME": "$ of Upheaval",
    "EN_DESC": "|*Against Stunned enemies:#You deal @ £.*Against enemies you hit from the back:#You deal @ £.#You have +@ £.#You have +@ £.*After @ seconds while in combat without casting Skills, you gain Bludgeon for the next @ seconds, granting you a @ £ to Stun enemies hit for the next @ seconds.*act:238/\n|*The first time you deal damage to a Stunned enemy, you apply Resonating Force, dealing @ of that damage as Skill Damage every second until he dies./\n|*Melee attacks from enemies have a @ £ to Stun you for the next @ second.",
    "EN_LORE": "It is written below: \"Property of Jerelzor, Contact me by Slorm Message Service if found\".",
    "EN": "$ of Upheaval//|*Against Stunned enemies:#You deal @ £.*Against enemies you hit from the back:#You deal @ £.#You have +@ £.#You have +@ £.*After @ seconds while in combat without casting Skills, you gain Bludgeon for the next @ seconds, granting you a @ £ to Stun enemies hit for the next @ seconds.*act:238/\n|*The first time you deal damage to a Stunned enemy, you apply Resonating Force, dealing @ of that damage as Skill Damage every second until he dies./\n|*Melee attacks from enemies have a @ £ to Stun you for the next @ second.//It is written below: \"Property of Jerelzor, Contact me by Slorm Message Service if found\".",
    "FR": "$ de Bouleversement//|*Contre les ennemis Étourdis :#Vous infligez @ £.*Contre les ennemis que vous frappez dans le dos :#Vous infligez @ de £.#Vous avez +@ £.# Vous avez +@ £.*Après @ secondes en combat sans lancer de Compétences, vous gagnez Gourdin pour les @ prochaines secondes, vous conférant @ de £ d'Étourdir les ennemis touchés pour les @ prochaines secondes.*act:238/\n|*La première fois que vous infligez des dégâts à un ennemi Étourdi, vous appliquez Force Résonante, qui inflige @ de ces dégâts en tant que Dégâts Physiques toutes les secondes jusqu'à ce qu'il meure./\n|*Les attaques de Mêlée des ennemis ont  @ de £ de vous Étourdir pour les @ prochaines secondes.//Il est écrit en dessous: \"Propriété de Jerelzor. Me contacter par Service de Messagerie du Slorm si retrouvé\".",
    "FR_NAME": "$ de Bouleversement",
    "FR_DESC": "|*Contre les ennemis Étourdis :#Vous infligez @ £.*Contre les ennemis que vous frappez dans le dos :#Vous infligez @ de £.#Vous avez +@ £.# Vous avez +@ £.*Après @ secondes en combat sans lancer de Compétences, vous gagnez Gourdin pour les @ prochaines secondes, vous conférant @ de £ d'Étourdir les ennemis touchés pour les @ prochaines secondes.*act:238/\n|*La première fois que vous infligez des dégâts à un ennemi Étourdi, vous appliquez Force Résonante, qui inflige @ de ces dégâts en tant que Dégâts Physiques toutes les secondes jusqu'à ce qu'il meure./\n|*Les attaques de Mêlée des ennemis ont  @ de £ de vous Étourdir pour les @ prochaines secondes.",
    "FR_LORE": "Il est écrit en dessous: \"Propriété de Jerelzor. Me contacter par Service de Messagerie du Slorm si retrouvé\".",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 30,
    "SPRITE": 45,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|15|15|2|7|15|4|\n30|\n15|0.5",
    "VALUE_TYPE": "%|%|%|%|||%||\n%|\n%|",
    "VALUE_LEVEL": "rl|rl|||||||\n|\n|",
    "VALUE_STAT": "increased_damage|increased_damage|crit_chance_percent|brut_chance_percent|chance|\nchance",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 20,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1603 - 2837"
  },
  {
    "REF": 36,
    "EN_NAME": "$ of the Bloodthirsty Beast",
    "EN_DESC": "*+@ £|*Whenever you kill an enemy, you gain @ Bloodthirst stacks, up to @.#Whenever you're dealt damage while Blood Freenzy is not active, you gain a Bloodthirst stack.*You have +_ % £.#Life on Kill Multiplier is equal to the number of Bloodthirst stacks.*Whenever you should be defeated, your Life is set to @ instead, you gain up to @ Bloodthirst stacks and immediately trigger Blood Frenzy.#This effect has a @ seconds Cooldown.*act:239/\n|*While Blood Frenzy is active, you also gain +_ Reaper Damage.#Reaper Damage is equal to your Life on Kill./\n|*You can now only restore Life with Life on Kill.",
    "EN_LORE": "\"A Reaper who becomes stronger in the face of danger, can reach pinnacles of power if stimulated properly.\" - Legendary Reapers, by Pr.Geranium.",
    "EN": "$ of the Bloodthirsty Beast//*+@ £|*Whenever you kill an enemy, you gain @ Bloodthirst stacks, up to @.#Whenever you're dealt damage while Blood Freenzy is not active, you gain a Bloodthirst stack.*You have +_ % £.#Life on Kill Multiplier is equal to the number of Bloodthirst stacks.*Whenever you should be defeated, your Life is set to @ instead, you gain up to @ Bloodthirst stacks and immediately trigger Blood Frenzy.#This effect has a @ seconds Cooldown.*act:239/\n|*While Blood Frenzy is active, you also gain +_ Reaper Damage.#Reaper Damage is equal to your Life on Kill./\n|*You can now only restore Life with Life on Kill.//\"A Reaper who becomes stronger in the face of danger, can reach pinnacles of power if stimulated properly.\" - Legendary Reapers, by Pr.Geranium.",
    "FR": "$ de la Bête Sanguinaire//*+@ £|*Chaque fois que vous tuez un ennemi, vous gagnez @ cumuls de Soif de Sang, jusqu'à @.#Chaque fois que vous subissez des dégâts alors que Fureur Sanguinaire n'est pas active, vous gagnez un cumul de Soif de Sang.*Vous avez +_% de £.#Ce Multiplicateur de Vie par Victime est équivalent à votre nombre de cumuls de Soif de Sang.*Si vous êtes vaincu, votre Vie devient @, vous gagnez jusqu'à @ cumuls de Soif de Sang et déclenchez immédiatement Fureur Sanguinaire.#Cet effet a un Temps de Recharge de @ secondes.*act:239/\n|*Lorsque Fureur Sanguinaire est active, vous gagnez également +_ Dégâts de Reaper.#Ces Dégâts de Reaper sont équivalents à votre Vie par Victime./\n|*Vous ne pouvez plus restaurer la vie qu'avec Vie par Victime.//\"Un Reaper qui devient plus fort face au danger, peut atteindre des sommets si il est stimulé\". - Reapers de Légende, par le Pr.Géranium.",
    "FR_NAME": "$ de la Bête Sanguinaire",
    "FR_DESC": "*+@ £|*Chaque fois que vous tuez un ennemi, vous gagnez @ cumuls de Soif de Sang, jusqu'à @.#Chaque fois que vous subissez des dégâts alors que Fureur Sanguinaire n'est pas active, vous gagnez un cumul de Soif de Sang.*Vous avez +_% de £.#Ce Multiplicateur de Vie par Victime est équivalent à votre nombre de cumuls de Soif de Sang.*Si vous êtes vaincu, votre Vie devient @, vous gagnez jusqu'à @ cumuls de Soif de Sang et déclenchez immédiatement Fureur Sanguinaire.#Cet effet a un Temps de Recharge de @ secondes.*act:239/\n|*Lorsque Fureur Sanguinaire est active, vous gagnez également +_ Dégâts de Reaper.#Ces Dégâts de Reaper sont équivalents à votre Vie par Victime./\n|*Vous ne pouvez plus restaurer la vie qu'avec Vie par Victime.",
    "FR_LORE": "\"Un Reaper qui devient plus fort face au danger, peut atteindre des sommets si il est stimulé\". - Reapers de Légende, par le Pr.Géranium.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 27,
    "SPRITE": 25,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|1|1|1|60",
    "VALUE_TYPE": "|||||",
    "VALUE_LEVEL": "rl||rl||rl|",
    "VALUE_STAT": "health_on_kill_add|health_on_kill_global_mult",
    "VALUE_REAL": "synergy:bloodthirst_stacks:1|\nsynergy:health_on_kill:100",
    "BASE_DMG_MIN": 28,
    "BASE_DMG_MAX": 32,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1203 - 2410"
  },
  {
    "REF": 37,
    "EN_NAME": "Drum-$ of War",
    "EN_DESC": "*+@ £*+@ £|*Both Multipliers are also added to Drum Effects.*Whenever you cast a Projectile Skill with your Primary or Secondary Skill, the following Drum Effects are added to the cast and every subsequent cast until you reach the end of the Drum Loop:*Drum Effects:#Cast 1: Deals @ £.#Cast 2: Has +@ £.#Cast 3: Has +@ £./\n|*Whenever you complete a Drum Loop, you gain a Drums of War stack, up to @. Drums of War grants:#+@ £ on Cast @ and @.#+@ £ on Projectile Skills.#+@ £ on Projectile Skills.*Whenever you complete a Drum Loop, if you already have @ Drums of War stacks, you lose your Drums of War stacks./\n|*Your Additional Projectiles Stat is reduced to 0.",
    "EN_LORE": "The music of battle is as powerful as sharp steel.",
    "EN": "Drum-$ of War//*+@ £*+@ £|*Both Multipliers are also added to Drum Effects.*Whenever you cast a Projectile Skill with your Primary or Secondary Skill, the following Drum Effects are added to the cast and every subsequent cast until you reach the end of the Drum Loop:*Drum Effects:#Cast 1: Deals @ £.#Cast 2: Has +@ £.#Cast 3: Has +@ £./\n|*Whenever you complete a Drum Loop, you gain a Drums of War stack, up to @. Drums of War grants:#+@ £ on Cast @ and @.#+@ £ on Projectile Skills.#+@ £ on Projectile Skills.*Whenever you complete a Drum Loop, if you already have @ Drums of War stacks, you lose your Drums of War stacks./\n|*Your Additional Projectiles Stat is reduced to 0.//The music of battle is as powerful as sharp steel.",
    "FR": "$-Tambour de Guerre//*+@ £*+@ £|*Ces deux Multiplicateurs sont également ajoutés au Effets de Tambour.*Chaque fois que vous lancez une Compétence Primaire ou Secondaire de Projectile, les Effets de Tambour suivants sont ajoutés au lancer et à tous les lancers suivants jusqu'à ce que vous atteignez la fin de la Boucle de Tambour :*Effets de Tambour :#Lancer 1 : Inflige @ £.#Lancer 2 : a +@ de £.#Lancer 3 : a +@ de £./\n|*Chaque fois que vous terminez une Boucle de Tambour, vous gagnez un cumul de Tambours de Guerre, jusqu'à @. Tambours de Guerre vous confère :#+@ de £ aux Lancers @ et @.#+@ de £ aux Compétences de Projectile.#+@ de £ aux Compétences de Projectile.*A chaque fois que vous terminez une Boucle de Tambour, si vous avez déjà @ cumuls de Tambours de Guerre, vous perdez vos cumuls de Tambours de Guerre./\n|*Votre Stat de Projectiles Additionnels est réduit à 0.//La musique de la bataille est aussi puissante que l'acier tranchant.",
    "FR_NAME": "$-Tambour de Guerre",
    "FR_DESC": "*+@ £*+@ £|*Ces deux Multiplicateurs sont également ajoutés au Effets de Tambour.*Chaque fois que vous lancez une Compétence Primaire ou Secondaire de Projectile, les Effets de Tambour suivants sont ajoutés au lancer et à tous les lancers suivants jusqu'à ce que vous atteignez la fin de la Boucle de Tambour :*Effets de Tambour :#Lancer 1 : Inflige @ £.#Lancer 2 : a +@ de £.#Lancer 3 : a +@ de £./\n|*Chaque fois que vous terminez une Boucle de Tambour, vous gagnez un cumul de Tambours de Guerre, jusqu'à @. Tambours de Guerre vous confère :#+@ de £ aux Lancers @ et @.#+@ de £ aux Compétences de Projectile.#+@ de £ aux Compétences de Projectile.*A chaque fois que vous terminez une Boucle de Tambour, si vous avez déjà @ cumuls de Tambours de Guerre, vous perdez vos cumuls de Tambours de Guerre./\n|*Votre Stat de Projectiles Additionnels est réduit à 0.",
    "FR_LORE": "La musique de la bataille est aussi puissante que l'acier tranchant.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 37,
    "SPRITE": 32,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|0.5|15|15|\n5|1|4|6|10|20|5",
    "VALUE_TYPE": "%|%|%|%|%|\n||||%|%||",
    "VALUE_LEVEL": "rl|rl|rl|||\n|||||||",
    "VALUE_STAT": "chance_to_pierce_global_mult|fork_chance_global_mult|increased_damage|chance_to_pierce_percent|fork_chance_percent|\nadditional_projectile_add|crit_damage_percent|brut_damage_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 30,
    "BASE_DMG_MAX": 42,
    "MIN_DMG_LVL": 12,
    "MAX_DMG_LVL": 16,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 12,
    "EVOLVE_IN": 38,
    "100": "2175 - 2878"
  },
  {
    "REF": 38,
    "EN_NAME": "Epic Drum-$ of War",
    "EN_DESC": "|*Cast 4: Has +@ £.#Cast 5: Deals @ £.",
    "EN_LORE": "Enemies shall dance to the tune of their own defeat.",
    "EN": "Epic Drum-$ of War//|*Cast 4: Has +@ £.#Cast 5: Deals @ £.//Enemies shall dance to the tune of their own defeat.",
    "FR": "$-Tambour Épique de Guerre//|*Lancer 4: a +@ de £.#Lancer 5 : Inflige @ £.//Les ennemis danseront au rythme de leur propre défaite.",
    "FR_NAME": "$-Tambour Épique de Guerre",
    "FR_DESC": "|*Lancer 4: a +@ de £.#Lancer 5 : Inflige @ £.",
    "FR_LORE": "Les ennemis danseront au rythme de leur propre défaite.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 38,
    "SPRITE": 33,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "2|0.5",
    "VALUE_TYPE": "|%",
    "VALUE_LEVEL": "|rl",
    "VALUE_STAT": "additional_projectile_add|increased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 30,
    "BASE_DMG_MAX": 42,
    "MIN_DMG_LVL": 13,
    "MAX_DMG_LVL": 16,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 22,
    "EVOLVE_IN": 39,
    "100": "2346 - 2878"
  },
  {
    "REF": 39,
    "EN_NAME": "Epic Resounding Drum-$ of War",
    "EN_DESC": "|*Cast 6: Has +@ £.#Cast 7: Deals @ £.",
    "EN_LORE": "The drums of war thunder... once again...",
    "EN": "Epic Resounding Drum-$ of War//|*Cast 6: Has +@ £.#Cast 7: Deals @ £.//The drums of war thunder... once again...",
    "FR": "$-Tambour Épique de Guerre Résonnant/$-Tambour Épique de Guerre Résonnante//|*Lancer 6: a +@ de £.#Lancer 7 : Inflige @ £.//Une fois encore... Les tambours de guerre résonnent...",
    "FR_NAME": "$-Tambour Épique de Guerre Résonnant/$-Tambour Épique de Guerre Résonnante",
    "FR_DESC": "|*Lancer 6: a +@ de £.#Lancer 7 : Inflige @ £.",
    "FR_LORE": "Une fois encore... Les tambours de guerre résonnent...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 39,
    "SPRITE": 34,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "4|0.5",
    "VALUE_TYPE": "|%",
    "VALUE_LEVEL": "|rl",
    "VALUE_STAT": "additional_projectile_add|increased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 30,
    "BASE_DMG_MAX": 42,
    "MIN_DMG_LVL": 14,
    "MAX_DMG_LVL": 16,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2514 - 2878"
  },
  {
    "REF": 40,
    "EN_NAME": "Thornbite $",
    "EN_DESC": "*+@ £*+@ £|*When Idle for at least @ seconds:#+@ £##You also gain <Thornbite Protection> that shields you from the next _ Incoming Damage.#Thornbite Protection is equal to @ of the sum of your Armor, Elemental Resistance and Evasion, has a @ seconds Cooldown and disappears when you move./\n|*Thorns Damage have _% £.#Critical Strike Chance is equal to yours.*When Idle for at least @ seconds:#Thorns Damage have @ £./\n|*Non-Thorns Damage can no longer deal Critical Strikes.",
    "EN_LORE": "Careful, it stings.",
    "EN": "Thornbite $//*+@ £*+@ £|*When Idle for at least @ seconds:#+@ £##You also gain <Thornbite Protection> that shields you from the next _ Incoming Damage.#Thornbite Protection is equal to @ of the sum of your Armor, Elemental Resistance and Evasion, has a @ seconds Cooldown and disappears when you move./\n|*Thorns Damage have _% £.#Critical Strike Chance is equal to yours.*When Idle for at least @ seconds:#Thorns Damage have @ £./\n|*Non-Thorns Damage can no longer deal Critical Strikes.//Careful, it stings.",
    "FR": "$ en Mordépine//*+@ £*+@ £|*Lorsque vous restez Immobile pour au moins @ secondes :#+@ £##Vous gagnez également <Protection en Mordépine>, prévenant les _ prochains Dégâts reçus.#Protection en Mordépine est équivalente à @ de la somme de votre Armure, de votre Résistance Élémentaire et d'Évasion, a un Temps de Recharge de @ secondes et disparaît lorsque vous vous déplacez./\n|*Les Dégâts d'Épines ont _% de £.#Cette Chance de Coup Critique est équivalente à la vôtre.*Lorsque vous restez Immobile pour au moins @ secondes :#Les Dégâts d'Épines ont @ de £./\n|*Les Dégâts non-Épines ne peuvent plus infliger de Coups Critiques.//Attention, ça pique.",
    "FR_NAME": "$ en Mordépine",
    "FR_DESC": "*+@ £*+@ £|*Lorsque vous restez Immobile pour au moins @ secondes :#+@ £##Vous gagnez également <Protection en Mordépine>, prévenant les _ prochains Dégâts reçus.#Protection en Mordépine est équivalente à @ de la somme de votre Armure, de votre Résistance Élémentaire et d'Évasion, a un Temps de Recharge de @ secondes et disparaît lorsque vous vous déplacez./\n|*Les Dégâts d'Épines ont _% de £.#Cette Chance de Coup Critique est équivalente à la vôtre.*Lorsque vous restez Immobile pour au moins @ secondes :#Les Dégâts d'Épines ont @ de £./\n|*Les Dégâts non-Épines ne peuvent plus infliger de Coups Critiques.",
    "FR_LORE": "Attention, ça pique.",
    "CH": "棘刺 $//*+@£*+@£|*站立不动@秒后：#+@£##同时获得<Thornbite Protection>，可抵御_伤害。#棘刺防护等于护甲、元素抗性、闪避总和的@，有@秒冷却时间，在移动时消失。/\n|*荆棘伤害获得_%£。#暴击率等于你的。*站立不动@秒后：#荆棘伤害获得@£。/\n|*非荆棘伤害无法暴击。//小心，它会扎人。",
    "CH_NAME": "棘刺 $",
    "CH_DESCRIPTION": "*+@£*+@£|*站立不动@秒后：#+@£##同时获得<Thornbite Protection>，可抵御_伤害。#棘刺防护等于护甲、元素抗性、闪避总和的@，有@秒冷却时间，在移动时消失。/\n|*荆棘伤害获得_%£。#暴击率等于你的。*站立不动@秒后：#荆棘伤害获得@£。/\n|*非荆棘伤害无法暴击。",
    "CH_LORE": "小心，它会扎人。",
    "TW": "棘刺 $//*+@£*+@£|*站立不動@秒後：#+@£##同時獲得<Thornbite Protection>，可抵禦_傷害。 #棘刺防護等於護甲、元素抗性、閃避總和的@，有@秒冷卻時間，在移動時消失。 /\n|*荊棘傷害獲得_%£。 #暴擊率等於你的。 *站立不動@秒後：#荊棘傷害獲得@£。 /\n|*非荊棘傷害無法暴擊。//小心，它會扎人。",
    "TW_NAME": "棘刺 $",
    "TW_DESCRIPTION": "*+@£*+@£|*站立不動@秒後：#+@£##同時獲得<Thornbite Protection>，可抵禦_傷害。 #棘刺防護等於護甲、元素抗性、閃避總和的@，有@秒冷卻時間，在移動時消失。 /\n|*荊棘傷害獲得_%£。 #暴擊率等於你的。 *站立不動@秒後：#荊棘傷害獲得@£。 /\n|*非荊棘傷害無法暴擊。",
    "TW_LORE": "小心，它會扎人。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 40,
    "SPRITE": 40,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "4|0.5|0.5|1|10|2|\n0.5|1|\n",
    "VALUE_TYPE": "|%||%|%||\n|%|",
    "VALUE_LEVEL": "rl|rl||rl|||\n|rl|",
    "VALUE_STAT": "thorns_add|thorns_percent|thorns_global_mult|\ncrit_chance_percent|crit_chance_global_mult",
    "VALUE_REAL": "synergy:sum_all_resistances:10|\nsynergy:crit_chance:1",
    "BASE_DMG_MIN": 6,
    "BASE_DMG_MAX": 12,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 1,
    "MAX_LVL": 52,
    "EVOLVE_IN": 41,
    "100": "939 - 951"
  },
  {
    "REF": 41,
    "EN_NAME": "Thornbite $ of Idleness",
    "EN_DESC": "|*You have +_ £.#Thorns Damage is equal to your $.*The shield value of Thornbite Protection is also added to your Thorn Damage.",
    "EN_LORE": "Do not sit on it.",
    "EN": "Thornbite $ of Idleness//|*You have +_ £.#Thorns Damage is equal to your $.*The shield value of Thornbite Protection is also added to your Thorn Damage.//Do not sit on it.",
    "FR": "$ en Mordépine d'Oisiveté//|*Vous avez +_ de £.#Ces Dégâts d'Épines sont équivalents à vos $.*La valeur du bouclier de Protection en Mordépine est ajoutée à vos Dégâts d'Épines.//Evitez de vous asseoir dessus.",
    "FR_NAME": "$ en Mordépine d'Oisiveté",
    "FR_DESC": "|*Vous avez +_ de £.#Ces Dégâts d'Épines sont équivalents à vos $.*La valeur du bouclier de Protection en Mordépine est ajoutée à vos Dégâts d'Épines.",
    "FR_LORE": "Evitez de vous asseoir dessus.",
    "CH": "不动棘刺$//|*你获得+_£。#荆棘伤害等于你的$。*棘刺防护的护盾值同样被加算到荆棘伤害中。//别坐在上面。",
    "CH_NAME": "不动棘刺$",
    "CH_DESCRIPTION": "|*你获得+_£。#荆棘伤害等于你的$。*棘刺防护的护盾值同样被加算到荆棘伤害中。",
    "CH_LORE": "别坐在上面。",
    "TW": "不動棘刺$//|*你獲得+_£。 #荊棘傷害等於你的$。 *棘刺防護的護盾值同樣被加算到荊棘傷害中。//別坐在上面。",
    "TW_NAME": "不動棘刺$",
    "TW_DESCRIPTION": "|*你獲得+_£。 #荊棘傷害等於你的$。 *棘刺防護的護盾值同樣被加算到荊棘傷害中。",
    "TW_LORE": "別坐在上面。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 41,
    "SPRITE": 41,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "thorns_add",
    "VALUE_REAL": "synergy:physical_damage:100",
    "BASE_DMG_MIN": 6,
    "BASE_DMG_MAX": 12,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "939 - 951"
  },
  {
    "REF": 42,
    "EN_NAME": "$ of Necromancy",
    "EN_DESC": "\"*+@ £*+@ £|*The Attack Speed, Armor, Elemental Resistance, Critical Strike Chance and Critical Strike Damage of Skeleton Squires are equal to yours.*Mana Lock from Summons is reduced by @.*act:217*act:218/\n|*You have +_% £. Minion Increased Damage is equal to your percent of Mana Locked./\n|*Your Mana Leech is reduced to 0.",
    "EN_LORE": "\"The difference between Necromancy and Slormancy is very subtle...\"",
    "EN": "$ of Necromancy//\"*+@ £*+@ £|*The Attack Speed, Armor, Elemental Resistance, Critical Strike Chance and Critical Strike Damage of Skeleton Squires are equal to yours.*Mana Lock from Summons is reduced by @.*act:217*act:218/\n|*You have +_% £. Minion Increased Damage is equal to your percent of Mana Locked./\n|*Your Mana Leech is reduced to 0.//\"The difference between Necromancy and Slormancy is very subtle...\"",
    "FR": "$ de Nécromancie//\"*+@ £*+@ £|*La Vitesse d'Attaque, l'Armure, la Resistance Élémentaire, la Chance de Coup Critique et les Dégâts de Coup Critique des Ecuyers Squelettes sont équivalents aux votres.*Le Mana Verrouillé par les Invocations est réduit de @.*act:217*act:218/\n|*Vous avez +_% £. Les Dégâts Supplémentaires de Serviteur sont équivalents à votre pourcentage de Mana Verouillé./\n|*Le Vol de Mana est réduit à 0.//\"La différence entre la Slormancie et la Nécromancie est très subtile...\"",
    "FR_NAME": "$ de Nécromancie",
    "FR_DESC": "\"*+@ £*+@ £|*La Vitesse d'Attaque, l'Armure, la Resistance Élémentaire, la Chance de Coup Critique et les Dégâts de Coup Critique des Ecuyers Squelettes sont équivalents aux votres.*Le Mana Verrouillé par les Invocations est réduit de @.*act:217*act:218/\n|*Vous avez +_% £. Les Dégâts Supplémentaires de Serviteur sont équivalents à votre pourcentage de Mana Verouillé./\n|*Le Vol de Mana est réduit à 0.",
    "FR_LORE": "\"La différence entre la Slormancie et la Nécromancie est très subtile...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 42,
    "SPRITE": 42,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.5|0.25",
    "VALUE_TYPE": "%|%|%",
    "VALUE_LEVEL": "rl|rl|rl",
    "VALUE_STAT": "minion_increased_damage_percent|minion_increased_damage_global_mult|\nminion_increased_damage_percent",
    "VALUE_REAL": "synergy:mana_lock_percent:100",
    "BASE_DMG_MIN": 27,
    "BASE_DMG_MAX": 42,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 2,
    "MAX_LVL": 36,
    "EVOLVE_IN": 43,
    "100": "978 - 1429"
  },
  {
    "REF": 43,
    "EN_NAME": "$ of Great Necromancy",
    "EN_DESC": "|*Unstable Bones deals _% £.#Increased Damage is equal to your $.",
    "EN_LORE": "\"...Maybe a bit too much...\"",
    "EN": "$ of Great Necromancy//|*Unstable Bones deals _% £.#Increased Damage is equal to your $.//\"...Maybe a bit too much...\"",
    "FR": "$ de Grande Nécromancie//|*Os Instables inflige _% £.#Ces Dégâts Supplémentaires sont équivalents à votre $.//\"... Un peu trop peut-être...\"",
    "FR_NAME": "$ de Grande Nécromancie",
    "FR_DESC": "|*Os Instables inflige _% £.#Ces Dégâts Supplémentaires sont équivalents à votre $.",
    "FR_LORE": "\"... Un peu trop peut-être...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 43,
    "SPRITE": 43,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "increased_damage",
    "VALUE_REAL": "synergy:minion_increased_damage:1",
    "BASE_DMG_MIN": 30,
    "BASE_DMG_MAX": 45,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 52,
    "EVOLVE_IN": 44,
    "100": "981 - 1441"
  },
  {
    "REF": 44,
    "EN_NAME": "Sovereign $ of Great Necromancy",
    "EN_DESC": "|*Unstable Bones deals Increased Damage equal to the percent of Life missing of the sacrificed Skeleton Squire. ",
    "EN_LORE": "\"Truth be told, no one can really tell the difference...\"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "EN": "Sovereign $ of Great Necromancy//|*Unstable Bones deals Increased Damage equal to the percent of Life missing of the sacrificed Skeleton Squire. //\"Truth be told, no one can really tell the difference...\"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "FR": "$ Souverain de Grande Nécromancie/$ Souveraine de Grande Nécromancie//|*Os Instables inflige des Dégats Supplémentaires équivalents au pourcentage de Vie manquante de l'Ecuyer Squelette sacrifié.//\"A vrai dire, personne ne peut vraiment faire la différence...\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "FR_NAME": "$ Souverain de Grande Nécromancie/$ Souveraine de Grande Nécromancie",
    "FR_DESC": "|*Os Instables inflige des Dégats Supplémentaires équivalents au pourcentage de Vie manquante de l'Ecuyer Squelette sacrifié.",
    "FR_LORE": "\"A vrai dire, personne ne peut vraiment faire la différence...\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 44,
    "SPRITE": 44,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 33,
    "BASE_DMG_MAX": 48,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "990 - 1444"
  },
  {
    "REF": 45,
    "EN_NAME": "$ of the Recurring Nightmare",
    "EN_DESC": "*+@ £|*You have +_ Armor and +_ Elemental Resistance.#Armor and Elemental Resistance are equal to @ of your Max Life.*Whenever you're dealt Skill Damage, you gain a Fighter Bane Stack, up to @. Each Fighter Bane stack increases your Armor by @, and your Armor Penetration by @.*Whenever you're dealt Elemental Damage, you gain a Mage Bane stack, up to @. Each Mage Bane stack increases your Elemental Resistance by @, and your Elemental Resistance Penetration by @.*Whenever you deal Critical Strike Damage, you lose a Fighter Bane or a Mage Bane stack, whichever is the highest.#Whenever you deal Ancestral Strike Damage, you lose all your Fighter Bane and Mage Bane stacks./\n|*Your Primary and Secondary Skills deal _ % £.#Increased Damage is equal to twice the sum of Damage absorbed by your Armor and Elemental Resistance at this Wrath Level./\n|*Added Skill Damage and Elemental Damage gained from Equipment Stats are halved.",
    "EN_LORE": "An unholy Reaper, born from the darkness of troubled dreams.",
    "EN": "$ of the Recurring Nightmare//*+@ £|*You have +_ Armor and +_ Elemental Resistance.#Armor and Elemental Resistance are equal to @ of your Max Life.*Whenever you're dealt Skill Damage, you gain a Fighter Bane Stack, up to @. Each Fighter Bane stack increases your Armor by @, and your Armor Penetration by @.*Whenever you're dealt Elemental Damage, you gain a Mage Bane stack, up to @. Each Mage Bane stack increases your Elemental Resistance by @, and your Elemental Resistance Penetration by @.*Whenever you deal Critical Strike Damage, you lose a Fighter Bane or a Mage Bane stack, whichever is the highest.#Whenever you deal Ancestral Strike Damage, you lose all your Fighter Bane and Mage Bane stacks./\n|*Your Primary and Secondary Skills deal _ % £.#Increased Damage is equal to twice the sum of Damage absorbed by your Armor and Elemental Resistance at this Wrath Level./\n|*Added Skill Damage and Elemental Damage gained from Equipment Stats are halved.//An unholy Reaper, born from the darkness of troubled dreams.",
    "FR": "$ de Cauchemar Récurrent//*+@ £|*Vous avez +_ d'Armure et +_ de Résistance Élémentaire.#L'Armure et la Résistance Élémentaire sont équivalentes à @ de votre Vie Max.*Chaque fois que vous subissez des Dégâts Physiques, vous gagnez un cumul de Fléau du Combattant, jusqu'à @. Chaque cumul de Fléau du Combattant augmente votre Armure de @ et votre Pénétration d'Armure de @.*Chaque fois que vous subissez des Dégâts Élémentaires, vous gagnez un cumul de Fléau du Mage, jusqu'à @. Chaque cumul de Fléau du Mage augmente votre Résistance Élémentaire de @ et votre Pénétration de Résistance Élémentaire de @.*Chaque fois que vous infligez des Dégâts Critiques, vous perdez un cumul de Fléau du Combattant ou du Mage, selon la plus élevée.#Chaque fois que vous infligez des Dégâts Ancestraux, vous perdez tous vos cumuls de Fléau du Combattant et du Mage./\n|*Vos Compétences Primaires et Secondaires infligent _ % de £.#Ces Dégâts Supplémentaires sont équivalents au double de la somme des Dégâts absorbables par votre Armure et votre Résistance Élémentaire à ce Niveau de Colère./\n|*Les Dégâts Physiques Additionnels et les Dégâts Élémentaires Additionnels obtenus à partir des Stats d'Équipement sont réduites de moitié.//Un Reaper maléfique, né de l'obscurité de songes troublés.",
    "FR_NAME": "$ de Cauchemar Récurrent",
    "FR_DESC": "*+@ £|*Vous avez +_ d'Armure et +_ de Résistance Élémentaire.#L'Armure et la Résistance Élémentaire sont équivalentes à @ de votre Vie Max.*Chaque fois que vous subissez des Dégâts Physiques, vous gagnez un cumul de Fléau du Combattant, jusqu'à @. Chaque cumul de Fléau du Combattant augmente votre Armure de @ et votre Pénétration d'Armure de @.*Chaque fois que vous subissez des Dégâts Élémentaires, vous gagnez un cumul de Fléau du Mage, jusqu'à @. Chaque cumul de Fléau du Mage augmente votre Résistance Élémentaire de @ et votre Pénétration de Résistance Élémentaire de @.*Chaque fois que vous infligez des Dégâts Critiques, vous perdez un cumul de Fléau du Combattant ou du Mage, selon la plus élevée.#Chaque fois que vous infligez des Dégâts Ancestraux, vous perdez tous vos cumuls de Fléau du Combattant et du Mage./\n|*Vos Compétences Primaires et Secondaires infligent _ % de £.#Ces Dégâts Supplémentaires sont équivalents au double de la somme des Dégâts absorbables par votre Armure et votre Résistance Élémentaire à ce Niveau de Colère./\n|*Les Dégâts Physiques Additionnels et les Dégâts Élémentaires Additionnels obtenus à partir des Stats d'Équipement sont réduites de moitié.",
    "FR_LORE": "Un Reaper maléfique, né de l'obscurité de songes troublés.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 45,
    "SPRITE": 107,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.2|30|3|2|30|3|2|\n0",
    "VALUE_TYPE": "%|%||%|%||%|%|",
    "VALUE_LEVEL": "rl|rl|||||||",
    "VALUE_STAT": "the_max_health_percent|\nincreased_damage",
    "VALUE_REAL": "synergy:max_health:0.2*rl|synergy:max_health:0.2*rl|\nsynergy:absorbed_damage_wrath:2",
    "BASE_DMG_MIN": 14,
    "BASE_DMG_MAX": 22,
    "MIN_DMG_LVL": 8,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": null,
    "EVOLVE_IN": null,
    "100": "1465 - 1985"
  },
  {
    "REF": 46,
    "EN_NAME": "$ of Ferocious Affinity",
    "EN_DESC": "*+@ £*+@ £|*You have +_ £.#Raw Damage is equal to @ of your $.*Whenever you deal damage to an enemy, you have @ £ to <Afflict> him, dealing _ £ over @ seconds to him.#Afflict Damage is equal to @% of your $./\n|*You have +_ £.#Elemental Damage is equal to @ of your $./\n|*Whenever you cast a Skill, @ of your Mana is consumed.",
    "EN_LORE": "\"If enemies see you handling this Slorm Reaper... then it's already too late for them... \"#- Slorm Reapers of Legend, by Pr. Geranium",
    "EN": "$ of Ferocious Affinity//*+@ £*+@ £|*You have +_ £.#Raw Damage is equal to @ of your $.*Whenever you deal damage to an enemy, you have @ £ to <Afflict> him, dealing _ £ over @ seconds to him.#Afflict Damage is equal to @% of your $./\n|*You have +_ £.#Elemental Damage is equal to @ of your $./\n|*Whenever you cast a Skill, @ of your Mana is consumed.//\"If enemies see you handling this Slorm Reaper... then it's already too late for them... \"#- Slorm Reapers of Legend, by Pr. Geranium",
    "FR": "$ d'Affinité Farouche//*+@ £*+@ £|*Vous avez +_ de £.#Ces Dégats Bruts sont équivalents à @ de votre $.*Lorsque vous infligez des dégâts à un ennemi, vous avez @ de £ de lui appliquer <Affliction>, infligeant _ de £ sur @ secondes.#Les Dégats d'Affliction sont équivalents à @% de votre $./\n|*Vous avez +_ de £.#Ces Dégâts Élémentaires sont équivalents à @ de votre $./\n|*Lorsque vous lancez une Compétence, @ de votre Mana est consommé.//\"Si des ennemis vous voient manier ce Slorm Reaper... alors il est déjà trop tard pour eux...\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "$ d'Affinité Farouche",
    "FR_DESC": "*+@ £*+@ £|*Vous avez +_ de £.#Ces Dégats Bruts sont équivalents à @ de votre $.*Lorsque vous infligez des dégâts à un ennemi, vous avez @ de £ de lui appliquer <Affliction>, infligeant _ de £ sur @ secondes.#Les Dégats d'Affliction sont équivalents à @% de votre $./\n|*Vous avez +_ de £.#Ces Dégâts Élémentaires sont équivalents à @ de votre $./\n|*Lorsque vous lancez une Compétence, @ de votre Mana est consommé.",
    "FR_LORE": "\"Si des ennemis vous voient manier ce Slorm Reaper... alors il est déjà trop tard pour eux...\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "勇猛亲和之$//*+@£*+@£|*你获得+_£。#原始伤害等于你的$的@。*造成伤害时，有@£施加<Afflict>，在@秒内造成_£。#磨难伤害等于你的$的@%。/\n|*你获得+_£。#元素伤害等于你的$的@%。/\n|*施放技能时，消耗法力的@。//“如果敌人看到你拿着这玩意儿……那么就太晚了……”#- 猎魂传奇，杰兰涅姆教授",
    "CH_NAME": "勇猛亲和之$",
    "CH_DESCRIPTION": "*+@£*+@£|*你获得+_£。#原始伤害等于你的$的@。*造成伤害时，有@£施加<Afflict>，在@秒内造成_£。#磨难伤害等于你的$的@%。/\n|*你获得+_£。#元素伤害等于你的$的@%。/\n|*施放技能时，消耗法力的@。",
    "CH_LORE": "“如果敌人看到你拿着这玩意儿……那么就太晚了……”#- 猎魂传奇，杰兰涅姆教授",
    "TW": "勇猛親和之$//*+@£*+@£|*你獲得+_£。 #原始傷害等於你的$的@。 *造成傷害時，有@£施加<Afflict>，在@秒內造成_£。 #磨難傷害等於你的$的@%。 /\n|*你獲得+_£。 #元素傷害等於你的$的@%。 /\n|*施放技能時，消耗法力的@。//“如果敵人看到你拿著這玩意兒……那麼就太晚了……”#- 獵魂傳奇，傑蘭涅姆教授",
    "TW_NAME": "勇猛親和之$",
    "TW_DESCRIPTION": "*+@£*+@£|*你獲得+_£。 #原始傷害等於你的$的@。 *造成傷害時，有@£施加<Afflict>，在@秒內造成_£。 #磨難傷害等於你的$的@%。 /\n|*你獲得+_£。 #元素傷害等於你的$的@%。 /\n|*施放技能時，消耗法力的@。",
    "TW_LORE": "“如果敵人看到你拿著這玩意兒……那麼就太晚了……”#- 獵魂傳奇，傑蘭涅姆教授",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 46,
    "SPRITE": 46,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "15|0.5|8|0.3|10|100|\n15|\n5",
    "VALUE_TYPE": "|%|%|%|||\n%|\n%",
    "VALUE_LEVEL": "rl|rl||rl|||\n|\n",
    "VALUE_STAT": "the_max_mana_add|the_max_mana_percent|min_basic_damage_add|chance|elemental_damage|\nmin_elemental_damage_add|",
    "VALUE_REAL": "synergy:max_mana:8|synergy:current_mana:100|\nsynergy:max_mana:15",
    "BASE_DMG_MIN": 23,
    "BASE_DMG_MAX": 28,
    "MIN_DMG_LVL": 7,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1319 - 2005"
  },
  {
    "REF": 47,
    "EN_NAME": "Legion, The First $ of Unity",
    "EN_DESC": "|*You now have a Vigilant Blade patrolling around you that automatically targets and strikes random enemies within a @ yard radius, dealing _ £.*Vigilant Blades deal _ Additional Damage. Additional Damage is equal to the total Reaper Damage from all Slorm Reapers called Legion, across all your Characters.*Whenever Legion evolves, its evolution is set back to Level @ and its previous form can continue to level up./\n|*You have +_ £. Reaper Damage is equal to the total Reaper Damage of all Slorm Reapers called Legion, across all your Characters./\n|*Your Raw Damage is set to @.",
    "EN_LORE": "\"I...",
    "EN": "Legion, The First $ of Unity//|*You now have a Vigilant Blade patrolling around you that automatically targets and strikes random enemies within a @ yard radius, dealing _ £.*Vigilant Blades deal _ Additional Damage. Additional Damage is equal to the total Reaper Damage from all Slorm Reapers called Legion, across all your Characters.*Whenever Legion evolves, its evolution is set back to Level @ and its previous form can continue to level up./\n|*You have +_ £. Reaper Damage is equal to the total Reaper Damage of all Slorm Reapers called Legion, across all your Characters./\n|*Your Raw Damage is set to @.//\"I...",
    "FR": "Légion, Le Premier $ d'Unité/Légion, La Premiere $ d'Unité//|*Vous avez maintenant une Lame Vigilante patrouillant autour de vous qui cible automatiquement et frappe des ennemis aléatoires dans un rayon de @ mètres, infligeant _ de £.*Les Lames Vigilantes infligent _ de Dégâts Additionnels. Ces Dégâts Additionnels sont équivalents au total des Dégâts de Reaper de tous les Slorm Reapers appelés Légion, à travers tous vos Personnages*Chaque fois que Légion évolue, son évolution est ramenée au Niveau @ et sa forme précédente peut continuer à monter de niveau./\n|*Vous avez +_ de £. Ces Dégâts de Reaper sont équivalents au total des Dégâts de Reaper de tous les Slorm Reapers appelés Légion, à travers tous vos Personnages./\n|*Vos Dégâts Bruts deviennent @.//\"Je...",
    "FR_NAME": "Légion, Le Premier $ d'Unité/Légion, La Premiere $ d'Unité",
    "FR_DESC": "|*Vous avez maintenant une Lame Vigilante patrouillant autour de vous qui cible automatiquement et frappe des ennemis aléatoires dans un rayon de @ mètres, infligeant _ de £.*Les Lames Vigilantes infligent _ de Dégâts Additionnels. Ces Dégâts Additionnels sont équivalents au total des Dégâts de Reaper de tous les Slorm Reapers appelés Légion, à travers tous vos Personnages*Chaque fois que Légion évolue, son évolution est ramenée au Niveau @ et sa forme précédente peut continuer à monter de niveau./\n|*Vous avez +_ de £. Ces Dégâts de Reaper sont équivalents au total des Dégâts de Reaper de tous les Slorm Reapers appelés Légion, à travers tous vos Personnages./\n|*Vos Dégâts Bruts deviennent @.",
    "FR_LORE": "\"Je...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 96,
    "SPRITE": 4,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "2|1|\n1",
    "VALUE_TYPE": "||",
    "VALUE_LEVEL": "||",
    "VALUE_STAT": "physical_damage|\nmin_weapon_damage_add\n",
    "VALUE_REAL": "damage:physical_damage:30|synergy:legion_reaper_dmg:1|\nsynergy:legion_reaper_dmg:1",
    "BASE_DMG_MIN": 2,
    "BASE_DMG_MAX": 4,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 3,
    "MAX_LVL": 100,
    "EVOLVE_IN": 48,
    "100": "764 - 1104"
  },
  {
    "REF": 48,
    "EN_NAME": "Legion, The Second $ of Unity",
    "EN_DESC": "|*Vigilant Blades now strikes _ additional times before coming back to you. This being equal to the number of Slorm Reapers called Legion, The Second you own across all your Characters.",
    "EN_LORE": "Am...",
    "EN": "Legion, The Second $ of Unity//|*Vigilant Blades now strikes _ additional times before coming back to you. This being equal to the number of Slorm Reapers called Legion, The Second you own across all your Characters.//Am...",
    "FR": "Légion, Le Second $ d'Unité/Légion, La Seconde $ d'Unité//|*Les Lames Vigilantes frappent désormais _ fois supplémentaires avant de revenir vers vous. Ces frappes supplémentaires sont équivalentes au nombre de Slorm Reapers appelés Légion, Second que vous possédez sur l'ensemble de vos Personnages.//Suis...",
    "FR_NAME": "Légion, Le Second $ d'Unité/Légion, La Seconde $ d'Unité",
    "FR_DESC": "|*Les Lames Vigilantes frappent désormais _ fois supplémentaires avant de revenir vers vous. Ces frappes supplémentaires sont équivalentes au nombre de Slorm Reapers appelés Légion, Second que vous possédez sur l'ensemble de vos Personnages.",
    "FR_LORE": "Suis...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 97,
    "SPRITE": 52,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:legion_2:1",
    "BASE_DMG_MIN": 2,
    "BASE_DMG_MAX": 4,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": 49,
    "100": "764 - 1104"
  },
  {
    "REF": 49,
    "EN_NAME": "Legion, The Third $ of Unity",
    "EN_DESC": "|*Target radius of Vigilant Blades and Strike length are increased by _ %. This being equal to @ for each Slorm Reaper called Legion, the Third you own across all your Characters.",
    "EN_LORE": "Legion!",
    "EN": "Legion, The Third $ of Unity//|*Target radius of Vigilant Blades and Strike length are increased by _ %. This being equal to @ for each Slorm Reaper called Legion, the Third you own across all your Characters.//Legion!",
    "FR": "Légion, Le Troisième $ d'Unité/Légion, La Troisième $ d'Unité//|*Le rayon des Lames Vigilantes et la longueur de la frappe sont augmentés de _ %. Ce nombre est équivalent à @ pour chaque Slorm Reaper appelé Légion, Le Troisième que vous possédez sur l'ensemble de vos personnages.//Légion !",
    "FR_NAME": "Légion, Le Troisième $ d'Unité/Légion, La Troisième $ d'Unité",
    "FR_DESC": "|*Le rayon des Lames Vigilantes et la longueur de la frappe sont augmentés de _ %. Ce nombre est équivalent à @ pour chaque Slorm Reaper appelé Légion, Le Troisième que vous possédez sur l'ensemble de vos personnages.",
    "FR_LORE": "Légion !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 98,
    "SPRITE": 5,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "30",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:legion_3:30",
    "BASE_DMG_MIN": 2,
    "BASE_DMG_MAX": 4,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": 50,
    "100": "764 - 1104"
  },
  {
    "REF": 50,
    "EN_NAME": "Legion, The Fourth $ of Unity",
    "EN_DESC": "|*Vigilant Blades have +_% Critical Strike Damage. This being equal to @ for each Slorm Reaper called Legion, the Fourth you own across all your Characters.",
    "EN_LORE": "We...",
    "EN": "Legion, The Fourth $ of Unity//|*Vigilant Blades have +_% Critical Strike Damage. This being equal to @ for each Slorm Reaper called Legion, the Fourth you own across all your Characters.//We...",
    "FR": "Légion, Le Quatrième $ d'Unité/Légion, La Quatrième $ d'Unité//|*Les Lames Vigilantes ont +_% de Dégâts Critiques. Ces Dégâts Critiques sont équivalents à @ pour chaque Slorm Reaper appelé Légion, le Quatrième que vous possédez sur l'ensemble de vos personnages.//Nous...",
    "FR_NAME": "Légion, Le Quatrième $ d'Unité/Légion, La Quatrième $ d'Unité",
    "FR_DESC": "|*Les Lames Vigilantes ont +_% de Dégâts Critiques. Ces Dégâts Critiques sont équivalents à @ pour chaque Slorm Reaper appelé Légion, le Quatrième que vous possédez sur l'ensemble de vos personnages.",
    "FR_LORE": "Nous...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 99,
    "SPRITE": 55,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "50",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:legion_4:50",
    "BASE_DMG_MIN": 2,
    "BASE_DMG_MAX": 4,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": 51,
    "100": "764 - 1104"
  },
  {
    "REF": 51,
    "EN_NAME": "Legion, The Fifth $ of Unity",
    "EN_DESC": "|*The number of Vigilant Blades patrolling around you is increased by _. This being equal to the sum of Slorm Reapers called Legion, The Fifth and Legion, The Sixth you own across all your Characters.",
    "EN_LORE": "Are...",
    "EN": "Legion, The Fifth $ of Unity//|*The number of Vigilant Blades patrolling around you is increased by _. This being equal to the sum of Slorm Reapers called Legion, The Fifth and Legion, The Sixth you own across all your Characters.//Are...",
    "FR": "Légion, L(e&a) Cinquième $ d'Unité/Légion, La Cinquième $ d'Unité//|*Le nombre de Lames Vigilantes qui patrouillent autour de vous est augmenté de _. Ce nombre est équivalent la somme de Slorm Reapers appelés Légion, Le Cinquième et Légion, Le Sixième que vous possédez sur l'ensemble de vos personnages.//Sommes...",
    "FR_NAME": "Légion, L(e&a) Cinquième $ d'Unité/Légion, La Cinquième $ d'Unité",
    "FR_DESC": "|*Le nombre de Lames Vigilantes qui patrouillent autour de vous est augmenté de _. Ce nombre est équivalent la somme de Slorm Reapers appelés Légion, Le Cinquième et Légion, Le Sixième que vous possédez sur l'ensemble de vos personnages.",
    "FR_LORE": "Sommes...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 100,
    "SPRITE": 59,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:legion_5:1",
    "BASE_DMG_MIN": 2,
    "BASE_DMG_MAX": 4,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": 52,
    "100": "764 - 1104"
  },
  {
    "REF": 52,
    "EN_NAME": "Legion, The Sixth $ of Unity",
    "EN_DESC": "|*act:240",
    "EN_LORE": "Unity!\"",
    "EN": "Legion, The Sixth $ of Unity//|*act:240//Unity!\"",
    "FR": "Légion, Le Sixième $ d'Unité/Légion, La Sixième $ d'Unité//|*act:240//Unité !\"",
    "FR_NAME": "Légion, Le Sixième $ d'Unité/Légion, La Sixième $ d'Unité",
    "FR_DESC": "|*act:240",
    "FR_LORE": "Unité !\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 101,
    "SPRITE": 108,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 2,
    "BASE_DMG_MAX": 4,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "764 - 1104"
  },
  {
    "REF": 53,
    "EN_NAME": "Fate-Crusher, $ of the Seven",
    "EN_DESC": "|*A Forge Hammer has _% Chance to spawn near you every second. Chance to spawn is equal to the total of Affinity Levels on your Equipment.*Picking up the Forge Hammer triggers a Slorm Hammer, dealing _ Skill Damage to a random enemy nearby.*The Slorm Hammer deals @ Increased Damage.*Astorias' Chain:#When the Slorm Hammer hits an enemy, it has _% Chance to target a new random enemy nearby.*Adrianne's Division:#When the Slorm Hammer hits an enemy, it has _% Chance to create a new Slorm Hammer, once.*Beighart's Alchimy:#When the Slorm Hammer ends, it has _% Chance to create a new Forge Hammer.*Cory's Ingeniosity:#Forge Hammers have _% Chance to trigger themselves.*Smaloron's Might:#Slorm Hammers deal _% Increased Damage for every enemy hit.*Fulgurorn's Dedication:#Triggering a Forge Hammer grants you Fulgurorn's Dedication increasing the Affinity Level of this Reaper by @ for the next @ seconds. Fulgurorn's Dedication can stack up to _ times.*Hagan's Heroism:#Slorm Hammers have +_% Increased Critical Strike Damage and have _% Chance to Stun for the next @ seconds./\n|*Picking up a Forge Hammer triggers a second Slorm Hammer./\n|*Only Slorm Hammers can kill enemies.",
    "EN_LORE": "Wrought by the Seven Reapersmiths to defeat HIM.",
    "EN": "Fate-Crusher, $ of the Seven//|*A Forge Hammer has _% Chance to spawn near you every second. Chance to spawn is equal to the total of Affinity Levels on your Equipment.*Picking up the Forge Hammer triggers a Slorm Hammer, dealing _ Skill Damage to a random enemy nearby.*The Slorm Hammer deals @ Increased Damage.*Astorias' Chain:#When the Slorm Hammer hits an enemy, it has _% Chance to target a new random enemy nearby.*Adrianne's Division:#When the Slorm Hammer hits an enemy, it has _% Chance to create a new Slorm Hammer, once.*Beighart's Alchimy:#When the Slorm Hammer ends, it has _% Chance to create a new Forge Hammer.*Cory's Ingeniosity:#Forge Hammers have _% Chance to trigger themselves.*Smaloron's Might:#Slorm Hammers deal _% Increased Damage for every enemy hit.*Fulgurorn's Dedication:#Triggering a Forge Hammer grants you Fulgurorn's Dedication increasing the Affinity Level of this Reaper by @ for the next @ seconds. Fulgurorn's Dedication can stack up to _ times.*Hagan's Heroism:#Slorm Hammers have +_% Increased Critical Strike Damage and have _% Chance to Stun for the next @ seconds./\n|*Picking up a Forge Hammer triggers a second Slorm Hammer./\n|*Only Slorm Hammers can kill enemies.//Wrought by the Seven Reapersmiths to defeat HIM.",
    "FR": "Renverse-Destin, $ des Sept//|*Un Marteau de Forge a _% de Chance d'apparaître près de vous chaque seconde. Cette Chance est équivalente au total de vos bonus d'Affinité sur votre équipement.*Ramasser un Marteau de Forge déclenche un Marteau de Slorm, infligeant _ Dégats Physiques à un ennemi aléatoire proche.*Le Marteau de Slorm inflige @ de Dégâts Supplémentaires.*Liens d'Astorias :#Lorsque le Marteau de Slorm touche un ennemi, il a _% de Chance de cibler un nouvel ennemi aléatoire proche.*Division d'Adrianne :#Lorsque le Marteau de Slorm touche un ennemi, il a _% de Chance de créer un autre Marteau de Slorm, une seule fois.*Alchimie de Beighart :#Lorsqu'un Marteau de Slorm se termine, il a _% de Chance de créer un nouveau Marteau de Forge.*Ingéniosité de Cory :#Les Marteaux de Forge ont _% de Chance de se déclencher tout seuls.*Puissance de Smaloron :#Les Marteaux de Slorm infligent _% de Dégats Supplémentaires par ennemi touché.*Dévouement de Fulgurorn :#Déclencher un Marteau de Forge vous confère Dévouement de Fulgurorn, augmentant le niveau d'Affinité de ce Slorm Reaper de @ pour les @ prochaines secondes. Dévouement de Fulgurorn peut se cumuler jusqu'à _ fois.*Héroïsme d'Hagan :#Les Marteaux de Slorm ont +_% de Dégâts de Coup Critique et ont _% de Chance d'Étourdir les ennemis touchés pour les @ prochaines secondes./\n|*Ramasser un Marteau de Forge déclenche un second Marteau de Slorm./\n|*Seuls les Marteaux de Slorm peuvent tuer les ennemis.//Créé par les sept Reapergerons afin de le vaincre. Lui.",
    "FR_NAME": "Renverse-Destin, $ des Sept",
    "FR_DESC": "|*Un Marteau de Forge a _% de Chance d'apparaître près de vous chaque seconde. Cette Chance est équivalente au total de vos bonus d'Affinité sur votre équipement.*Ramasser un Marteau de Forge déclenche un Marteau de Slorm, infligeant _ Dégats Physiques à un ennemi aléatoire proche.*Le Marteau de Slorm inflige @ de Dégâts Supplémentaires.*Liens d'Astorias :#Lorsque le Marteau de Slorm touche un ennemi, il a _% de Chance de cibler un nouvel ennemi aléatoire proche.*Division d'Adrianne :#Lorsque le Marteau de Slorm touche un ennemi, il a _% de Chance de créer un autre Marteau de Slorm, une seule fois.*Alchimie de Beighart :#Lorsqu'un Marteau de Slorm se termine, il a _% de Chance de créer un nouveau Marteau de Forge.*Ingéniosité de Cory :#Les Marteaux de Forge ont _% de Chance de se déclencher tout seuls.*Puissance de Smaloron :#Les Marteaux de Slorm infligent _% de Dégats Supplémentaires par ennemi touché.*Dévouement de Fulgurorn :#Déclencher un Marteau de Forge vous confère Dévouement de Fulgurorn, augmentant le niveau d'Affinité de ce Slorm Reaper de @ pour les @ prochaines secondes. Dévouement de Fulgurorn peut se cumuler jusqu'à _ fois.*Héroïsme d'Hagan :#Les Marteaux de Slorm ont +_% de Dégâts de Coup Critique et ont _% de Chance d'Étourdir les ennemis touchés pour les @ prochaines secondes./\n|*Ramasser un Marteau de Forge déclenche un second Marteau de Slorm./\n|*Seuls les Marteaux de Slorm peuvent tuer les ennemis.",
    "FR_LORE": "Créé par les sept Reapergerons afin de le vaincre. Lui.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 53,
    "SPRITE": 53,
    "BLACKSMITH": 8,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|10|5",
    "VALUE_TYPE": "%|||",
    "VALUE_LEVEL": "rl|||",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:reaper_bonus:1|damage:physical_damage:75|synergy:reapersmith_0:3|synergy:reapersmith_1:4|synergy:reapersmith_2:2|synergy:reapersmith_3:10|synergy:reapersmith_4:1|synergy:reapersmith_5:3|synergy:reapersmith_6:2|synergy:reapersmith_6:1|\n\n",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 16,
    "MIN_DMG_LVL": 2,
    "MAX_DMG_LVL": 4,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "654 - 1150"
  },
  {
    "REF": 54,
    "EN_NAME": "The Most Powerful $",
    "EN_DESC": "/\n*+@ £|/\n|*Your Elemental Damage is reduced to 0.",
    "EN_LORE": "\"Well, the damage output is fairly decent\"#- A Victim.",
    "EN": "The Most Powerful $///\n*+@ £|/\n|*Your Elemental Damage is reduced to 0.//\"Well, the damage output is fairly decent\"#- A Victim.",
    "FR": "$ le Plus Puissant/$ la Plus Puissante///\n*+@ £|/\n|*Vos Dégâts Élémentaires sont réduits à 0.//\"Les dégâts sont tout à fait corrects.\"#- Une Victime",
    "FR_NAME": "$ le Plus Puissant/$ la Plus Puissante",
    "FR_DESC": "/\n*+@ £|/\n|*Vos Dégâts Élémentaires sont réduits à 0.",
    "FR_LORE": "\"Les dégâts sont tout à fait corrects.\"#- Une Victime",
    "CH": "最强大的$///\n*+@£|/\n|*元素伤害减少为0.//*“哦，真是成吨的伤害”#- 受害者",
    "CH_NAME": "最强大的$",
    "CH_DESCRIPTION": "/\n*+@£|/\n|*元素伤害减少为0.",
    "CH_LORE": "*“哦，真是成吨的伤害”#- 受害者",
    "TW": "最強大的$///\n*+@£|/\n|*元素傷害減少為0.//*“哦，真是成噸的傷害”#- 受害者",
    "TW_NAME": "最強大的$",
    "TW_DESCRIPTION": "/\n*+@£|/\n|*元素傷害減少為0.",
    "TW_LORE": "*“哦，真是成噸的傷害”#- 受害者",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 56,
    "SPRITE": 54,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "rl|",
    "VALUE_STAT": "weapon_damage_mult",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 22,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 5,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1866 - 3058"
  },
  {
    "REF": 55,
    "EN_NAME": "$ of Time and Space",
    "EN_DESC": "|*Reaper Damage is added to both Skill Damage and Elemental Damage.*Whenever you deal Damage with your Primary or Secondary Skill you have a @ £ to gain Brilliance for Brutal Meteor for the next @ seconds if you dealt Skill Damage and a @ £ to gain Brilliance for Elemental Meteor for the next @ seconds if you dealt Elemental Damage. When you gain Brilliance, it resets the Cooldown of the Meteor.*While Brilliance is active, casting the Meteor will cause it to deal @ £.#When Brilliance ends, if you didn't cast the Meteor,  it gains a Meteor Charge, up to @, increasing the number of Meteors of the next cast.*act:241*act:242/\n|*Whenever you gain a Meteor Charge, you have a @ £ to gain Brilliance for Astral Meteor for the next @ seconds.*act:243/\n|*When casting Brutal or Elemental Meteor, if the previous Brutal or Elemental Meteor cast is the same, you lose all your Meteor Charges.",
    "EN_LORE": "\"Remember to look up at the stars...\"",
    "EN": "$ of Time and Space//|*Reaper Damage is added to both Skill Damage and Elemental Damage.*Whenever you deal Damage with your Primary or Secondary Skill you have a @ £ to gain Brilliance for Brutal Meteor for the next @ seconds if you dealt Skill Damage and a @ £ to gain Brilliance for Elemental Meteor for the next @ seconds if you dealt Elemental Damage. When you gain Brilliance, it resets the Cooldown of the Meteor.*While Brilliance is active, casting the Meteor will cause it to deal @ £.#When Brilliance ends, if you didn't cast the Meteor,  it gains a Meteor Charge, up to @, increasing the number of Meteors of the next cast.*act:241*act:242/\n|*Whenever you gain a Meteor Charge, you have a @ £ to gain Brilliance for Astral Meteor for the next @ seconds.*act:243/\n|*When casting Brutal or Elemental Meteor, if the previous Brutal or Elemental Meteor cast is the same, you lose all your Meteor Charges.//\"Remember to look up at the stars...\"",
    "FR": "$ du Temps et de l'Espace//|*Les Dégâts de Reaper sont ajoutés à la fois aux Dégâts Physiques et aux Dégâts Élémentaires.*Chaque fois que vous infligez des Dégâts avec votre Compétence Primaire ou Secondaire, vous avez @ de £ de gagner Brillance pour Météore Brutal pendant les @ prochaines secondes si vous avez infligé des Dégâts Physiques, et @ de £ de gagner Brillance pour Météore Élémentaire pendant les @ prochaines secondes si vous avez infligé des Dégâts Élémentaires. Lorsque vous gagnez Brillance, cela réinitialise le Temps de Recharge du Météore.*Tant que Brillance est active, lancer le Météore lui permettra d'infliger @ de £.#Lorsque Brillance se termine, si vous n'avez pas lancé le Météore, il gagne une Charge de Météore, jusqu'à @, augmentant le nombre de Météores du prochain lancer.*act:241*act:242/\n|*Chaque fois que vous gagnez une Charge de Météore, vous avez @ de £ de gagner Brillance pour Météore Astral pour les @ prochaines secondes.*act:243/\n|*Lors du lancement de Météore Brutal ou Élémentaire, si le précédent lancement de Météore Brutal ou Élémentaire est identique, vous perdez toutes vos Charges de Météore.//\"N'oubliez pas de regarder les étoiles...\"",
    "FR_NAME": "$ du Temps et de l'Espace",
    "FR_DESC": "|*Les Dégâts de Reaper sont ajoutés à la fois aux Dégâts Physiques et aux Dégâts Élémentaires.*Chaque fois que vous infligez des Dégâts avec votre Compétence Primaire ou Secondaire, vous avez @ de £ de gagner Brillance pour Météore Brutal pendant les @ prochaines secondes si vous avez infligé des Dégâts Physiques, et @ de £ de gagner Brillance pour Météore Élémentaire pendant les @ prochaines secondes si vous avez infligé des Dégâts Élémentaires. Lorsque vous gagnez Brillance, cela réinitialise le Temps de Recharge du Météore.*Tant que Brillance est active, lancer le Météore lui permettra d'infliger @ de £.#Lorsque Brillance se termine, si vous n'avez pas lancé le Météore, il gagne une Charge de Météore, jusqu'à @, augmentant le nombre de Météores du prochain lancer.*act:241*act:242/\n|*Chaque fois que vous gagnez une Charge de Météore, vous avez @ de £ de gagner Brillance pour Météore Astral pour les @ prochaines secondes.*act:243/\n|*Lors du lancement de Météore Brutal ou Élémentaire, si le précédent lancement de Météore Brutal ou Élémentaire est identique, vous perdez toutes vos Charges de Météore.",
    "FR_LORE": "\"N'oubliez pas de regarder les étoiles...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 54,
    "SPRITE": 100,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "10|2|10|2|1|3|\n10|2",
    "VALUE_TYPE": "%||%||%||\n%|",
    "VALUE_LEVEL": "||||rl||\n|",
    "VALUE_STAT": "chance|chance|increased_damage|\nchance",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 9,
    "BASE_DMG_MAX": 15,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 2,
    "MAX_LVL": 40,
    "EVOLVE_IN": 56,
    "100": "876 - 1860"
  },
  {
    "REF": 56,
    "EN_NAME": "$ of Infinite Time and Deep Space",
    "EN_DESC": "|*Your Meteors can now gain up to @ additional Meteor Charges.",
    "EN_LORE": "\"...and not down at your feet. Unless you're near a hole.\"",
    "EN": "$ of Infinite Time and Deep Space//|*Your Meteors can now gain up to @ additional Meteor Charges.//\"...and not down at your feet. Unless you're near a hole.\"",
    "FR": "$ du Temps Infini et de l'Espace Profond//|*Vos Météores peuvent désormais gagner jusqu'à @ Charges de Météore supplémentaires.//\"...Et non vos pieds. Sauf si vous êtes près d'un trou.\"",
    "FR_NAME": "$ du Temps Infini et de l'Espace Profond",
    "FR_DESC": "|*Vos Météores peuvent désormais gagner jusqu'à @ Charges de Météore supplémentaires.",
    "FR_LORE": "\"...Et non vos pieds. Sauf si vous êtes près d'un trou.\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 55,
    "SPRITE": 101,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "7",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 9,
    "BASE_DMG_MAX": 15,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "876 - 2098"
  },
  {
    "REF": 57,
    "EN_NAME": "$ of Inner Fireworks",
    "EN_DESC": "*+@ £|*Fireworks deal @ Increased Damage.*Your Primary and Secondary Skills apply Inner Weakness to enemies hit, increasing damage received from Inner Fires by @.#Inner Weakness can stack up to @ times.*Inner Fires have @ Chance to apply Inner Weakness.*Inner Fires have @ Chance to trigger Fireworks when dealing damage to enemies affected by Inner Weakness. Chance to trigger Firework is increased by @ for each yard between you and the enemy.*Fireworks deal _ £ to enemies within a ~ yard radius./\n*+@ £|*Fireworks apply Inner Weakness to enemies hit.*While your Primary and Secondary Skill are on Cooldown, Inner Fires deal @ £./\n|*The Cooldown of Fast Skills is increased by @.",
    "EN_LORE": "To give your enemies a beautiful show before you slaughter them.",
    "EN": "$ of Inner Fireworks//*+@ £|*Fireworks deal @ Increased Damage.*Your Primary and Secondary Skills apply Inner Weakness to enemies hit, increasing damage received from Inner Fires by @.#Inner Weakness can stack up to @ times.*Inner Fires have @ Chance to apply Inner Weakness.*Inner Fires have @ Chance to trigger Fireworks when dealing damage to enemies affected by Inner Weakness. Chance to trigger Firework is increased by @ for each yard between you and the enemy.*Fireworks deal _ £ to enemies within a ~ yard radius./\n*+@ £|*Fireworks apply Inner Weakness to enemies hit.*While your Primary and Secondary Skill are on Cooldown, Inner Fires deal @ £./\n|*The Cooldown of Fast Skills is increased by @.//To give your enemies a beautiful show before you slaughter them.",
    "FR": "$ de Feux d'Artifice Intérieurs//*+@ £|*Feu d'Artifice inflige @ de Dégâts Supplémentaires.*Vos Compétences Primaires et Secondaires appliquent Faiblesse Intérieure aux ennemis touchés, augmentant les Dégâts reçus de Feux Intérieurs de @.#Faiblesse Intérieure peut se cumuler jusqu'à @ fois.*Vos Feux Intérieurs ont @ de Chance d'appliquer Faiblesse Intérieure.*Vos Feux Intérieurs ont @ de Chance de déclencher un Feu d'Artifice lorsqu'ils infligent des dégâts aux ennemis affectés par Faiblesse Intérieure. Cette Chance de déclencher un Feu d'Artifice est augmentée de @ pour chaque mètre entre vous et l'ennemi touché.*Feu d'Artifice inflige _ £ aux ennemis dans un rayon de ~ mètres./\n*+@ £|*Feu d'Artifice applique Faiblesse Intérieure aux ennemis touchés.*Tant que vos Compétences Primaires et Secondaires sont en Recharge, les Feux Intérieurs infligent @ £./\n|*Le Temps de Recharge des Attaques Rapides est augmenté de @.//Offrez un spectacle magnifique à vos victimes avant de les occire.",
    "FR_NAME": "$ de Feux d'Artifice Intérieurs",
    "FR_DESC": "*+@ £|*Feu d'Artifice inflige @ de Dégâts Supplémentaires.*Vos Compétences Primaires et Secondaires appliquent Faiblesse Intérieure aux ennemis touchés, augmentant les Dégâts reçus de Feux Intérieurs de @.#Faiblesse Intérieure peut se cumuler jusqu'à @ fois.*Vos Feux Intérieurs ont @ de Chance d'appliquer Faiblesse Intérieure.*Vos Feux Intérieurs ont @ de Chance de déclencher un Feu d'Artifice lorsqu'ils infligent des dégâts aux ennemis affectés par Faiblesse Intérieure. Cette Chance de déclencher un Feu d'Artifice est augmentée de @ pour chaque mètre entre vous et l'ennemi touché.*Feu d'Artifice inflige _ £ aux ennemis dans un rayon de ~ mètres./\n*+@ £|*Feu d'Artifice applique Faiblesse Intérieure aux ennemis touchés.*Tant que vos Compétences Primaires et Secondaires sont en Recharge, les Feux Intérieurs infligent @ £./\n|*Le Temps de Recharge des Attaques Rapides est augmenté de @.",
    "FR_LORE": "Offrez un spectacle magnifique à vos victimes avant de les occire.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 57,
    "SPRITE": 57,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "15|0.5|0.5|10|10|5|5|\n85|1|\n100",
    "VALUE_TYPE": "%|%|%||%|%|%|\n%|%|\n%",
    "VALUE_LEVEL": "|rl|rl|||||\n|rl|\n",
    "VALUE_STAT": "inner_fire_chance_percent|elemental_damage|\ninner_fire_chance_percent|increased_damage",
    "VALUE_REAL": "damage:elemental_damage:100",
    "BASE_DMG_MIN": 22,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 44,
    "EVOLVE_IN": 58,
    "100": "1148 - 2175"
  },
  {
    "REF": 58,
    "EN_NAME": "Supercharged $ of Inner Fireworks",
    "EN_DESC": "|*The size of Fireworks is increased by @ for each stack of Inner Weakness of the enemy hit.*Fireworks Blinds enemies hit for the next @ seconds.",
    "EN_LORE": "Protect your eyes before use.",
    "EN": "Supercharged $ of Inner Fireworks//|*The size of Fireworks is increased by @ for each stack of Inner Weakness of the enemy hit.*Fireworks Blinds enemies hit for the next @ seconds.//Protect your eyes before use.",
    "FR": "$ Surchargée de Feux d'Artifice Intérieurs//|*La taille de Feu d'Artifice est augmentée de @ pour chaque cumul de Feu Intérieur sur l'ennemi touché.*Feu d'Artifice Aveugle les ennemis touchés pour les @ prochaines secondes.//Attention les yeux !",
    "FR_NAME": "$ Surchargée de Feux d'Artifice Intérieurs",
    "FR_DESC": "|*La taille de Feu d'Artifice est augmentée de @ pour chaque cumul de Feu Intérieur sur l'ennemi touché.*Feu d'Artifice Aveugle les ennemis touchés pour les @ prochaines secondes.",
    "FR_LORE": "Attention les yeux !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 58,
    "SPRITE": 58,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "10|5",
    "VALUE_TYPE": "%|",
    "VALUE_LEVEL": "|",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 22,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 8,
    "MAX_DMG_LVL": 14,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1486 - 2514"
  },
  {
    "REF": 59,
    "EN_NAME": "$ of the Spider Nest",
    "EN_DESC": "*+@ £*+@ £#*+@ £*+@ £|*Whenever you cast a Projectile Skill with your Primary or Secondary Skill, you have a @ £ to trigger a Web Projectile in the opposite direction.#The Web Projectile deals _ £ to enemies hit, has a @ £ and a @ £.*Projectiles now Rebound from the Edges of the World instead of Walls.*Projectiles with more than @ Pierce Chance have a Chance equal to the Percent above @ to trigger an Entangling Web, dealing _ £ to enemies within a ~ yard radius, and Rooting them for the next @ seconds./\n|*When a Web Projectile hits the Edge of the World, it has a Chance equal to the Percent of your Rebound Chance above @ to trigger @ Filaments, dealing _ £ to enemies hit, and Rooting them for the next @ seconds. Whenever Web Filament triggers, it has up to +@ £./\n|*Projectiles can no longer Fork.",
    "EN_LORE": "Smaloron was inspired by a creature he met near the Prison of Adreart...",
    "EN": "$ of the Spider Nest//*+@ £*+@ £#*+@ £*+@ £|*Whenever you cast a Projectile Skill with your Primary or Secondary Skill, you have a @ £ to trigger a Web Projectile in the opposite direction.#The Web Projectile deals _ £ to enemies hit, has a @ £ and a @ £.*Projectiles now Rebound from the Edges of the World instead of Walls.*Projectiles with more than @ Pierce Chance have a Chance equal to the Percent above @ to trigger an Entangling Web, dealing _ £ to enemies within a ~ yard radius, and Rooting them for the next @ seconds./\n|*When a Web Projectile hits the Edge of the World, it has a Chance equal to the Percent of your Rebound Chance above @ to trigger @ Filaments, dealing _ £ to enemies hit, and Rooting them for the next @ seconds. Whenever Web Filament triggers, it has up to +@ £./\n|*Projectiles can no longer Fork.//Smaloron was inspired by a creature he met near the Prison of Adreart...",
    "FR": "$ du Nid d'Araignée//*+@ £*+@ £#*+@ £*+@ £|*Chaque fois que vous lancez une Compétence de Projectile Primaire ou Secondaire, vous avez @ de £ de déclencher un Projectile Toile dans la direction opposée.#Le Projectile Toile inflige _ de £ aux ennemis touchés, a @ de £ et @ de £.*Les Projectiles rebondissent désormais depuis les Bords du Monde au lieu des murs.*Les Projectiles avec plus de @ de Chance de Percer ont une Chance équivalente au pourcentage au-dessus de @ de déclencher une Toile Enchevêtrante, infligeant _ de £ aux ennemis dans un rayon d'environ ~ mètres, et les Enracinant pour les @ prochaines secondes./\n|*Lorsqu'un Projectile Toile atteint le Bord du Monde, il a une chance égale au pourcentage de votre Chance de Rebond au-dessus de @ de déclencher @ Filaments, infligeant _ de £ aux ennemis touchés, et les Enracinant pour les @ prochaines secondes. Chaque fois que Filaments est déclenché, il peut lancer jusqu'à +@ £./\n|*Les Projectiles ne peuvent plus se Fourcher.//Smaloron se serait inspiré d'une créature rencontrée dans la réserve du chateau d'Adreart...",
    "FR_NAME": "$ du Nid d'Araignée",
    "FR_DESC": "*+@ £*+@ £#*+@ £*+@ £|*Chaque fois que vous lancez une Compétence de Projectile Primaire ou Secondaire, vous avez @ de £ de déclencher un Projectile Toile dans la direction opposée.#Le Projectile Toile inflige _ de £ aux ennemis touchés, a @ de £ et @ de £.*Les Projectiles rebondissent désormais depuis les Bords du Monde au lieu des murs.*Les Projectiles avec plus de @ de Chance de Percer ont une Chance équivalente au pourcentage au-dessus de @ de déclencher une Toile Enchevêtrante, infligeant _ de £ aux ennemis dans un rayon d'environ ~ mètres, et les Enracinant pour les @ prochaines secondes./\n|*Lorsqu'un Projectile Toile atteint le Bord du Monde, il a une chance égale au pourcentage de votre Chance de Rebond au-dessus de @ de déclencher @ Filaments, infligeant _ de £ aux ennemis touchés, et les Enracinant pour les @ prochaines secondes. Chaque fois que Filaments est déclenché, il peut lancer jusqu'à +@ £./\n|*Les Projectiles ne peuvent plus se Fourcher.",
    "FR_LORE": "Smaloron se serait inspiré d'une créature rencontrée dans la réserve du chateau d'Adreart...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 59,
    "SPRITE": 47,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "5|5|1|1|1|200|100|100|100|4|\n100|7|4|0.1",
    "VALUE_TYPE": "%|%|%|%|%|%|%|%|%||\n%|||",
    "VALUE_LEVEL": "||rl|rl|rl||||||\n|||rl",
    "VALUE_STAT": "chance_to_pierce_percent|rebound_chance_percent|chance_to_pierce_global_mult|rebound_chance_global_mult|chance|physical_damage|chance_to_pierce_percent|rebound_chance_percent|physical_damage|\nphysical_damage|additional_projectile_add",
    "VALUE_REAL": "damage:physical_damage:100|damage:physical_damage:70|damage:physical_damage:100",
    "BASE_DMG_MIN": 24,
    "BASE_DMG_MAX": 36,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "986 - 1859"
  },
  {
    "REF": 60,
    "EN_NAME": "$ of the Sharpshooter",
    "EN_DESC": "*With Projectile Skills:*+@ £*+@ £*+@ £|*When Idle for at least @ seconds:#You gain <Steady Feet>, granting you +@ £.#<Steady Feet> disappears when you move./\n|*When Idle for at least @ seconds:#+@ £/\n|*Without <Steady Feet>:#You can no longer fire Additional Projectiles.",
    "EN_LORE": "Vilinis the Loner could shoot so many arrows at once all around him that it had become dangerous to fight by his side.\" #- Champions of Arah, Heroes or Imposters, by Adam Nostrus",
    "EN": "$ of the Sharpshooter//*With Projectile Skills:*+@ £*+@ £*+@ £|*When Idle for at least @ seconds:#You gain <Steady Feet>, granting you +@ £.#<Steady Feet> disappears when you move./\n|*When Idle for at least @ seconds:#+@ £/\n|*Without <Steady Feet>:#You can no longer fire Additional Projectiles.//Vilinis the Loner could shoot so many arrows at once all around him that it had become dangerous to fight by his side.\" #- Champions of Arah, Heroes or Imposters, by Adam Nostrus",
    "FR": "$ du Tireur d'Élite//*Avec vos Compétences de Projectiles :*+@ £*+@ £*+@ £|*Lorsque vous restez Immobile pour au moins @ secondes :#Vous gagnez <Pied Assuré>, vous conférant +@ de £.#<Pied Assuré> disparaît lorsque vous vous déplacez./\n|*Lorsque vous restez Immobile pour au moins @ secondes :#+@ £/\n|*Sans <Pied Assuré>:#Vous ne pouvez lancer de Projectiles Additionnels.//\"Vilinis le Solitaire pouvait tirer tellement de flèches à la fois tout autour de lui qu'il était devenu dangereux de se battre à ses côtés.\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "FR_NAME": "$ du Tireur d'Élite",
    "FR_DESC": "*Avec vos Compétences de Projectiles :*+@ £*+@ £*+@ £|*Lorsque vous restez Immobile pour au moins @ secondes :#Vous gagnez <Pied Assuré>, vous conférant +@ de £.#<Pied Assuré> disparaît lorsque vous vous déplacez./\n|*Lorsque vous restez Immobile pour au moins @ secondes :#+@ £/\n|*Sans <Pied Assuré>:#Vous ne pouvez lancer de Projectiles Additionnels.",
    "FR_LORE": "\"Vilinis le Solitaire pouvait tirer tellement de flèches à la fois tout autour de lui qu'il était devenu dangereux de se battre à ses côtés.\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "CH": "神射手之$//*远程技能：*+@£*+@£*+@£|*站立不动@秒后：#获得<Steady Feet>，为你赋予+@£。#<Steady Feet>在移动时消失。/\n|*站立不动@秒后：#+@£/\n|*没有<Steady Feet>时：#无法发射额外投射物。//“孤独者”威利尼斯可以一次在身边射出许多箭，和他并肩作战都会有危险。”#- 《亚拉勇者，英雄还是小人》，亚当·诺斯图斯著",
    "CH_NAME": "神射手之$",
    "CH_DESCRIPTION": "*远程技能：*+@£*+@£*+@£|*站立不动@秒后：#获得<Steady Feet>，为你赋予+@£。#<Steady Feet>在移动时消失。/\n|*站立不动@秒后：#+@£/\n|*没有<Steady Feet>时：#无法发射额外投射物。",
    "CH_LORE": "“孤独者”威利尼斯可以一次在身边射出许多箭，和他并肩作战都会有危险。”#- 《亚拉勇者，英雄还是小人》，亚当·诺斯图斯著",
    "TW": "神射手之$//*遠程技能：*+@£*+@£*+@£|*站立不動@秒後：#獲得<Steady Feet>，為你賦予+@£。 #<Steady Feet>在移動時消失。 /\n|*站立不動@秒後：#+@£/\n|*沒有<Steady Feet>時：#無法發射額外投射物。//“孤獨者”威利尼斯可以一次在身邊射出許多箭，和他並肩作戰都會有危險。 ”#- 《亞拉勇者，英雄還是小人》，亞當·諾斯圖斯著",
    "TW_NAME": "神射手之$",
    "TW_DESCRIPTION": "*遠程技能：*+@£*+@£*+@£|*站立不動@秒後：#獲得<Steady Feet>，為你賦予+@£。 #<Steady Feet>在移動時消失。 /\n|*站立不動@秒後：#+@£/\n|*沒有<Steady Feet>時：#無法發射額外投射物。",
    "TW_LORE": "“孤獨者”威利尼斯可以一次在身邊射出許多箭，和他並肩作戰都會有危險。 ”#- 《亞拉勇者，英雄還是小人》，亞當·諾斯圖斯著",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 60,
    "SPRITE": 60,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.5|0.5|0.5|3|\n0.5|0.5",
    "VALUE_TYPE": "%|%|%|||\n|%",
    "VALUE_LEVEL": "rl|rl|rl|||\n|rl\n",
    "VALUE_STAT": "weapon_damage_mult|armor_penetration|elemental_penetration|additional_projectile_add|\nadditional_projectile_global_mult",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 28,
    "BASE_DMG_MAX": 35,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "994 - 1688"
  },
  {
    "REF": 61,
    "EN_NAME": "Gold-Plated $",
    "EN_DESC": "*+@ £|*You have +_ £.#Max Life is equal to your $.*act:200/\n|*Your Primary and Secondary Skills deal _ Additional Damage.#Additional Damage is equal to @ of your $./\n|*Without Gold Armor:# Incoming Damage is increased by @.",
    "EN_LORE": "Beighart wrought this Reaper using the Gloves of Midos, creating a very unique weapon.",
    "EN": "Gold-Plated $//*+@ £|*You have +_ £.#Max Life is equal to your $.*act:200/\n|*Your Primary and Secondary Skills deal _ Additional Damage.#Additional Damage is equal to @ of your $./\n|*Without Gold Armor:# Incoming Damage is increased by @.//Beighart wrought this Reaper using the Gloves of Midos, creating a very unique weapon.",
    "FR": "$ en Or-Plaqué//*+@ £|*Vous avez +_ de £.#Cette Vie Max est équivalente à votre $.*act:200/\n|*Vos Compétences Primaires et Secondaires infligent _ Dégâts Additionnels.#Ces Dégâts Additionnels sont équivalents à @ de vos $./\n|*Sans Armure Dorée:#Les Dégâts reçus sont augmentés de @.//Beighart a forgé ce Reaper en utilisant les Gants de Midos, créant ainsi une arme tout à fait unique.",
    "FR_NAME": "$ en Or-Plaqué",
    "FR_DESC": "*+@ £|*Vous avez +_ de £.#Cette Vie Max est équivalente à votre $.*act:200/\n|*Vos Compétences Primaires et Secondaires infligent _ Dégâts Additionnels.#Ces Dégâts Additionnels sont équivalents à @ de vos $./\n|*Sans Armure Dorée:#Les Dégâts reçus sont augmentés de @.",
    "FR_LORE": "Beighart a forgé ce Reaper en utilisant les Gants de Midos, créant ainsi une arme tout à fait unique.",
    "CH": "黄金板甲$//*+@£|*你获得+_£。#最大生命值等于你的$。*act:200/\n|*主技能或副技能造成_额外伤害。#额外伤害等于你的$的@%。/\n|*没有黄金甲时：#受到的伤害增加@。//贝哈特用米多斯手套锻造了这把武器，使其显得格外特别。",
    "CH_NAME": "黄金板甲$",
    "CH_DESCRIPTION": "*+@£|*你获得+_£。#最大生命值等于你的$。*act:200/\n|*主技能或副技能造成_额外伤害。#额外伤害等于你的$的@%。/\n|*没有黄金甲时：#受到的伤害增加@。",
    "CH_LORE": "贝哈特用米多斯手套锻造了这把武器，使其显得格外特别。",
    "TW": "黃金板甲$//*+@£|*你獲得+_£。 #最大生命值等於你的$。 *act:200/\n|*主技能或副技能造成_額外傷害。 #額外傷害等於你的$的@%。 /\n|*沒有黃金甲時：#受到的傷害增加@。//貝哈特用米多斯手套鍛造了這把武器，使其顯得格外特別。",
    "TW_NAME": "黃金板甲$",
    "TW_DESCRIPTION": "*+@£|*你獲得+_£。 #最大生命值等於你的$。 *act:200/\n|*主技能或副技能造成_額外傷害。 #額外傷害等於你的$的@%。 /\n|*沒有黃金甲時：#受到的傷害增加@。",
    "TW_LORE": "貝哈特用米多斯手套鍛造了這把武器，使其顯得格外特別。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 61,
    "SPRITE": 61,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "6|\n1|\n100",
    "VALUE_TYPE": "|\n%|\n%",
    "VALUE_LEVEL": "rl|\nrl|",
    "VALUE_STAT": "thorns_add|the_max_health_add",
    "VALUE_REAL": "synergy:thorns:100|\nsynergy:thorns:1*rl",
    "BASE_DMG_MIN": 24,
    "BASE_DMG_MAX": 36,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 1,
    "MAX_LVL": 60,
    "EVOLVE_IN": 62,
    "100": "972 - 1845"
  },
  {
    "REF": 62,
    "EN_NAME": "Superior Gold-Plated $",
    "EN_DESC": "|*You have +_% £.#Thorns Damage is equal to your $.*The Cooldown Time of Gold Armor is reduced by @ second every time an enemy deals damage to you.",
    "EN_LORE": "If you have money problems, you can always melt it down...",
    "EN": "Superior Gold-Plated $//|*You have +_% £.#Thorns Damage is equal to your $.*The Cooldown Time of Gold Armor is reduced by @ second every time an enemy deals damage to you.//If you have money problems, you can always melt it down...",
    "FR": "$ Supérieur en Or-Plaqué/$ Supérieure en Or-Plaqué//|*Vous avez +_% de £.#Ces Dégâts d'Épines sont équivalents à votre $.*Le Temps de Recharge d'Armure Dorée est réduit de @ secondes chaque fois qu'un ennemi vous inflige des dégâts.//Si vous avez des problèmes d'argent, vous pouvez toujours le faire fondre...",
    "FR_NAME": "$ Supérieur en Or-Plaqué/$ Supérieure en Or-Plaqué",
    "FR_DESC": "|*Vous avez +_% de £.#Ces Dégâts d'Épines sont équivalents à votre $.*Le Temps de Recharge d'Armure Dorée est réduit de @ secondes chaque fois qu'un ennemi vous inflige des dégâts.",
    "FR_LORE": "Si vous avez des problèmes d'argent, vous pouvez toujours le faire fondre...",
    "CH": "超级黄金板甲$//|*你获得+_%£。#荆棘伤害等于你的$。*每当敌人对你造成伤害，黄金甲的冷却时间减少@。//如果你有经济困难，随时都可以把它融了……",
    "CH_NAME": "超级黄金板甲$",
    "CH_DESCRIPTION": "|*你获得+_%£。#荆棘伤害等于你的$。*每当敌人对你造成伤害，黄金甲的冷却时间减少@。",
    "CH_LORE": "如果你有经济困难，随时都可以把它融了……",
    "TW": "超級黃金板甲$//|*你獲得+_%£。 #荊棘傷害等於你的$。 *每當敵人對你造成傷害，黃金甲的冷卻時間減少@。//如果你有經濟困難，隨時都可以把它融了……",
    "TW_NAME": "超級黃金板甲$",
    "TW_DESCRIPTION": "|*你獲得+_%£。 #荊棘傷害等於你的$。 *每當敵人對你造成傷害，黃金甲的冷卻時間減少@。",
    "TW_LORE": "如果你有經濟困難，隨時都可以把它融了……",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 62,
    "SPRITE": 62,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "thorns_percent",
    "VALUE_REAL": "synergy:retaliate:1",
    "BASE_DMG_MIN": 30,
    "BASE_DMG_MAX": 42,
    "MIN_DMG_LVL": 12,
    "MAX_DMG_LVL": 18,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1837 - 2711"
  },
  {
    "REF": 63,
    "EN_NAME": "$ of Resonating Thunder",
    "EN_DESC": "*+@ £|*Whenever you deal damage to an enemy with a Recast, you gain a Recaster stack, up to @.*Recast Skills deal @ £ for each Recaster stack you have.*Whenever you cast a Melee Skill, you gain a Caster stack, up to @.*act:244/\n|*+@ £ with Melee Skills.*+@ £ with Melee Skills.*act:245/\n|*Your Critical Strike Chance is set to _%.#Critical Strike Chance is equal to _% of its value.*Your Ancestral Strike Chance is set to _%. Ancestral Strike Chance is equal to _% of its value.*The Percent Base Value is equal to your Recast Chance.",
    "EN_LORE": "Repetition makes Reputation.",
    "EN": "$ of Resonating Thunder//*+@ £|*Whenever you deal damage to an enemy with a Recast, you gain a Recaster stack, up to @.*Recast Skills deal @ £ for each Recaster stack you have.*Whenever you cast a Melee Skill, you gain a Caster stack, up to @.*act:244/\n|*+@ £ with Melee Skills.*+@ £ with Melee Skills.*act:245/\n|*Your Critical Strike Chance is set to _%.#Critical Strike Chance is equal to _% of its value.*Your Ancestral Strike Chance is set to _%. Ancestral Strike Chance is equal to _% of its value.*The Percent Base Value is equal to your Recast Chance.//Repetition makes Reputation.",
    "FR": "$ de Tempête Résonante//*+@ £|*Chaque fois que vous infligez des dégâts à un ennemi avec une Relance, vous gagnez un cumul de Relanceur, jusqu'à @.*Les Compétences Relancées infligent @ de £ pour chaque cumul de Relanceur que vous avez.*Chaque fois que vous lancez une Compétence de Mêlée, vous gagnez un cumul de Lanceur, jusqu'à @.*act:244/\n|*+@ de £ avec les Compétences de Mêlée.*+@ de £ avec les Compétences de Mêlée.*act:245/\n|*Votre Chance de Coup Critique devient _%.#La Chance de Coup Critique est équivalente à _% de sa valeur.*Votre Chance de Coup Ancestral devient _%.#La Chance de Coup Ancestral est équivalente à _% de sa valeur.*Cette Valeur de Base en Pourcentage est égale à votre Chance de Relance.//La Répétition fait la Réputation.",
    "FR_NAME": "$ de Tempête Résonante",
    "FR_DESC": "*+@ £|*Chaque fois que vous infligez des dégâts à un ennemi avec une Relance, vous gagnez un cumul de Relanceur, jusqu'à @.*Les Compétences Relancées infligent @ de £ pour chaque cumul de Relanceur que vous avez.*Chaque fois que vous lancez une Compétence de Mêlée, vous gagnez un cumul de Lanceur, jusqu'à @.*act:244/\n|*+@ de £ avec les Compétences de Mêlée.*+@ de £ avec les Compétences de Mêlée.*act:245/\n|*Votre Chance de Coup Critique devient _%.#La Chance de Coup Critique est équivalente à _% de sa valeur.*Votre Chance de Coup Ancestral devient _%.#La Chance de Coup Ancestral est équivalente à _% de sa valeur.*Cette Valeur de Base en Pourcentage est égale à votre Chance de Relance.",
    "FR_LORE": "La Répétition fait la Réputation.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 63,
    "SPRITE": 96,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.2|1|1|50|\n0.3|0.3",
    "VALUE_TYPE": "%||%||\n%|%",
    "VALUE_LEVEL": "rl|rl|||\nrl|rl",
    "VALUE_STAT": "recast_chance_percent|increased_damage|\ncrit_chance_global_mult|brut_chance_global_mult",
    "VALUE_REAL": "synergy:crit_base_recast:1|synergy:recast_chance:1|synergy:brutal_base_recast:1|synergy:recast_chance:1",
    "BASE_DMG_MIN": 28,
    "BASE_DMG_MAX": 41,
    "MIN_DMG_LVL": 8,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 52,
    "EVOLVE_IN": 64,
    "100": "1497 - 2040"
  },
  {
    "REF": 64,
    "EN_NAME": "Meticulous $ of Resonating Thunder",
    "EN_DESC": "|*Recast Skills have a _% £ to Recast a second time then a _% £ to Recast a third time.#The Chance to Recast again is equal to their Critical Strike Chance.#The Chance to Recast a third time is equal to their Ancestral Strike Chance.",
    "EN_LORE": "Repetition makes Reputation.",
    "EN": "Meticulous $ of Resonating Thunder//|*Recast Skills have a _% £ to Recast a second time then a _% £ to Recast a third time.#The Chance to Recast again is equal to their Critical Strike Chance.#The Chance to Recast a third time is equal to their Ancestral Strike Chance.//Repetition makes Reputation.",
    "FR": "$ Méticuleux de Tempête Résonante/$ Méticuleuse de Tempête Résonante//|*Les Compétences Relancées ont _% de £ de Relancer une deuxième fois, puis _% de £ de Relancer une troisième fois.#La Chance de Relance supplémentaire est équivalente à leur Chance de Coup Critique.#La troisième Chance de Renvoi est équivalente à leur Chance de Coup Ancestral.//La Répétition fait la Réputation.",
    "FR_NAME": "$ Méticuleux de Tempête Résonante/$ Méticuleuse de Tempête Résonante",
    "FR_DESC": "|*Les Compétences Relancées ont _% de £ de Relancer une deuxième fois, puis _% de £ de Relancer une troisième fois.#La Chance de Relance supplémentaire est équivalente à leur Chance de Coup Critique.#La troisième Chance de Renvoi est équivalente à leur Chance de Coup Ancestral.",
    "FR_LORE": "La Répétition fait la Réputation.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 64,
    "SPRITE": 97,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "chance|chance",
    "VALUE_REAL": "synergy:crit_chance_melee:1|synergy:ancestral_chance_melee:1",
    "BASE_DMG_MIN": 28,
    "BASE_DMG_MAX": 41,
    "MIN_DMG_LVL": 9,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1667 - 2206"
  },
  {
    "REF": 65,
    "EN_NAME": "$ of Vindictive Slam",
    "EN_DESC": "*+@ £*+@ £|*Whenever you hit an enemy with your Primary or Secondary Skill, you have _% Chance to trigger a Vindictive Slam, dealing _ £ to enemies within a ~ yard radius.#Chance to trigger is equal to @ of your $./\n|*Reaper Damage is added to Elemental Damage instead of Skill Damage.*Vindictive Slam creates a Holy Ground on the floor that lasts @ seconds, dealing _ £ every second to enemies inside./\n|*Your Raw Damage is reduced to 0.",
    "EN_LORE": "Beware: Falling Objects !",
    "EN": "$ of Vindictive Slam//*+@ £*+@ £|*Whenever you hit an enemy with your Primary or Secondary Skill, you have _% Chance to trigger a Vindictive Slam, dealing _ £ to enemies within a ~ yard radius.#Chance to trigger is equal to @ of your $./\n|*Reaper Damage is added to Elemental Damage instead of Skill Damage.*Vindictive Slam creates a Holy Ground on the floor that lasts @ seconds, dealing _ £ every second to enemies inside./\n|*Your Raw Damage is reduced to 0.//Beware: Falling Objects !",
    "FR": "$ de Secousse Vindicative//*+@ £*+@ £|*Lorsque vous infligez des dégâts avec votre Compétence Primaire ou Secondaire, vous avez _% de Chance de déclencher une Secousse Vindicative, infligeant _ de £ aux ennemis dans un rayon de ~.#Cette Chance est équivalente à @ de votre $./\n|*Les Dégats de Reaper sont ajoutés aux Dégâts Élémentaires au lieu d'être ajoutés aux Dégats Physiques.*Secousse Vindicative crée un Sol Sacré au sol qui dure @ secondes, infligeant _ de £ chaque seconde aux ennemis à l'intérieur./\n|*Vos Dégats Bruts sont réduits à 0.//Attention : Chute d'objets !\n",
    "FR_NAME": "$ de Secousse Vindicative",
    "FR_DESC": "*+@ £*+@ £|*Lorsque vous infligez des dégâts avec votre Compétence Primaire ou Secondaire, vous avez _% de Chance de déclencher une Secousse Vindicative, infligeant _ de £ aux ennemis dans un rayon de ~.#Cette Chance est équivalente à @ de votre $./\n|*Les Dégats de Reaper sont ajoutés aux Dégâts Élémentaires au lieu d'être ajoutés aux Dégats Physiques.*Secousse Vindicative crée un Sol Sacré au sol qui dure @ secondes, infligeant _ de £ chaque seconde aux ennemis à l'intérieur./\n|*Vos Dégats Bruts sont réduits à 0.",
    "FR_LORE": "Attention : Chute d'objets !\n",
    "CH": "报复之掌$//*+@£*+@£|*主技能或副技能击中敌人时，有_%记录触发一次报复之掌，对~码半径内的敌人造成_£。#触发记录等于你的$的@。/\n|*武器伤害被加算到元素伤害中，而非技能伤害中。*报复之掌在地面上制造一块神圣大地，持续@秒，每秒对其中的敌人造成_£。/\n|*原始伤害减少为0。//小心：高空坠物！",
    "CH_NAME": "报复之掌$",
    "CH_DESCRIPTION": "*+@£*+@£|*主技能或副技能击中敌人时，有_%记录触发一次报复之掌，对~码半径内的敌人造成_£。#触发记录等于你的$的@。/\n|*武器伤害被加算到元素伤害中，而非技能伤害中。*报复之掌在地面上制造一块神圣大地，持续@秒，每秒对其中的敌人造成_£。/\n|*原始伤害减少为0。",
    "CH_LORE": "小心：高空坠物！",
    "TW": "報復之掌$//*+@£*+@£|*主技能或副技能擊中敵人時，有_%記錄觸發一次報復之掌，對~碼半徑內的敵人造成_£。 #觸發記錄等於你的$的@。 /\n|*武器傷害被加算到元素傷害中，而非技能傷害中。 *報復之掌在地面上製造一塊神聖大地，持續@秒，每秒對其中的敵人造成_£。 /\n|*原始傷害減少為0。//小心：高空墜物！",
    "TW_NAME": "報復之掌$",
    "TW_DESCRIPTION": "*+@£*+@£|*主技能或副技能擊中敵人時，有_%記錄觸發一次報復之掌，對~碼半徑內的敵人造成_£。 #觸發記錄等於你的$的@。 /\n|*武器傷害被加算到元素傷害中，而非技能傷害中。 *報復之掌在地面上製造一塊神聖大地，持續@秒，每秒對其中的敵人造成_£。 /\n|*原始傷害減少為0。",
    "TW_LORE": "小心：高空墜物！",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 66,
    "SPRITE": 65,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|15|0.1|\n6|40",
    "VALUE_TYPE": "||%|\n||",
    "VALUE_LEVEL": "rl|rl||\n||",
    "VALUE_STAT": "health_regen_add|the_max_health_add|elemental_damage|\nelemental_damage",
    "VALUE_REAL": "synergy:max_health:0.2|damage:elemental_damage:3*rl|\ndamage:elemental_damage:40",
    "BASE_DMG_MIN": 17,
    "BASE_DMG_MAX": 21,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 24,
    "EVOLVE_IN": 66,
    "100": "634 - 1145"
  },
  {
    "REF": 66,
    "EN_NAME": "Greater $ of the Vindictive Slam",
    "EN_DESC": "|*Vindictive Slam has _% Increased Size.#Increased Size is equal to @ of your Armor.*Vindictive Slam deals _% Increased Damage.#Increased Damage is equal to @ of your Elemental Resistance.",
    "EN_LORE": "Safety Helmets must be worn at all times.",
    "EN": "Greater $ of the Vindictive Slam//|*Vindictive Slam has _% Increased Size.#Increased Size is equal to @ of your Armor.*Vindictive Slam deals _% Increased Damage.#Increased Damage is equal to @ of your Elemental Resistance.//Safety Helmets must be worn at all times.",
    "FR": "$ Amélioré de Secousse Vindicative/$ Améliorée de Secousse Vindicative//|*La taille de Secousse Vindicative est augmentée de _%.#Cette augmentation est équivalente à @ de votre Armure.*Secousse Vindicative inflige _% de Dégats Supplémentaires.#Ces Dégâts Supplémentaires sont équivalents à @ de votre Résistance Élémentaire.//Port du casque obligatoire !",
    "FR_NAME": "$ Amélioré de Secousse Vindicative/$ Améliorée de Secousse Vindicative",
    "FR_DESC": "|*La taille de Secousse Vindicative est augmentée de _%.#Cette augmentation est équivalente à @ de votre Armure.*Secousse Vindicative inflige _% de Dégats Supplémentaires.#Ces Dégâts Supplémentaires sont équivalents à @ de votre Résistance Élémentaire.",
    "FR_LORE": "Port du casque obligatoire !",
    "CH": "加强版报复之掌$//|*报复之掌的范围增加_%。#增加的范围等于你的护甲的@。*报复之掌的伤害增加_%#增加的伤害等于元素抗性的@。//安全头盔必须全程佩戴。",
    "CH_NAME": "加强版报复之掌$",
    "CH_DESCRIPTION": "|*报复之掌的范围增加_%。#增加的范围等于你的护甲的@。*报复之掌的伤害增加_%#增加的伤害等于元素抗性的@。",
    "CH_LORE": "安全头盔必须全程佩戴。",
    "TW": "加強版報復之掌$//|*報復之掌的範圍增加_%。 #增加的範圍等於你的護甲的@。 *報復之掌的傷害增加_%#增加的傷害等於元素抗性的@。//安全頭盔必須全程佩戴。",
    "TW_NAME": "加強版報復之掌$",
    "TW_DESCRIPTION": "|*報復之掌的範圍增加_%。 #增加的範圍等於你的護甲的@。 *報復之掌的傷害增加_%#增加的傷害等於元素抗性的@。",
    "TW_LORE": "安全頭盔必須全程佩戴。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 67,
    "SPRITE": 66,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.2|0.2",
    "VALUE_TYPE": "%|%",
    "VALUE_LEVEL": "|",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:armor:0.2|synergy:elemental_resist:0.2",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 23,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 40,
    "EVOLVE_IN": 67,
    "100": "971 - 1488"
  },
  {
    "REF": 67,
    "EN_NAME": "Greater $ of the Implacable Vindictive Slam",
    "EN_DESC": "|*Vindictive Slam has _% Chance to Stun enemies hit for @ seconds.#Chance is equal to the sum of your Melee Damage Reduction, Projectile Damage Reduction and Area Damage Reduction.",
    "EN_LORE": "One has the right to wonder where do all these hammers come from...",
    "EN": "Greater $ of the Implacable Vindictive Slam//|*Vindictive Slam has _% Chance to Stun enemies hit for @ seconds.#Chance is equal to the sum of your Melee Damage Reduction, Projectile Damage Reduction and Area Damage Reduction.//One has the right to wonder where do all these hammers come from...",
    "FR": "$ Amélioré d'Implacable Secousse Vindicative/$ Améliorée d'Implacable Secousse Vindicative//|*Secousse Vindication a _% de Chance d'Assommer les ennemis touchés pendant @ secondes.#Cette Chance est équivalente à la somme de votre Réduction de Dégâts en Mêlée, de votre Réduction des Dégâts de Projectile et de votre Réduction des Dégâts de Zone.//On est en droit de se demander d'où viennent tous ces marteaux...",
    "FR_NAME": "$ Amélioré d'Implacable Secousse Vindicative/$ Améliorée d'Implacable Secousse Vindicative",
    "FR_DESC": "|*Secousse Vindication a _% de Chance d'Assommer les ennemis touchés pendant @ secondes.#Cette Chance est équivalente à la somme de votre Réduction de Dégâts en Mêlée, de votre Réduction des Dégâts de Projectile et de votre Réduction des Dégâts de Zone.",
    "FR_LORE": "On est en droit de se demander d'où viennent tous ces marteaux...",
    "CH": "威力加强版报复之掌$//|*报复之掌有_%记录眩晕敌人，持续@秒。#几率等于近战伤害减免、远程伤害减免、范围伤害减免的总和。//大家都有理由猜想，这些锤子是哪儿来的……",
    "CH_NAME": "威力加强版报复之掌$",
    "CH_DESCRIPTION": "|*报复之掌有_%记录眩晕敌人，持续@秒。#几率等于近战伤害减免、远程伤害减免、范围伤害减免的总和。",
    "CH_LORE": "大家都有理由猜想，这些锤子是哪儿来的……",
    "TW": "威力加強版報復之掌$//|*報復之掌有_%記錄眩暈敵人，持續@秒。 #機率等於近戰傷害減免、遠程傷害減免、範圍傷害減免的總和。//大家都有理由猜想，這些錘子是哪兒來的……",
    "TW_NAME": "威力加強版報復之掌$",
    "TW_DESCRIPTION": "|*報復之掌有_%記錄眩暈敵人，持續@秒。 #機率等於近戰傷害減免、遠程傷害減免、範圍傷害減免的總和。",
    "TW_LORE": "大家都有理由猜想，這些錘子是哪兒來的……",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 68,
    "SPRITE": 67,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1.5",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "synergy:sum_reduced_resistances:1",
    "BASE_DMG_MIN": 20,
    "BASE_DMG_MAX": 24,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1143 - 1659"
  },
  {
    "REF": 68,
    "EN_NAME": "$ of Benediction",
    "EN_DESC": "*+@ £*+@ £|*Whenever you should restore anyone's Life, Life restored is increased by @.*Whenever you deal damage to an enemy with your Primary or Secondary Skill, you have a @ £ to reset the Cooldown Time of all the Ancestral Skills granted by this Slorm Reaper.*Whenever you restore Life to an enemy affected by Necrotic Curse, it spread to another nearby enemy.*act:246*act:247*act:248*act:249/\n|*You now continuously apply Necrotic Curse to all enemies within a @ yard radius./\n|*You can no longer restore Life with Life on Hit, Life on Kill and Life Leech.",
    "EN_LORE": "\"Each strike carries hope and healing...\"",
    "EN": "$ of Benediction//*+@ £*+@ £|*Whenever you should restore anyone's Life, Life restored is increased by @.*Whenever you deal damage to an enemy with your Primary or Secondary Skill, you have a @ £ to reset the Cooldown Time of all the Ancestral Skills granted by this Slorm Reaper.*Whenever you restore Life to an enemy affected by Necrotic Curse, it spread to another nearby enemy.*act:246*act:247*act:248*act:249/\n|*You now continuously apply Necrotic Curse to all enemies within a @ yard radius./\n|*You can no longer restore Life with Life on Hit, Life on Kill and Life Leech.//\"Each strike carries hope and healing...\"",
    "FR": "$ de Bénédiction//*+@ £*+@ £|*Chaque fois que vous devriez restaurer la Vie de quelqu'un, la Vie restaurée est augmentée de @.*Chaque fois que vous infligez des dégâts avec votre Compétence Primaire ou Secondaire, vous avez @ de £ de réinitialiser le Temps de Recharge des Compétences Ancestrales accordées par ce Slorm Reaper.*Chaque fois que vous restaurez la Vie à un ennemi affecté par Malédiction Nécrotique, elle se propage à un autre ennemi proche.*act:246*act:247*act:248*act:249/\n|*Vous appliquez désormais continuellement Malédiction Nécrotique à tous les ennemis dans un rayon d'environ @ mètres./\n|*Vous ne pouvez plus restaurer de la Vie avec Vie par Coup, Vie par Victime et Vol de Vie.//\"Chaque frappe est porteuse d'espoir et de guérison...\"",
    "FR_NAME": "$ de Bénédiction",
    "FR_DESC": "*+@ £*+@ £|*Chaque fois que vous devriez restaurer la Vie de quelqu'un, la Vie restaurée est augmentée de @.*Chaque fois que vous infligez des dégâts avec votre Compétence Primaire ou Secondaire, vous avez @ de £ de réinitialiser le Temps de Recharge des Compétences Ancestrales accordées par ce Slorm Reaper.*Chaque fois que vous restaurez la Vie à un ennemi affecté par Malédiction Nécrotique, elle se propage à un autre ennemi proche.*act:246*act:247*act:248*act:249/\n|*Vous appliquez désormais continuellement Malédiction Nécrotique à tous les ennemis dans un rayon d'environ @ mètres./\n|*Vous ne pouvez plus restaurer de la Vie avec Vie par Coup, Vie par Victime et Vol de Vie.",
    "FR_LORE": "\"Chaque frappe est porteuse d'espoir et de guérison...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 69,
    "SPRITE": 68,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "9|1|1|10|\n3",
    "VALUE_TYPE": "|%|%|%|\n",
    "VALUE_LEVEL": "rl|rl|rl||",
    "VALUE_STAT": "the_max_health_add|the_max_health_global_mult|chance",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 27,
    "BASE_DMG_MAX": 44,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 1,
    "MAX_LVL": 24,
    "EVOLVE_IN": 69,
    "100": "551 - 1007"
  },
  {
    "REF": 69,
    "EN_NAME": "$ of Holy Benediction",
    "EN_DESC": "|*Life restored by Target Healing, Rejuvenation and Radiant Healing is increased by @ on enemies affected by Necrotic Curse.",
    "EN_LORE": "\"... soothing hearts, and strengthening spirits...\"",
    "EN": "$ of Holy Benediction//|*Life restored by Target Healing, Rejuvenation and Radiant Healing is increased by @ on enemies affected by Necrotic Curse.//\"... soothing hearts, and strengthening spirits...\"",
    "FR": "$ de Bénédiction Bénite//|*La Vie restaurée par Guérison Ciblée, Rajeunissement et Guérison Rayonnante est augmentée de @ sur les ennemis affectés par Malédiction Nécrotique.//\"... apaisant les cœurs et fortifiant les esprits...\"",
    "FR_NAME": "$ de Bénédiction Bénite",
    "FR_DESC": "|*La Vie restaurée par Guérison Ciblée, Rajeunissement et Guérison Rayonnante est augmentée de @ sur les ennemis affectés par Malédiction Nécrotique.",
    "FR_LORE": "\"... apaisant les cœurs et fortifiant les esprits...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 70,
    "SPRITE": 69,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "rl",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 27,
    "BASE_DMG_MAX": 44,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 36,
    "EVOLVE_IN": 70,
    "100": "836 - 1293"
  },
  {
    "REF": 70,
    "EN_NAME": "$ of Smoldering Holy Benediction",
    "EN_DESC": "|*Whenever you restore your own Life with a Skill or a Triggered effect, you now restore that much Life to enemies affected by Necrotic Curse.",
    "EN_LORE": "\"... but most importantly... Tons of Damage!\"#- Slorm Reapers of Legend, by Pr. Geranium",
    "EN": "$ of Smoldering Holy Benediction//|*Whenever you restore your own Life with a Skill or a Triggered effect, you now restore that much Life to enemies affected by Necrotic Curse.//\"... but most importantly... Tons of Damage!\"#- Slorm Reapers of Legend, by Pr. Geranium",
    "FR": "$ Fumant de Bénédiction Bénite/$ Fumante de Bénédiction Bénite//|*Chaque fois que vous restaurez votre propre Vie avec une Compétence ou un effet déclenché, vous restaurez désormais la même quantité de Vie aux ennemis affectés par Malédiction Nécrotique.//\"... mais par-dessus tout... des Tonnes de Dégats !\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "$ Fumant de Bénédiction Bénite/$ Fumante de Bénédiction Bénite",
    "FR_DESC": "|*Chaque fois que vous restaurez votre propre Vie avec une Compétence ou un effet déclenché, vous restaurez désormais la même quantité de Vie aux ennemis affectés par Malédiction Nécrotique.",
    "FR_LORE": "\"... mais par-dessus tout... des Tonnes de Dégats !\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 71,
    "SPRITE": 70,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 27,
    "BASE_DMG_MAX": 44,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "978 - 1439"
  },
  {
    "REF": 71,
    "EN_NAME": "Slormandrite $ of the Shaman",
    "EN_DESC": "*+@ £|*Totems deal @ Increased Damage.*act:219/\n|*Summon Prime Totem has @ charges./\n|*Your Primary, Secondary and Support Skills are locked if you are not within @ yards of an active Totem.",
    "EN_LORE": "\"This is Jerelzor's favorite Reaper...\"",
    "EN": "Slormandrite $ of the Shaman//*+@ £|*Totems deal @ Increased Damage.*act:219/\n|*Summon Prime Totem has @ charges./\n|*Your Primary, Secondary and Support Skills are locked if you are not within @ yards of an active Totem.//\"This is Jerelzor's favorite Reaper...\"",
    "FR": "$ en Slormandrite du Shaman//*+@ £|*Vos Totems infligent @ de Dégats Supplémentaires.*act:219/\n|*Invocation de Totem Primal a @ charges./\n|*Vos Compétences Primaires, Secondaires et de Support sont Verrouillées si vous n'êtes pas dans un rayon de @ mètres d'un Totem.//\"C'est le Reaper préféré de Jerelzor...\"",
    "FR_NAME": "$ en Slormandrite du Shaman",
    "FR_DESC": "*+@ £|*Vos Totems infligent @ de Dégats Supplémentaires.*act:219/\n|*Invocation de Totem Primal a @ charges./\n|*Vos Compétences Primaires, Secondaires et de Support sont Verrouillées si vous n'êtes pas dans un rayon de @ mètres d'un Totem.",
    "FR_LORE": "\"C'est le Reaper préféré de Jerelzor...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 72,
    "SPRITE": 71,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|3|5",
    "VALUE_TYPE": "%|%||",
    "VALUE_LEVEL": "rl|rl||",
    "VALUE_STAT": "totem_increased_effect_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 2,
    "MAX_LVL": 44,
    "EVOLVE_IN": 72,
    "100": "535 - 1408"
  },
  {
    "REF": 72,
    "EN_NAME": "Slormandrite $ of the Great Shaman",
    "EN_DESC": "|*act:220.",
    "EN_LORE": "\"... He never used it. But he likes the idea.\" #- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "EN": "Slormandrite $ of the Great Shaman//|*act:220.//\"... He never used it. But he likes the idea.\" #- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "FR": "$ en Slormandrite du Grand Shaman//|*act:220.//\"... Il ne l'a jamais utilisé. Mais il adore le concept.\" #- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "FR_NAME": "$ en Slormandrite du Grand Shaman",
    "FR_DESC": "|*act:220.",
    "FR_LORE": "\"... Il ne l'a jamais utilisé. Mais il adore le concept.\" #- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 73,
    "SPRITE": 72,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "963 - 1837"
  },
  {
    "REF": 73,
    "EN_NAME": "$ of the Lifebender",
    "EN_DESC": "*+@ £|*Whenever you cast a Skill, you restore Life equal to @ of its Mana cost.*Whenever you're dealt damage, you restore @ of your Missing Mana./\n|*The value of the highest damage received during this Expedition is stored.#Your Primary Skill deals _ Additional Damage.#Additional Damage is equal to the damage stored.*You have +_ £.#Max Life is equal to @ of the damage stored./\n*Your Mana Regeneration is reduced to 0.",
    "EN_LORE": "The Slorm Reaper of Sahrusiel the manipulator... capable of amplifying the force of any attack... \"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "EN": "$ of the Lifebender//*+@ £|*Whenever you cast a Skill, you restore Life equal to @ of its Mana cost.*Whenever you're dealt damage, you restore @ of your Missing Mana./\n|*The value of the highest damage received during this Expedition is stored.#Your Primary Skill deals _ Additional Damage.#Additional Damage is equal to the damage stored.*You have +_ £.#Max Life is equal to @ of the damage stored./\n*Your Mana Regeneration is reduced to 0.//The Slorm Reaper of Sahrusiel the manipulator... capable of amplifying the force of any attack... \"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "FR": "$ de l'Artisan de Vie//*+@ £|*Lorsque vous lancez une Compétence, vous récupérez de la Vie équivalente à @ de son Coût en Mana.*Lorsque vous subissez des dégâts, vous récupérez @ de votre Mana Manquant./\n|*La valeur des dégâts reçus les plus élevés de cette Expedition est conservée.#Votre Compétence Primaire inflige _ de Dégâts Additionnels.#Ces Dégâts Additionnels sont équivalents aux dégats conservés.*Vous avez +_ de £.#Cette Vie Max est équivalente à @ des dégâts conservée./\n*Votre Régénération de Mana est réduite à 0.//Le Slorm Reaper de Sahrusiel le manipulateur... capable d'amplifier la force de n'importe quelle attaque...\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "FR_NAME": "$ de l'Artisan de Vie",
    "FR_DESC": "*+@ £|*Lorsque vous lancez une Compétence, vous récupérez de la Vie équivalente à @ de son Coût en Mana.*Lorsque vous subissez des dégâts, vous récupérez @ de votre Mana Manquant./\n|*La valeur des dégâts reçus les plus élevés de cette Expedition est conservée.#Votre Compétence Primaire inflige _ de Dégâts Additionnels.#Ces Dégâts Additionnels sont équivalents aux dégats conservés.*Vous avez +_ de £.#Cette Vie Max est équivalente à @ des dégâts conservée./\n*Votre Régénération de Mana est réduite à 0.",
    "FR_LORE": "Le Slorm Reaper de Sahrusiel le manipulateur... capable d'amplifier la force de n'importe quelle attaque...\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "CH": "生命掌控者$//*+@£|*施放技能时恢复生命值，数值相当于其法力消耗的@。*造成伤害时，恢复已损失法力的@。/\n|*上一次受到的伤害被储存起来。#主技能造成_额外伤害。#额外伤害等于已储存的伤害。*获得+_£。#最大生命值等于已储存的伤害的@。/\n*法力再生减少为0。//萨鲁希尔的猎魂武装……这位操控者能够强化各种类型的攻击……”#- 《亚拉勇者，英雄还是小人》，亚当·诺斯图斯著",
    "CH_NAME": "生命掌控者$",
    "CH_DESCRIPTION": "*+@£|*施放技能时恢复生命值，数值相当于其法力消耗的@。*造成伤害时，恢复已损失法力的@。/\n|*上一次受到的伤害被储存起来。#主技能造成_额外伤害。#额外伤害等于已储存的伤害。*获得+_£。#最大生命值等于已储存的伤害的@。/\n*法力再生减少为0。",
    "CH_LORE": "萨鲁希尔的猎魂武装……这位操控者能够强化各种类型的攻击……”#- 《亚拉勇者，英雄还是小人》，亚当·诺斯图斯著",
    "TW": "生命掌控者$//*+@£|*施放技能時恢復生命值，數值相當於其法力消耗的@。 *造成傷害時，恢復已損失法力的@。 /\n|*上一次受到的傷害被儲存起來。 #主技能造成_額外傷害。 #額外傷害等於已儲存的傷害。 *獲得+_£。 #最大生命值等於已儲存的傷害的@。 /\n*法力再生減少為0。//薩魯希爾的獵魂武裝……這位操控者能夠強化各種類型的攻擊……”#- 《亞拉勇者，英雄還是小人》，亞當·諾斯圖斯著",
    "TW_NAME": "生命掌控者$",
    "TW_DESCRIPTION": "*+@£|*施放技能時恢復生命值，數值相當於其法力消耗的@。 *造成傷害時，恢復已損失法力的@。 /\n|*上一次受到的傷害被儲存起來。 #主技能造成_額外傷害。 #額外傷害等於已儲存的傷害。 *獲得+_£。 #最大生命值等於已儲存的傷害的@。 /\n*法力再生減少為0。",
    "TW_LORE": "薩魯希爾的獵魂武裝……這位操控者能夠強化各種類型的攻擊……”#- 《亞拉勇者，英雄還是小人》，亞當·諾斯圖斯著",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 74,
    "SPRITE": 73,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.2|0.1|\n1|\n-100",
    "VALUE_TYPE": "%|%|%|\n%|\n%",
    "VALUE_LEVEL": "rl|rl|rl|\nrl|",
    "VALUE_STAT": "the_max_health_percent|\nthe_max_health_add|\nmana_regen_global_mult",
    "VALUE_REAL": "synergy:damage_stored:100|synergy:damage_stored:1*rl",
    "BASE_DMG_MIN": 42,
    "BASE_DMG_MAX": 48,
    "MIN_DMG_LVL": 9,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1705 - 2223"
  },
  {
    "REF": 74,
    "EN_NAME": "$ of the Manabender",
    "EN_DESC": "*+@ £|*Your Primary and Secondary Skills deal Additional Damage equal to @ of their Mana cost.*You have +_ £.#Raw Damage is equal to @ of your $.*act:202*The Cooldown Time of Manabender is equal to your Max Mana divided by your Mana Regeneration./\n|*Critical Strikes from your Primary and Secondary Skills create a Mana Detonation, dealing their Additional Damage as Elemental Damage to enemies within a ~ yard radius./\n|*Whenever you cast a Skill, @ of your Life is consumed.",
    "EN_LORE": "\"Mana is like good wine. When it runs out... It runs out.\" -Innkeeper's Memoirs, by Karanos",
    "EN": "$ of the Manabender//*+@ £|*Your Primary and Secondary Skills deal Additional Damage equal to @ of their Mana cost.*You have +_ £.#Raw Damage is equal to @ of your $.*act:202*The Cooldown Time of Manabender is equal to your Max Mana divided by your Mana Regeneration./\n|*Critical Strikes from your Primary and Secondary Skills create a Mana Detonation, dealing their Additional Damage as Elemental Damage to enemies within a ~ yard radius./\n|*Whenever you cast a Skill, @ of your Life is consumed.//\"Mana is like good wine. When it runs out... It runs out.\" -Innkeeper's Memoirs, by Karanos",
    "FR": "$ de l'Artisan de Mana//*+@ £|*Vos Compétences Primaires et Secondaires infligent des Dégâts Additionnels équivalents à @ de leur Coût en Mana.*Vous avez +_ de £.#Ces Dégâts Bruts sont équivalents à @ de votre $.*act:202*Le Temps de Recharge d'Artisan de Mana est équivalent à votre Mana Max divisé par votre Régénération de Mana./\n|*Les Coups Critiques de vos Compétences Primaires et Secondaires créent une Détonation de Mana, infligeant leurs Dégâts Additionnels en tant que Dégâts Élémentaires aux ennemis dans un rayon de ~ mètres./\n|*Lorsque vous lancez une Compétence, @ de votre Vie est consumée.//\"Le Mana, c'est comme le bon vin. Quand y en a plus... Y en a plus.\" -Mémoires d'aubergiste, par Karanos",
    "FR_NAME": "$ de l'Artisan de Mana",
    "FR_DESC": "*+@ £|*Vos Compétences Primaires et Secondaires infligent des Dégâts Additionnels équivalents à @ de leur Coût en Mana.*Vous avez +_ de £.#Ces Dégâts Bruts sont équivalents à @ de votre $.*act:202*Le Temps de Recharge d'Artisan de Mana est équivalent à votre Mana Max divisé par votre Régénération de Mana./\n|*Les Coups Critiques de vos Compétences Primaires et Secondaires créent une Détonation de Mana, infligeant leurs Dégâts Additionnels en tant que Dégâts Élémentaires aux ennemis dans un rayon de ~ mètres./\n|*Lorsque vous lancez une Compétence, @ de votre Vie est consumée.",
    "FR_LORE": "\"Le Mana, c'est comme le bon vin. Quand y en a plus... Y en a plus.\" -Mémoires d'aubergiste, par Karanos",
    "CH": "法力掌控者$//*+@£|*主技能或副技能造成额外伤害，数值相当于其法力消耗的@。*获得+_£。#原始伤害等于你的$的@。*act:202*法力掌控者的冷却时间等于最大法力除以法力再生。/\n|*激活法力掌控者时，主技能或副技能的暴击会引发一次法力爆破，对~码半径内的敌人造成额外伤害（元素伤害）。/\n|*施放技能时，消耗生命的@。//“法力就像美酒。喝完了……那就真完了。”- 《旅店往事》，卡拉诺斯著",
    "CH_NAME": "法力掌控者$",
    "CH_DESCRIPTION": "*+@£|*主技能或副技能造成额外伤害，数值相当于其法力消耗的@。*获得+_£。#原始伤害等于你的$的@。*act:202*法力掌控者的冷却时间等于最大法力除以法力再生。/\n|*激活法力掌控者时，主技能或副技能的暴击会引发一次法力爆破，对~码半径内的敌人造成额外伤害（元素伤害）。/\n|*施放技能时，消耗生命的@。",
    "CH_LORE": "“法力就像美酒。喝完了……那就真完了。”- 《旅店往事》，卡拉诺斯著",
    "TW": "法力掌控者$//*+@£|*主技能或副技能造成額外傷害，數值相當於其法力消耗的@。 *獲得+_£。 #原始傷害等於你的$的@。*act:202*法力掌控者的冷卻時間等於最大法力除以法力再生。 /\n|*激活法力掌控者時，主技能或副技能的暴擊會引發一次法力爆破，對~碼半徑內的敵人造成額外傷害（元素傷害）。 /\n|*施放技能時，消耗生命的@。//“法力就像美酒。喝完了……那就真完了。”- 《旅店往事》，卡拉諾斯著",
    "TW_NAME": "法力掌控者$",
    "TW_DESCRIPTION": "*+@£|*主技能或副技能造成額外傷害，數值相當於其法力消耗的@。 *獲得+_£。 #原始傷害等於你的$的@。*act:202*法力掌控者的冷卻時間等於最大法力除以法力再生。 /\n|*激活法力掌控者時，主技能或副技能的暴擊會引發一次法力爆破，對~碼半徑內的敵人造成額外傷害（元素傷害）。 /\n|*施放技能時，消耗生命的@。",
    "TW_LORE": "“法力就像美酒。喝完了……那就真完了。”- 《旅店往事》，卡拉諾斯著",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 75,
    "SPRITE": 74,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1.2|8|1|\n\n1",
    "VALUE_TYPE": "|%|%|\n\n%",
    "VALUE_LEVEL": "rl|rl|rl|",
    "VALUE_STAT": "mana_regen_add|min_basic_damage_add",
    "VALUE_REAL": "synergy:mana_regeneration:1*rl",
    "BASE_DMG_MIN": 20,
    "BASE_DMG_MAX": 26,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "637 - 1495"
  },
  {
    "REF": 75,
    "EN_NAME": "$ of the Nimble Warrior",
    "EN_DESC": "*Nimble Effects:*+@ £*+@ £*+@ £*+@ £*Your Primary Skill deals @ £.|*Whenever you lose Life when hit, effects listed as Nimble are deactivated for the next @ seconds./\n|*Additional Nimble Effect:#While in Combat, you gain a <Nimble Champion> stack every second, up to 100.#Effects listed as Nimble have a @ Multiplier for each stack of <Nimble Champion>.*Whenever you should receive damage, if you have <Nimble Champion> stacks, you ignore Incoming Damage, lose your <Nimble Champion> stacks instead and cannot gain <Nimble Champion> stacks for the next @ seconds./\n|*Whenever you're dealt damage and lose Life this way, your Primary and Secondary Skills are Locked for the next @ seconds.",
    "EN_LORE": "\"It is the Champion Blade-Master Engil and his pronounced taste for danger that inspired Adrianne to forge this Slorm Reaper. \"#- Slorm Reapers of Legend, by Pr. Geranium",
    "EN": "$ of the Nimble Warrior//*Nimble Effects:*+@ £*+@ £*+@ £*+@ £*Your Primary Skill deals @ £.|*Whenever you lose Life when hit, effects listed as Nimble are deactivated for the next @ seconds./\n|*Additional Nimble Effect:#While in Combat, you gain a <Nimble Champion> stack every second, up to 100.#Effects listed as Nimble have a @ Multiplier for each stack of <Nimble Champion>.*Whenever you should receive damage, if you have <Nimble Champion> stacks, you ignore Incoming Damage, lose your <Nimble Champion> stacks instead and cannot gain <Nimble Champion> stacks for the next @ seconds./\n|*Whenever you're dealt damage and lose Life this way, your Primary and Secondary Skills are Locked for the next @ seconds.//\"It is the Champion Blade-Master Engil and his pronounced taste for danger that inspired Adrianne to forge this Slorm Reaper. \"#- Slorm Reapers of Legend, by Pr. Geranium",
    "FR": "$ du Guerrier Agile//*Effets d'Agilité:*+@ £*+@ £*+@ £*+@ £*Votre Compétence Primaire inflige @ de £.|*Lorsque vous subissez des dégâts et perdez de la Vie, les Effets d'Agilité sont désactivés pour les @ prochaines secondes./\n|*Effets d'Agilité Supplémentaire :#En Combat, vous gagnez un cumul de <Champion Agile> chaque seconde, jusqu'à 100.#Les Effets d'Agilité reçoivent un Multiplicateur de @ pour chaque cumul de <Champion Agile>.*Lorsque vous subissez des dégâts, si vous avez des cumuls de <Champion Agile>, ces dégâts sont ignorés, vous perdez vos cumuls de <Champion Agile> à la place et ne pouvez plus gagner de cumuls de <Champions Agile> pour les @ prochaines secondes./\n|*Lorsque vous subissez des dégâts et perdez de la Vie, vos Compétences Primaires et Secondaires sont Verrouillées pour les @ prochaines secondes.//\"C'est le Champion Maître-Lame Engil et son goût prononcé pour le danger qui ont inspirés Adrianne à forger ce Slorm Reaper.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "$ du Guerrier Agile",
    "FR_DESC": "*Effets d'Agilité:*+@ £*+@ £*+@ £*+@ £*Votre Compétence Primaire inflige @ de £.|*Lorsque vous subissez des dégâts et perdez de la Vie, les Effets d'Agilité sont désactivés pour les @ prochaines secondes./\n|*Effets d'Agilité Supplémentaire :#En Combat, vous gagnez un cumul de <Champion Agile> chaque seconde, jusqu'à 100.#Les Effets d'Agilité reçoivent un Multiplicateur de @ pour chaque cumul de <Champion Agile>.*Lorsque vous subissez des dégâts, si vous avez des cumuls de <Champion Agile>, ces dégâts sont ignorés, vous perdez vos cumuls de <Champion Agile> à la place et ne pouvez plus gagner de cumuls de <Champions Agile> pour les @ prochaines secondes./\n|*Lorsque vous subissez des dégâts et perdez de la Vie, vos Compétences Primaires et Secondaires sont Verrouillées pour les @ prochaines secondes.",
    "FR_LORE": "\"C'est le Champion Maître-Lame Engil et son goût prononcé pour le danger qui ont inspirés Adrianne à forger ce Slorm Reaper.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "机敏战士$//*机敏：*+@£*+@£*+@£*+@£*主技能造成@£。|*造成伤害时，机敏的效果暂时禁用@秒。/\n|*在战斗中，每秒获得一层<Nimble Champion>，最多100层。#每层<Nimble Champion>使机敏的效果获得@倍率。*受到伤害时，若拥有<Nimble Champion>层数，则无视该伤害，损失<Nimble Champion>层数，在此后@秒内无法获得<Nimble Champion>层数。/\n|*受到伤害时，主技能或副技能被锁定，持续@秒。//“剑圣勇者恩吉尔和他对危险的一贯直觉启迪了阿德里安娜锻造这把武器。”#- 猎魂传奇，杰兰涅姆教授",
    "CH_NAME": "机敏战士$",
    "CH_DESCRIPTION": "*机敏：*+@£*+@£*+@£*+@£*主技能造成@£。|*造成伤害时，机敏的效果暂时禁用@秒。/\n|*在战斗中，每秒获得一层<Nimble Champion>，最多100层。#每层<Nimble Champion>使机敏的效果获得@倍率。*受到伤害时，若拥有<Nimble Champion>层数，则无视该伤害，损失<Nimble Champion>层数，在此后@秒内无法获得<Nimble Champion>层数。/\n|*受到伤害时，主技能或副技能被锁定，持续@秒。",
    "CH_LORE": "“剑圣勇者恩吉尔和他对危险的一贯直觉启迪了阿德里安娜锻造这把武器。”#- 猎魂传奇，杰兰涅姆教授",
    "TW": "機敏戰士$//*機敏：*+@£*+@£*+@£*+@£*主技能造成@£。 |*造成傷害時，機敏的效果暫時禁用@秒。 /\n|*在戰鬥中，每秒獲得一層<Nimble Champion>，最多100層。 #每層<Nimble Champion>使機敏的效果獲得@倍率。 *受到傷害時，若擁有<Nimble Champion>層數，則無視該傷害，損失<Nimble Champion>層數，在此後@秒內無法獲得<Nimble Champion>層數。 /\n|*受到傷害時，主技能或副技能被鎖定，持續@秒。//“劍聖勇者恩吉爾和他對危險的一貫直覺啟迪了阿德里安娜鍛造這把武器。”#- 獵魂傳奇，傑蘭涅姆教授",
    "TW_NAME": "機敏戰士$",
    "TW_DESCRIPTION": "*機敏：*+@£*+@£*+@£*+@£*主技能造成@£。 |*造成傷害時，機敏的效果暫時禁用@秒。 /\n|*在戰鬥中，每秒獲得一層<Nimble Champion>，最多100層。 #每層<Nimble Champion>使機敏的效果獲得@倍率。 *受到傷害時，若擁有<Nimble Champion>層數，則無視該傷害，損失<Nimble Champion>層數，在此後@秒內無法獲得<Nimble Champion>層數。 /\n|*受到傷害時，主技能或副技能被鎖定，持續@秒。",
    "TW_LORE": "“劍聖勇者恩吉爾和他對危險的一貫直覺啟迪了阿德里安娜鍛造這把武器。”#- 獵魂傳奇，傑蘭涅姆教授",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 76,
    "SPRITE": 75,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|3|15|5|1|3|\n2|6|\n6",
    "VALUE_TYPE": "%|%|%|%|%||\n%||",
    "VALUE_LEVEL": "rl|rl|||rl||\n||",
    "VALUE_STAT": "crit_damage_percent|brut_damage_percent|crit_chance_percent|brut_chance_percent|increased_damage|\n",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 21,
    "BASE_DMG_MAX": 32,
    "MIN_DMG_LVL": 8,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2127 - 2894"
  },
  {
    "REF": 76,
    "EN_NAME": "$ of The Timeshifter",
    "EN_DESC": "|*Whenever you spend or lose Mana, @ of that amount is added to a Timeshift counter that rapidly decreases.#While your Timeshift counter is above your total Max Mana, you are Curving Time.*While Curving Time or Timeshifting, you have:#+@ £#+@ £#+@ £*act:250/\n|*While Curving Time or Timeshifting, you deal @ £ and gain +@ £./\n|*When not Curving Time or Timeshifting, your Attack Speed and Movement Speed are set to their base value.",
    "EN_LORE": "+ 10% Coffee Making Speed",
    "EN": "$ of The Timeshifter//|*Whenever you spend or lose Mana, @ of that amount is added to a Timeshift counter that rapidly decreases.#While your Timeshift counter is above your total Max Mana, you are Curving Time.*While Curving Time or Timeshifting, you have:#+@ £#+@ £#+@ £*act:250/\n|*While Curving Time or Timeshifting, you deal @ £ and gain +@ £./\n|*When not Curving Time or Timeshifting, your Attack Speed and Movement Speed are set to their base value.//+ 10% Coffee Making Speed",
    "FR": "$ de Décalage Temporaire//|*Chaque fois que vous dépensez ou perdez du Mana, @ de cette quantité est ajouté à un compteur de Temporalité qui diminue rapidement.#Tant que votre compteur de Temporalité est supérieur à votre Mana Max, vous êtes Courbez le Temps.*Tant que vous Courbez ou Décalez le Temps, vous avez :#+@ £#+@ £#+@ £*act:250/\n|*Tant que vous Courbez ou Décalez le Temps, vous infligez @ de £ et gagnez +@ de £./\n|*Lorsque vous ne Courbez ou ne Décalez pas le Temps votre Vitesse d'Attaque et Vitesse de Déplacement sont réduites à leurs valeurs de base.//+10% de Vitesse en Préparation de Café.",
    "FR_NAME": "$ de Décalage Temporaire",
    "FR_DESC": "|*Chaque fois que vous dépensez ou perdez du Mana, @ de cette quantité est ajouté à un compteur de Temporalité qui diminue rapidement.#Tant que votre compteur de Temporalité est supérieur à votre Mana Max, vous êtes Courbez le Temps.*Tant que vous Courbez ou Décalez le Temps, vous avez :#+@ £#+@ £#+@ £*act:250/\n|*Tant que vous Courbez ou Décalez le Temps, vous infligez @ de £ et gagnez +@ de £./\n|*Lorsque vous ne Courbez ou ne Décalez pas le Temps votre Vitesse d'Attaque et Vitesse de Déplacement sont réduites à leurs valeurs de base.",
    "FR_LORE": "+10% de Vitesse en Préparation de Café.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 77,
    "SPRITE": 102,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "100|15|10|0.5|\n0.5|1",
    "VALUE_TYPE": "%|%|%|%|\n%|%",
    "VALUE_LEVEL": "|||rl|\nrl|rl",
    "VALUE_STAT": "cooldown_reduction_global_mult|crit_chance_percent|the_speed_percent|\nincreased_damage|crit_damage_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 19,
    "BASE_DMG_MAX": 25,
    "MIN_DMG_LVL": 8,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1474 - 1995"
  },
  {
    "REF": 77,
    "EN_NAME": "$ of Ancestral Wrath",
    "EN_DESC": "*+@ £|*Imbued Skills and Ancestral Beams deal @ £ for every equipped Imbue Effect.*Whenever you deal Ancestral Strike Damage, you trigger an Ancestral Beam, a Piercing projectile coming from a random direction toward you, dealing _ £ and _ £ to enemies hit.*Whenever an Ancestral Beam reaches you, it is absorbed and you gain an Ancestral Wrath stack, up to @. You then have a _% £ to trigger Ancestral Wrath. This Chance is equal to your total Ancestral Wrath stacks.*Ancestral Beams are Imbued as if they were your Primary orSecondary Skill.*act:251/\n|*If both the First and Second Imbue Skills of an Element are Equipped, then the Third Imbue Skill of that Element is automatically Equipped and at Max Rank.*Whenever you activate Ancestral Wrath, you no longer consume your Ancestral Wrath stacks./\n|*Your Primary and Secondary Skills deal _% Decreased Damage. Decreased Damage is equal to your total Ancestral Wrath stacks.",
    "EN_LORE": "Fus Ro Dah!",
    "EN": "$ of Ancestral Wrath//*+@ £|*Imbued Skills and Ancestral Beams deal @ £ for every equipped Imbue Effect.*Whenever you deal Ancestral Strike Damage, you trigger an Ancestral Beam, a Piercing projectile coming from a random direction toward you, dealing _ £ and _ £ to enemies hit.*Whenever an Ancestral Beam reaches you, it is absorbed and you gain an Ancestral Wrath stack, up to @. You then have a _% £ to trigger Ancestral Wrath. This Chance is equal to your total Ancestral Wrath stacks.*Ancestral Beams are Imbued as if they were your Primary orSecondary Skill.*act:251/\n|*If both the First and Second Imbue Skills of an Element are Equipped, then the Third Imbue Skill of that Element is automatically Equipped and at Max Rank.*Whenever you activate Ancestral Wrath, you no longer consume your Ancestral Wrath stacks./\n|*Your Primary and Secondary Skills deal _% Decreased Damage. Decreased Damage is equal to your total Ancestral Wrath stacks.//Fus Ro Dah!",
    "FR": "$ de Colère Ancestrale//*+@ £|*Les Compétences Imprégnées et les Rayons Ancestraux infligent @ de £ pour chaque Effet d'Imprégnation équipé.*Chaque fois que vous infligez des Dégâts Ancestraux, vous déclenchez un Rayon Ancestral, un projectile Perforant venant d'une direction aléatoire vers vous, infligeant _ de £ et _ de £ aux ennemis touchés.*Chaque fois qu'un Rayon Ancestral vous atteint, il est absorbé et vous gagnez un cumul de Courroux Ancestral, jusqu'à @. Vous avez ensuite _% de £ de déclencher le Courroux Ancestral. Cette chance est équivalente au total de vos cumuls de Courroux Ancestral.*Les Rayons Ancestraux sont Imprégnés comme s'ils étaient une Compétence Primaire ou Secondaire.*act:251/\n|*Si à la fois la Première et la Deuxième Compétence Imprégnée d'un Élément sont Équipées, alors la Troisième Compétence Imprégnée de cet Élément est automatiquement Équipée et au Rang Max.*Chaque fois que vous activez le Courroux Ancestral, vous ne consommez plus vos cumuls de Courroux Ancestral./\n|*Vos Compétences Primaires et Secondaires infligent des Dégats Réduits de _%. Ces Dégâts Réduits sont équivalents au total de vos cumuls de Courroux Ancestral.//Fus Ro Dah !",
    "FR_NAME": "$ de Colère Ancestrale",
    "FR_DESC": "*+@ £|*Les Compétences Imprégnées et les Rayons Ancestraux infligent @ de £ pour chaque Effet d'Imprégnation équipé.*Chaque fois que vous infligez des Dégâts Ancestraux, vous déclenchez un Rayon Ancestral, un projectile Perforant venant d'une direction aléatoire vers vous, infligeant _ de £ et _ de £ aux ennemis touchés.*Chaque fois qu'un Rayon Ancestral vous atteint, il est absorbé et vous gagnez un cumul de Courroux Ancestral, jusqu'à @. Vous avez ensuite _% de £ de déclencher le Courroux Ancestral. Cette chance est équivalente au total de vos cumuls de Courroux Ancestral.*Les Rayons Ancestraux sont Imprégnés comme s'ils étaient une Compétence Primaire ou Secondaire.*act:251/\n|*Si à la fois la Première et la Deuxième Compétence Imprégnée d'un Élément sont Équipées, alors la Troisième Compétence Imprégnée de cet Élément est automatiquement Équipée et au Rang Max.*Chaque fois que vous activez le Courroux Ancestral, vous ne consommez plus vos cumuls de Courroux Ancestral./\n|*Vos Compétences Primaires et Secondaires infligent des Dégats Réduits de _%. Ces Dégâts Réduits sont équivalents au total de vos cumuls de Courroux Ancestral.",
    "FR_LORE": "Fus Ro Dah !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 65,
    "SPRITE": 56,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "5|1|1|\n1",
    "VALUE_TYPE": "%|%||\n%",
    "VALUE_LEVEL": "|rl|rl|\nrl",
    "VALUE_STAT": "brut_chance_percent|increased_damage|physical_damage|elemental_damage|chance",
    "VALUE_REAL": "damage:physical_damage:1*rl|damage:elemental_damage:1*rl|synergy:ancestral_wrath_stacks:1|synergy:ancestral_wrath_stacks:1",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 23,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "801 - 1657"
  },
  {
    "REF": 78,
    "EN_NAME": "$ of the Larva",
    "EN_DESC": "*Larva Effects:*@ £*@ £|/\n|*<Exhaustion> now also deals @ of the enemy's Max Life as Elemental Damage per tick./\n|*The Base Cooldown Time of your Skills can no longer be below @ seconds.",
    "EN_LORE": "\"Don't be fooled by this Larva...\"",
    "EN": "$ of the Larva//*Larva Effects:*@ £*@ £|/\n|*<Exhaustion> now also deals @ of the enemy's Max Life as Elemental Damage per tick./\n|*The Base Cooldown Time of your Skills can no longer be below @ seconds.//\"Don't be fooled by this Larva...\"",
    "FR": "$ de la Larve//*Effets de la Larve:*@ £*@ £|/\n|*<Épuisement> inflige @ de la Vie Max de l'ennemi en tant que Dégâts Élémentaires à chaque tick./\n|*Le Temps de Recharge de base de vos Compétences ne peut pas être réduit en dessous de @ secondes.//\"Ne vous laissez pas berner par cette Larve...\"\n",
    "FR_NAME": "$ de la Larve",
    "FR_DESC": "*Effets de la Larve:*@ £*@ £|/\n|*<Épuisement> inflige @ de la Vie Max de l'ennemi en tant que Dégâts Élémentaires à chaque tick./\n|*Le Temps de Recharge de base de vos Compétences ne peut pas être réduit en dessous de @ secondes.",
    "FR_LORE": "\"Ne vous laissez pas berner par cette Larve...\"\n",
    "CH": "熔岩$//*熔岩：*@£*@£|/\n|*<Exhaustion>的每次判定使敌人损失最大生命值的@（元素伤害）。/\n|*技能的基础冷却时间不少于@秒。//“别被熔岩给骗了……”",
    "CH_NAME": "熔岩$",
    "CH_DESCRIPTION": "*熔岩：*@£*@£|/\n|*<Exhaustion>的每次判定使敌人损失最大生命值的@（元素伤害）。/\n|*技能的基础冷却时间不少于@秒。",
    "CH_LORE": "“别被熔岩给骗了……”",
    "TW": "熔岩$//*熔岩：*@£*@£|/\n|*<Exhaustion>的每次判定使敵人損失最大生命值的@（元素傷害）。 /\n|*技能的基礎冷卻時間不少於@秒。//“別被熔岩給騙了……”",
    "TW_NAME": "熔岩$",
    "TW_DESCRIPTION": "*熔岩：*@£*@£|/\n|*<Exhaustion>的每次判定使敵人損失最大生命值的@（元素傷害）。 /\n|*技能的基礎冷卻時間不少於@秒。",
    "TW_LORE": "“別被熔岩給騙了……”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 78,
    "SPRITE": 78,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.3|0.5|\n3|\n2",
    "VALUE_TYPE": "%|%|\n%|",
    "VALUE_LEVEL": "rl|rl|\n|",
    "VALUE_STAT": "the_speed_percent|elemental_damage_global_mult",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 1,
    "BASE_DMG_MAX": 1,
    "MIN_DMG_LVL": 0,
    "MAX_DMG_LVL": 0,
    "DMG_MULTIPLIER": 0,
    "LOOTABLE": 2,
    "MAX_LVL": 18,
    "EVOLVE_IN": 79,
    "100": "1 - 1"
  },
  {
    "REF": 79,
    "EN_NAME": "$ of the Chrysalis",
    "EN_DESC": "|*Chrysalis Effect:#@ £ for every Negative Effect on enemies within a @ yard radius.",
    "EN_LORE": "\"... because it will eventually hatch and become a beautiful Butterfly...\"",
    "EN": "$ of the Chrysalis//|*Chrysalis Effect:#@ £ for every Negative Effect on enemies within a @ yard radius.//\"... because it will eventually hatch and become a beautiful Butterfly...\"",
    "FR": "$ de la Chrysalide//|*Effets de la Chrysalide:#@ de £ pour chaque Effet Négatif sur les ennemis dans un rayon de @ mètres.//\"... car elle finira par éclore et devenir un magnifique Papillon...\"",
    "FR_NAME": "$ de la Chrysalide",
    "FR_DESC": "|*Effets de la Chrysalide:#@ de £ pour chaque Effet Négatif sur les ennemis dans un rayon de @ mètres.",
    "FR_LORE": "\"... car elle finira par éclore et devenir un magnifique Papillon...\"",
    "CH": "茧蛹$//|*茧蛹：#@@码半径内的敌人身上的每个负面效果获得£。//“……它终将化茧成蝶，张开美丽的翅膀……”",
    "CH_NAME": "茧蛹$",
    "CH_DESCRIPTION": "|*茧蛹：#@@码半径内的敌人身上的每个负面效果获得£。",
    "CH_LORE": "“……它终将化茧成蝶，张开美丽的翅膀……”",
    "TW": "繭蛹$//|*繭蛹：#@@碼半徑內的敵人身上的每個負面效果獲得£。//“……它終將化繭成蝶，張開美麗的翅膀……”",
    "TW_NAME": "繭蛹$",
    "TW_DESCRIPTION": "|*繭蛹：#@@碼半徑內的敵人身上的每個負面效果獲得£。",
    "TW_LORE": "“……它終將化繭成蝶，張開美麗的翅膀……”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 79,
    "SPRITE": 79,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "5|2",
    "VALUE_TYPE": "%|",
    "VALUE_LEVEL": "|",
    "VALUE_STAT": "elemental_damage_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 1,
    "BASE_DMG_MAX": 2,
    "MIN_DMG_LVL": 0,
    "MAX_DMG_LVL": 0,
    "DMG_MULTIPLIER": 0,
    "LOOTABLE": null,
    "MAX_LVL": 36,
    "EVOLVE_IN": 80,
    "100": "1 - 2"
  },
  {
    "REF": 80,
    "EN_NAME": "$ of the Sleepy Butterfly",
    "EN_DESC": "|*Larva and Chrysalis Effects are now positive.*Your Primary and Secondary Skills apply <Exhaustion> to enemies hit, dealing _ £ per tick for the next _ seconds.#The Duration is equal to your Movement Speed.*act:201",
    "EN_LORE": "\"... which will reward your patience.\" #- Slorm Reapers of Legend, by Pr. Geranium",
    "EN": "$ of the Sleepy Butterfly//|*Larva and Chrysalis Effects are now positive.*Your Primary and Secondary Skills apply <Exhaustion> to enemies hit, dealing _ £ per tick for the next _ seconds.#The Duration is equal to your Movement Speed.*act:201//\"... which will reward your patience.\" #- Slorm Reapers of Legend, by Pr. Geranium",
    "FR": "$ du Papillon Endormi//|*Les effets de la Larve et de la Chrysalide sont désormais positifs.*Vos Compétences Primaires et Secondaires appliquent <Epuisement> aux ennemis touchés, infligeant _ de £ par tick durant les _ prochaines secondes.#La Durée est équivalente à votre Vitesse de Déplacement.*act:201//\"... qui saura récompenser votre patience.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "$ du Papillon Endormi",
    "FR_DESC": "|*Les effets de la Larve et de la Chrysalide sont désormais positifs.*Vos Compétences Primaires et Secondaires appliquent <Epuisement> aux ennemis touchés, infligeant _ de £ par tick durant les _ prochaines secondes.#La Durée est équivalente à votre Vitesse de Déplacement.*act:201",
    "FR_LORE": "\"... qui saura récompenser votre patience.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "睡蝶$//|*熔岩和茧蛹变为正面效果。*主技能和副技能施加<Exhaustion>，每次判定造成_£，持续_秒。#持续时间等于移动速度。*act:201//“……这将使你的耐心有所回报。”#- 猎魂传奇，杰兰涅姆教授",
    "CH_NAME": "睡蝶$",
    "CH_DESCRIPTION": "|*熔岩和茧蛹变为正面效果。*主技能和副技能施加<Exhaustion>，每次判定造成_£，持续_秒。#持续时间等于移动速度。*act:201",
    "CH_LORE": "“……这将使你的耐心有所回报。”#- 猎魂传奇，杰兰涅姆教授",
    "TW": "睡蝶$//|*熔岩和繭蛹變為正面效果。 *主技能和副技能施加<Exhaustion>，每次判定造成_£，持續_秒。 #持續時間等於移動速度。 *act:201//“……這將使你的耐心有所回報。”#- 獵魂傳奇，傑蘭涅姆教授",
    "TW_NAME": "睡蝶$",
    "TW_DESCRIPTION": "|*熔岩和繭蛹變為正面效果。 *主技能和副技能施加<Exhaustion>，每次判定造成_£，持續_秒。 #持續時間等於移動速度。 *act:201",
    "TW_LORE": "“……這將使你的耐心有所回報。”#- 獵魂傳奇，傑蘭涅姆教授",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 80,
    "SPRITE": 80,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.3",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "rl",
    "VALUE_STAT": "elemental_damage",
    "VALUE_REAL": "damage:elemental_damage:0.3*rl|synergy:the_speed_value:1",
    "BASE_DMG_MIN": 19,
    "BASE_DMG_MAX": 25,
    "MIN_DMG_LVL": 7,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1880 - 2622"
  },
  {
    "REF": 81,
    "EN_NAME": "The Ancestral Legacy $",
    "EN_DESC": "*+@ £*+@ Power Crystal|*Reaper Damage is added to Elemental Damage instead of Skill Damage.*Power Crystals are rotating around you, firing piercing Crystal Shards every second, dealing _ $ to enemies hit.*Crystal Shards can deal Ancestral Strike Damage and trigger Critical Strike Effects and Ancestral Strike Effects.*Crystal Shards have a Critical Strike Chance and an Ancestral Strike Chance equal to yours./\n|*Power Crystals now fire @ Crystal Shards at once./\n|*Your Primary and Secondary Skills can no longer trigger Ancestral Strikes.",
    "EN_LORE": "\"Forged by Fulgurorn, directly on a cluster of Amethyst...\"",
    "EN": "The Ancestral Legacy $//*+@ £*+@ Power Crystal|*Reaper Damage is added to Elemental Damage instead of Skill Damage.*Power Crystals are rotating around you, firing piercing Crystal Shards every second, dealing _ $ to enemies hit.*Crystal Shards can deal Ancestral Strike Damage and trigger Critical Strike Effects and Ancestral Strike Effects.*Crystal Shards have a Critical Strike Chance and an Ancestral Strike Chance equal to yours./\n|*Power Crystals now fire @ Crystal Shards at once./\n|*Your Primary and Secondary Skills can no longer trigger Ancestral Strikes.//\"Forged by Fulgurorn, directly on a cluster of Amethyst...\"",
    "FR": "$ d'Héritage Ancestral//*+@ £*+@ Cristaux de Puissance|*Les Dégats de Reaper sont ajoutés aux Dégâts Élémentaires au lieu d'être ajoutés aux Dégats Physiques.*Des Cristaux de Puissance tournent autour de vous, tirant des Éclats de Cristal perçants chaque seconde, infligeant _ de $ aux ennemis touchés.*Les Eclats de Cristal peuvent infliger des Dégâts Ancestraux et déclencher des effets Critiques et Ancestraux.*Les Eclats de Cristal ont une Chance de Coup Critique et une Chance de Coup Ancestral équivalentes aux votres./\n|*Les Cristaux de Puissance tirent désormais @ Eclats de Cristal par salve./\n|*Vos Compétences Primaires et Secondaires ne peuvent plus déclencher des Coups Ancestraux.//\"Forgé par Fulgurorn, directement sur un amas d'Améthyste...\"",
    "FR_NAME": "$ d'Héritage Ancestral",
    "FR_DESC": "*+@ £*+@ Cristaux de Puissance|*Les Dégats de Reaper sont ajoutés aux Dégâts Élémentaires au lieu d'être ajoutés aux Dégats Physiques.*Des Cristaux de Puissance tournent autour de vous, tirant des Éclats de Cristal perçants chaque seconde, infligeant _ de $ aux ennemis touchés.*Les Eclats de Cristal peuvent infliger des Dégâts Ancestraux et déclencher des effets Critiques et Ancestraux.*Les Eclats de Cristal ont une Chance de Coup Critique et une Chance de Coup Ancestral équivalentes aux votres./\n|*Les Cristaux de Puissance tirent désormais @ Eclats de Cristal par salve./\n|*Vos Compétences Primaires et Secondaires ne peuvent plus déclencher des Coups Ancestraux.",
    "FR_LORE": "\"Forgé par Fulgurorn, directement sur un amas d'Améthyste...\"",
    "CH": "先祖遗产$//*+@£*+@能量水晶|*武器伤害被加算到元素伤害中，而非技能伤害中。*能量水晶围绕自身，每秒发射穿透性的水晶碎片，对击中的敌人造成_$。*水晶碎片可造成先祖攻击伤害并触发暴击效果和先祖攻击效果。*水晶碎片的暴击率和先祖攻击率套用你自身的属性。/\n|*能量水晶一次发射@个水晶碎片。/\n|*主技能和副技能伤害减少为0。//“弗古若恩锻造，直接采用整块紫晶……”",
    "CH_NAME": "先祖遗产$",
    "CH_DESCRIPTION": "*+@£*+@能量水晶|*武器伤害被加算到元素伤害中，而非技能伤害中。*能量水晶围绕自身，每秒发射穿透性的水晶碎片，对击中的敌人造成_$。*水晶碎片可造成先祖攻击伤害并触发暴击效果和先祖攻击效果。*水晶碎片的暴击率和先祖攻击率套用你自身的属性。/\n|*能量水晶一次发射@个水晶碎片。/\n|*主技能和副技能伤害减少为0。",
    "CH_LORE": "“弗古若恩锻造，直接采用整块紫晶……”",
    "TW": "先祖遺產$//*+@£*+@能量水晶|*武器傷害被加算到元素傷害中，而非技能傷害中。 *能量水晶圍繞自身，每秒發射穿透性的水晶碎片，對擊中的敵人造成_$。 *水晶碎片可造成先祖攻擊傷害並觸發暴擊效果和先祖攻擊效果。 *水晶碎片的暴擊率和先祖攻擊率套用你自身的屬性。 /\n|*能量水晶一次發射@個水晶碎片。 /\n|*主技能和副技能傷害減少為0。//“弗古若恩鍛造，直接採用整塊紫晶……”",
    "TW_NAME": "先祖遺產$",
    "TW_DESCRIPTION": "*+@£*+@能量水晶|*武器傷害被加算到元素傷害中，而非技能傷害中。 *能量水晶圍繞自身，每秒發射穿透性的水晶碎片，對擊中的敵人造成_$。 *水晶碎片可造成先祖攻擊傷害並觸發暴擊效果和先祖攻擊效果。 *水晶碎片的暴擊率和先祖攻擊率套用你自身的屬性。 /\n|*能量水晶一次發射@個水晶碎片。 /\n|*主技能和副技能傷害減少為0。",
    "TW_LORE": "“弗古若恩鍛造，直接採用整塊紫晶……”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 81,
    "SPRITE": 81,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|\n3|",
    "VALUE_TYPE": "%||\n|",
    "VALUE_LEVEL": "rl||\n|",
    "VALUE_STAT": "elemental_damage_percent",
    "VALUE_REAL": "damage:elemental_damage:100",
    "BASE_DMG_MIN": 26,
    "BASE_DMG_MAX": 34,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 1,
    "MAX_LVL": 42,
    "EVOLVE_IN": 82,
    "100": "691 - 991"
  },
  {
    "REF": 82,
    "EN_NAME": "The Ancient Ancestral Legacy $",
    "EN_DESC": "|*Whenever you deal Critical Strike Damage, you gain an <Ancient Legacy> stack granting you +@ £.#Whenever you deal Ancestral Strike Damage, you lose your <Ancient Legacy> stacks.",
    "EN_LORE": "\"...this Slorm Reaper was meant for Gazloka, a lazy champion to say the least...\"",
    "EN": "The Ancient Ancestral Legacy $//|*Whenever you deal Critical Strike Damage, you gain an <Ancient Legacy> stack granting you +@ £.#Whenever you deal Ancestral Strike Damage, you lose your <Ancient Legacy> stacks.//\"...this Slorm Reaper was meant for Gazloka, a lazy champion to say the least...\"",
    "FR": "$ Ancien d'Héritage Ancestral/$ Ancienne d'Héritage Ancestral//|*Lorsque vous infligez des Dégats Critiques, vous gagnez un cumul d'<Héritage des Anciens>, vous conférant +@ de £.#Lorsque vous infligez des Dégâts Ancestraux, vous perdez vos cumuls d'<Héritage des Anciens>.//\"...ce Slorm Reaper était destiné à Gazloka, un Champion pour le moins paresseux...\"",
    "FR_NAME": "$ Ancien d'Héritage Ancestral/$ Ancienne d'Héritage Ancestral",
    "FR_DESC": "|*Lorsque vous infligez des Dégats Critiques, vous gagnez un cumul d'<Héritage des Anciens>, vous conférant +@ de £.#Lorsque vous infligez des Dégâts Ancestraux, vous perdez vos cumuls d'<Héritage des Anciens>.",
    "FR_LORE": "\"...ce Slorm Reaper était destiné à Gazloka, un Champion pour le moins paresseux...\"",
    "CH": "古老先祖遗物$//|*造成暴击伤害时，获得一层<Ancient Legacy>，为你赋予+@£。#造成先祖攻击伤害时，失去<Ancient Legacy>层数。//“……这件猎魂武装是为加兹洛卡准备的，往好了说，这是个懒惰的勇者……”",
    "CH_NAME": "古老先祖遗物$",
    "CH_DESCRIPTION": "|*造成暴击伤害时，获得一层<Ancient Legacy>，为你赋予+@£。#造成先祖攻击伤害时，失去<Ancient Legacy>层数。",
    "CH_LORE": "“……这件猎魂武装是为加兹洛卡准备的，往好了说，这是个懒惰的勇者……”",
    "TW": "古老先祖遺物$//|*造成暴擊傷害時，獲得一層<Ancient Legacy>，為你賦予+@£。 #造成先祖攻擊傷害時，失去<Ancient Legacy>層數。//“……這件獵魂武裝是為加茲洛卡準備的，往好了說，這是個懶惰的勇者……”",
    "TW_NAME": "古老先祖遺物$",
    "TW_DESCRIPTION": "|*造成暴擊傷害時，獲得一層<Ancient Legacy>，為你賦予+@£。 #造成先祖攻擊傷害時，失去<Ancient Legacy>層數。",
    "TW_LORE": "“……這件獵魂武裝是為加茲洛卡準備的，往好了說，這是個懶惰的勇者……”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 82,
    "SPRITE": 82,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "brut_chance_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 26,
    "BASE_DMG_MAX": 34,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 68,
    "EVOLVE_IN": 83,
    "100": "824 - 1179"
  },
  {
    "REF": 83,
    "EN_NAME": "The Primal Ancient Ancestral Legacy $",
    "EN_DESC": "|*Whenever you deal Ancestral Strike Damage, you gain <Elemental Fervor>, granting you +_% £ for the next @ seconds.#Elemental Damage Multiplier is equal to your $.",
    "EN_LORE": "\"... who would prefer to do as little as possible... In fact, he died of inaction after taking a nap for more than 27 days. A record. \"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "EN": "The Primal Ancient Ancestral Legacy $//|*Whenever you deal Ancestral Strike Damage, you gain <Elemental Fervor>, granting you +_% £ for the next @ seconds.#Elemental Damage Multiplier is equal to your $.//\"... who would prefer to do as little as possible... In fact, he died of inaction after taking a nap for more than 27 days. A record. \"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "FR": "$ Ancien Primal d'Héritage Ancestral/$ Ancienne Primale d'Héritage Ancestral//|*Lorsque vous infligez des Dégats Ancestraux, vous gagnez <Ferveur Elementaire>, vous conférant +_% de £ pour les @ prochaines secondes.#Ce Multiplicateur de Dégâts Élémentaires est équivalent à votre $.//\"... qui préférait en faire le moins possible... Il mourut d'ailleurs d'inaction après avoir fait une sieste de plus de 27 jours. Un record.\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "FR_NAME": "$ Ancien Primal d'Héritage Ancestral/$ Ancienne Primale d'Héritage Ancestral",
    "FR_DESC": "|*Lorsque vous infligez des Dégats Ancestraux, vous gagnez <Ferveur Elementaire>, vous conférant +_% de £ pour les @ prochaines secondes.#Ce Multiplicateur de Dégâts Élémentaires est équivalent à votre $.",
    "FR_LORE": "\"... qui préférait en faire le moins possible... Il mourut d'ailleurs d'inaction après avoir fait une sieste de plus de 27 jours. Un record.\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "CH": "原始古老先祖遗物$//|*“造成先祖攻击伤害时，获得<Elemental Fervor>，为你赋予+_%£，持续@秒。#元素伤害倍率等于你的$。//”……有谁会愿意一事无成呢……事实上，他死于连续27天的呼呼大睡。真是创纪录了。”#- 《亚拉勇者，英雄还是小人》，亚当·诺斯图斯著",
    "CH_NAME": "原始古老先祖遗物$",
    "CH_DESCRIPTION": "|*“造成先祖攻击伤害时，获得<Elemental Fervor>，为你赋予+_%£，持续@秒。#元素伤害倍率等于你的$。",
    "CH_LORE": "”……有谁会愿意一事无成呢……事实上，他死于连续27天的呼呼大睡。真是创纪录了。”#- 《亚拉勇者，英雄还是小人》，亚当·诺斯图斯著",
    "TW": "原始古老先祖遺物$//|*“造成先祖攻擊傷害時，獲得<Elemental Fervor>，為你賦予+_%£，持續@秒。#元素傷害倍率等於你的$。//”……有誰會願意一事無成呢……事實上，他死於連續27天的呼呼大睡。真是創紀錄了。”#- 《亞拉勇者，英雄還是小人》，亞當·諾斯圖斯著",
    "TW_NAME": "原始古老先祖遺物$",
    "TW_DESCRIPTION": "|*“造成先祖攻擊傷害時，獲得<Elemental Fervor>，為你賦予+_%£，持續@秒。#元素傷害倍率等於你的$。",
    "TW_LORE": "”……有誰會願意一事無成呢……事實上，他死於連續27天的呼呼大睡。真是創紀錄了。”#- 《亞拉勇者，英雄還是小人》，亞當·諾斯圖斯著",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 83,
    "SPRITE": 83,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "10",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "elemental_damage_global_mult",
    "VALUE_REAL": "synergy:crit_chance:1",
    "BASE_DMG_MIN": 26,
    "BASE_DMG_MAX": 34,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1196 - 1704"
  },
  {
    "REF": 84,
    "EN_NAME": "Pure $ of the Temple Keeper",
    "EN_DESC": "*+@ £*+@ £*+@ £|*The Attack Speed of the Mini-Keeper is equal to yours.*act:221*act:222/\n|*Mini-Keeper deals _% Increased Elemental Damage.#Increased Elemental Damage is equal to your Highest Floor reached in The Slorm Temple./\n|*You can no longer benefit from Ultimatums.",
    "EN_LORE": "A Baby-Keeper.#Look how cute he is!",
    "EN": "Pure $ of the Temple Keeper//*+@ £*+@ £*+@ £|*The Attack Speed of the Mini-Keeper is equal to yours.*act:221*act:222/\n|*Mini-Keeper deals _% Increased Elemental Damage.#Increased Elemental Damage is equal to your Highest Floor reached in The Slorm Temple./\n|*You can no longer benefit from Ultimatums.//A Baby-Keeper.#Look how cute he is!",
    "FR": "$ Pur du Gardien du Temple/$ Pure du Gardien du Temple//*+@ £*+@ £*+@ £*|La Vitesse d'Attaque du Mini-Gardien est équivalente à la vôtre.*act:221*act:222/\n|*Le Mini-Gardien inflige _% de Dégats Éléméntaires Supplémentaires.#Ces Dégâts Supplémentaires sont équivalents à votre Étage atteint le plus élevé dans le Temple du Slorm./\n|*Vous ne pouvez plus bénéficier des Ultimatums.//Un bébé-Gardien.#Comme il est mignon !",
    "FR_NAME": "$ Pur du Gardien du Temple/$ Pure du Gardien du Temple",
    "FR_DESC": "*+@ £*+@ £*+@ £*|La Vitesse d'Attaque du Mini-Gardien est équivalente à la vôtre.*act:221*act:222/\n|*Le Mini-Gardien inflige _% de Dégats Éléméntaires Supplémentaires.#Ces Dégâts Supplémentaires sont équivalents à votre Étage atteint le plus élevé dans le Temple du Slorm./\n|*Vous ne pouvez plus bénéficier des Ultimatums.",
    "FR_LORE": "Un bébé-Gardien.#Comme il est mignon !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 84,
    "SPRITE": 84,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|1",
    "VALUE_TYPE": "%|%|%",
    "VALUE_LEVEL": "rl|rl|rl",
    "VALUE_STAT": "minion_increased_damage_percent|totem_increased_effect_percent|pure_slorm",
    "VALUE_REAL": "synergy:highest_floor:1",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 24,
    "MIN_DMG_LVL": 7,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1307 - 1825"
  },
  {
    "REF": 85,
    "EN_NAME": "Elder $ of the Slorm Temple",
    "EN_DESC": "*+@ £ Find*|*Non-Elite enemies have @ Chance to drop Elder Slorm upon dying.*Your Primary and Secondary Skills deal _ £.#Additional Damage is equal to @ of your current Elder Slorm.*Picking up Elder Slorm triggers Ultima-Momentum, granting you a +@ Multiplier applied to your Ultimatum for the next @ seconds./\n|*Whenever you pick up Elder Slorm, you trigger an Elder Inner Fire, dealing _ £.#Additional Damage is equal to @ of your current Elder Slorm./\n|*Inner Fire Max. Charges is set to @.*Inner Fires rotate on the same Orbit.",
    "EN_LORE": "\"Is that all?! At that price?!\" - You, right now.",
    "EN": "Elder $ of the Slorm Temple//*+@ £ Find*|*Non-Elite enemies have @ Chance to drop Elder Slorm upon dying.*Your Primary and Secondary Skills deal _ £.#Additional Damage is equal to @ of your current Elder Slorm.*Picking up Elder Slorm triggers Ultima-Momentum, granting you a +@ Multiplier applied to your Ultimatum for the next @ seconds./\n|*Whenever you pick up Elder Slorm, you trigger an Elder Inner Fire, dealing _ £.#Additional Damage is equal to @ of your current Elder Slorm./\n|*Inner Fire Max. Charges is set to @.*Inner Fires rotate on the same Orbit.//\"Is that all?! At that price?!\" - You, right now.",
    "FR": "$ Ancien du Temple du Slorm/$ Ancienne du Temple du Slorm//*+@ £ Find*|*Les ennemis non-Elite ont @ de Chance de laisser tomber du Slorm Ancien lorsqu'ils meurent.*Vos Compétences Primaires et Secondaires infligent _ de £.#Ces Dégats Supplémentaires sont équivalents à @ de votre montant actuel de Slorm Ancien.*Ramasser du Slorm Ancien déclenche Ultima-Momentum, conférant un Multiplicateur de +@ à votre Ultimatum pour les @ prochaines secondes./\n|*Lorsque vous ramassez du Slorm Ancien, vous déclenchez un Feu Intérieur Ancien, infligeant _ de £.#Ces Dégats Supplémentaires sont équivalent à @ de votre montant actuel de Slorm Ancien./\n|*Vos Charges Max. de Feu Intérieur sont deviennent @.*Vos Feux Intérieurs effectuent leur rotation sur la même orbite.//\"C'est tout ?! Au prix que ça coûte ?!\"#- Vous, en ce moment-même.",
    "FR_NAME": "$ Ancien du Temple du Slorm/$ Ancienne du Temple du Slorm",
    "FR_DESC": "*+@ £ Find*|*Les ennemis non-Elite ont @ de Chance de laisser tomber du Slorm Ancien lorsqu'ils meurent.*Vos Compétences Primaires et Secondaires infligent _ de £.#Ces Dégats Supplémentaires sont équivalents à @ de votre montant actuel de Slorm Ancien.*Ramasser du Slorm Ancien déclenche Ultima-Momentum, conférant un Multiplicateur de +@ à votre Ultimatum pour les @ prochaines secondes./\n|*Lorsque vous ramassez du Slorm Ancien, vous déclenchez un Feu Intérieur Ancien, infligeant _ de £.#Ces Dégats Supplémentaires sont équivalent à @ de votre montant actuel de Slorm Ancien./\n|*Vos Charges Max. de Feu Intérieur sont deviennent @.*Vos Feux Intérieurs effectuent leur rotation sur la même orbite.",
    "FR_LORE": "\"C'est tout ?! Au prix que ça coûte ?!\"#- Vous, en ce moment-même.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 85,
    "SPRITE": 85,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|2|0.15|1|12|\n0.15|\n5",
    "VALUE_TYPE": "%|%|%|%||\n%|\n",
    "VALUE_LEVEL": "rl|rl||rl||\nrl|",
    "VALUE_STAT": "corrupted_slorm|additional_damage|additional_damage",
    "VALUE_REAL": "synergy:corrupted_slorm:25|synergy:corrupted_slorm:25",
    "BASE_DMG_MIN": 16,
    "BASE_DMG_MAX": 22,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1629 - 2363"
  },
  {
    "REF": 86,
    "EN_NAME": "$ of Ancestral Harmony",
    "EN_DESC": "*+@ £|*Your Ancestral Strike Chance is set to _%.#Ancestral Strike Chance is equal to @ of your Ancestral Strike Damage.*Ancestral Strike Effects directly adjacent to an unlocked Ancestral Skill or Passive are automatically Equipped and at Max Rank.*Whenever you deal Ancestral Strike Damage to an enemy, you gain an Ancestral Command stack, up to @.*act:252/\n|*While in combat, you gain an Ancestral Preparation stack every second, granting +@ £ per stack.*Whenever you deal Ancestral Strike Damage, you lose your Ancestral Preparation stacks./\n|*Effects triggering with Critical Strike Damage can no longer trigger.",
    "EN_LORE": "The ancients whisper through this Reaper, creating symphonies of legends.",
    "EN": "$ of Ancestral Harmony//*+@ £|*Your Ancestral Strike Chance is set to _%.#Ancestral Strike Chance is equal to @ of your Ancestral Strike Damage.*Ancestral Strike Effects directly adjacent to an unlocked Ancestral Skill or Passive are automatically Equipped and at Max Rank.*Whenever you deal Ancestral Strike Damage to an enemy, you gain an Ancestral Command stack, up to @.*act:252/\n|*While in combat, you gain an Ancestral Preparation stack every second, granting +@ £ per stack.*Whenever you deal Ancestral Strike Damage, you lose your Ancestral Preparation stacks./\n|*Effects triggering with Critical Strike Damage can no longer trigger.//The ancients whisper through this Reaper, creating symphonies of legends.",
    "FR": "$ d'Harmonie Ancestrale//*+@ £|*Votre Chance de Coup Ancestral devient _%.#Cette Chance de Coup Ancestral est égale à @ de vos Dégâts Ancestraux.*Les Effets de Coup Ancestral directement adjacents à une Compétence Ancestrale ou à une Passive déverrouillée sont automatiquement Équipés et au Rang Max.*Chaque fois que vous infligez des Dégâts Ancestraux à un ennemi, vous gagnez une charge de Commandement Ancestral, jusqu'à @.*act:252/\n|*En combat, vous gagnez un cumul de Préparation Ancestrale chaque seconde, vous conférant +@ de £ par cumul.*Chaque fois que vous infligez des Dégâts Ancestraux, vous perdez vos cumuls de Préparation Ancestrale./\n|*Les Effets se déclenchant avec des Dégâts Critiques ne peuvent plus se déclencher.//Les anciens murmurent à travers ce Reaper, créant des symphonies de légendes.",
    "FR_NAME": "$ d'Harmonie Ancestrale",
    "FR_DESC": "*+@ £|*Votre Chance de Coup Ancestral devient _%.#Cette Chance de Coup Ancestral est égale à @ de vos Dégâts Ancestraux.*Les Effets de Coup Ancestral directement adjacents à une Compétence Ancestrale ou à une Passive déverrouillée sont automatiquement Équipés et au Rang Max.*Chaque fois que vous infligez des Dégâts Ancestraux à un ennemi, vous gagnez une charge de Commandement Ancestral, jusqu'à @.*act:252/\n|*En combat, vous gagnez un cumul de Préparation Ancestrale chaque seconde, vous conférant +@ de £ par cumul.*Chaque fois que vous infligez des Dégâts Ancestraux, vous perdez vos cumuls de Préparation Ancestrale./\n|*Les Effets se déclenchant avec des Dégâts Critiques ne peuvent plus se déclencher.",
    "FR_LORE": "Les anciens murmurent à travers ce Reaper, créant des symphonies de légendes.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 86,
    "SPRITE": 36,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|2|10|\n5",
    "VALUE_TYPE": "%|%||\n%",
    "VALUE_LEVEL": "rl|||\n",
    "VALUE_STAT": "brut_damage_percent|\nbrut_damage_percent",
    "VALUE_REAL": "synergy:ancestral_damage:0.02",
    "BASE_DMG_MIN": 22,
    "BASE_DMG_MAX": 28,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 7,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "979 - 1331"
  },
  {
    "REF": 87,
    "EN_NAME": "$ of Deadly Accuracy",
    "EN_DESC": "*+@ £*+@ £|act:223/\n|*Damage from enemies not affected by Armor Broken is reduced to 0.*You apply Armor Broken to enemies that hit you.*act:224/\n|*Enemies affected by Armor Broken deal @ £.",
    "EN_LORE": "You can just throw hits in random directions, this Reaper takes care of the rest.",
    "EN": "$ of Deadly Accuracy//*+@ £*+@ £|act:223/\n|*Damage from enemies not affected by Armor Broken is reduced to 0.*You apply Armor Broken to enemies that hit you.*act:224/\n|*Enemies affected by Armor Broken deal @ £.//You can just throw hits in random directions, this Reaper takes care of the rest.",
    "FR": "$ de Précision Mortelle//*+@ £*+@ £|act:223/\n|*Les Dégats provenant d'ennemis qui ne sont pas affectés par Armure Brisée sont réduits à 0.*Vous appliquez Armure Brisée aux ennemis qui vous touchent.*act:224/\n|*Les ennemis affectés par Armure brisée infligent @ de £.//Vous pouvez taper n'importe comment, le Reaper s'occupe du reste.",
    "FR_NAME": "$ de Précision Mortelle",
    "FR_DESC": "*+@ £*+@ £|act:223/\n|*Les Dégats provenant d'ennemis qui ne sont pas affectés par Armure Brisée sont réduits à 0.*Vous appliquez Armure Brisée aux ennemis qui vous touchent.*act:224/\n|*Les ennemis affectés par Armure brisée infligent @ de £.",
    "FR_LORE": "Vous pouvez taper n'importe comment, le Reaper s'occupe du reste.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 87,
    "SPRITE": 87,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.25|0.5|\n100",
    "VALUE_TYPE": "%|%|\n%",
    "VALUE_LEVEL": "rl|rl|",
    "VALUE_STAT": "armor_penetration_percent|armor_penetration_global_mult|\nincreased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 23,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": 1,
    "MAX_LVL": 36,
    "EVOLVE_IN": 88,
    "100": "1404 - 2585"
  },
  {
    "REF": 88,
    "EN_NAME": "Superior $ of Deadly Accuracy",
    "EN_DESC": "|*Whenever you deal damage to an enemy affected by Armor Broken, you have _% Chance to apply Trauma. Chance is equal to your Armor Penetration.*Trauma deals _ £ over @ seconds.",
    "EN_LORE": "What doesn't kill you kills you later.",
    "EN": "Superior $ of Deadly Accuracy//|*Whenever you deal damage to an enemy affected by Armor Broken, you have _% Chance to apply Trauma. Chance is equal to your Armor Penetration.*Trauma deals _ £ over @ seconds.//What doesn't kill you kills you later.",
    "FR": "$ Supérieur de Précision Mortelle/$ Supérieure de Précision Mortelle//|*Lorsque vous infligez des Dégats à un ennemi affecté par Armure Brisée, vous avez _% de Chance de lui appliquer Trauma. Cette Chance est équivalente à votre Pénétration d'Armure.*Trauma inflige _ de £ en @ seconds.//Ce qui ne vous tue pas vous tue plus tard.",
    "FR_NAME": "$ Supérieur de Précision Mortelle/$ Supérieure de Précision Mortelle",
    "FR_DESC": "|*Lorsque vous infligez des Dégats à un ennemi affecté par Armure Brisée, vous avez _% de Chance de lui appliquer Trauma. Cette Chance est équivalente à votre Pénétration d'Armure.*Trauma inflige _ de £ en @ seconds.",
    "FR_LORE": "Ce qui ne vous tue pas vous tue plus tard.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 88,
    "SPRITE": 88,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "5|3",
    "VALUE_TYPE": "|%",
    "VALUE_LEVEL": "|rl",
    "VALUE_STAT": "physical_damage",
    "VALUE_REAL": "synergy:armor_penetration:1|damage:physical_damage:3*rl",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 23,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": null,
    "MAX_LVL": 52,
    "EVOLVE_IN": 89,
    "100": "1691 - 2864"
  },
  {
    "REF": 89,
    "EN_NAME": "Almighty Superior $ of Deadly Accuracy",
    "EN_DESC": "|*Whenever you apply Armor Broken to an enemy affected by Trauma, you deal @ of the enemy's current Life as Skill Damage.",
    "EN_LORE": "Armor Penetration: Underrated.",
    "EN": "Almighty Superior $ of Deadly Accuracy//|*Whenever you apply Armor Broken to an enemy affected by Trauma, you deal @ of the enemy's current Life as Skill Damage.//Armor Penetration: Underrated.",
    "FR": "$ Supérieur Tout-Puissant de Précision Mortelle/$ Supérieure Toute-Puissante de Précision Mortelle//|*Lorsque vous appliquez Armure Brisée à un ennemi affecté par Trauma, vous infligez @ de la Vie actuelle de l'ennemi en tant que Dégâts Physiques.//La Pénétration d'Armure : Sous-côté.",
    "FR_NAME": "$ Supérieur Tout-Puissant de Précision Mortelle/$ Supérieure Toute-Puissante de Précision Mortelle",
    "FR_DESC": "|*Lorsque vous appliquez Armure Brisée à un ennemi affecté par Trauma, vous infligez @ de la Vie actuelle de l'ennemi en tant que Dégâts Physiques.",
    "FR_LORE": "La Pénétration d'Armure : Sous-côté.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 89,
    "SPRITE": 89,
    "BLACKSMITH": 1,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "25",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 18,
    "BASE_DMG_MAX": 23,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1986 - 2864"
  },
  {
    "REF": 90,
    "EN_NAME": "Shockwave $ of Havoc",
    "EN_DESC": "*+@ £*+@ £|*Whenever you deal damage with your Primary or Secondary Skill, you have a  @ £ to trigger a Shockwave dealing _ £ to enemies behind the first enemy hit, and then gain a Wreak Havoc stack, up to @. You then lose all your Wreak Havoc stacks if you've reached the stack limit.*For each stack of Wreak Havoc, the Shockwave gains:#+@ Increased Size#+@ £.*Effects of this Slorm Reaper are affected by your highest Slorm Reaper Affinity instead of its base one, outside of Equipment bonuses./\n|*Effects of this Slorm Reaper are now affected by the sum of all your Slorm Reaper Affinities instead of its base one, outside of Equipment bonuses./\n|*When Wreak Havoc is active, you have @ Attack Speed Multiplier. This Multiplier is reduced by @ for every Wreak Havoc stack you have.",
    "EN_LORE": "Dancing to the rhythm of anarchy, turning every strike into a symphony of disorder.",
    "EN": "Shockwave $ of Havoc//*+@ £*+@ £|*Whenever you deal damage with your Primary or Secondary Skill, you have a  @ £ to trigger a Shockwave dealing _ £ to enemies behind the first enemy hit, and then gain a Wreak Havoc stack, up to @. You then lose all your Wreak Havoc stacks if you've reached the stack limit.*For each stack of Wreak Havoc, the Shockwave gains:#+@ Increased Size#+@ £.*Effects of this Slorm Reaper are affected by your highest Slorm Reaper Affinity instead of its base one, outside of Equipment bonuses./\n|*Effects of this Slorm Reaper are now affected by the sum of all your Slorm Reaper Affinities instead of its base one, outside of Equipment bonuses./\n|*When Wreak Havoc is active, you have @ Attack Speed Multiplier. This Multiplier is reduced by @ for every Wreak Havoc stack you have.//Dancing to the rhythm of anarchy, turning every strike into a symphony of disorder.",
    "FR": "$ d'Onde de Choc du Chaos//*+@ £*+@ £|*Chaque fois que vous infligez des dégâts avec une Compétence Primaire ou Secondaire, vous avez @ de £ de déclencher une Onde de Choc infligeant _ de £ aux ennemis derrière le premier ennemi touché et de gagner un cumul de Faire des Ravages, jusqu'à @. Vous perdez ensuite tous vos cumuls de Faire des Ravages si vous atteignez la limite de cumuls.*Pour chaque cumul de Faire des Ravages, l'Onde de Choc gagne :#+@ de Taille#+@ de £.*Les effets de ce Slorm Reaper sont affectés par votre plus haute Affinité de Slorm Reaper au lieu de celle de base, en dehors des bonus d'équipement./\n|*Les effets de ce Slorm Reaper sont désormais affectés par la somme de toutes vos Affinités de Slorm Reapers au lieu de celle de base, en dehors des bonus d'équipement./\n|*Lorsque la Faire des Ravages est active, vous avez un Multiplicateur de Vitesse d'Attaque de @. Ce multiplicateur est réduit de @ pour chaque cumul de Faire des Ravages que vous avez.//Danser au rythme de l'anarchie, et transformer chaque coup en une symphonie de désordre.",
    "FR_NAME": "$ d'Onde de Choc du Chaos",
    "FR_DESC": "*+@ £*+@ £|*Chaque fois que vous infligez des dégâts avec une Compétence Primaire ou Secondaire, vous avez @ de £ de déclencher une Onde de Choc infligeant _ de £ aux ennemis derrière le premier ennemi touché et de gagner un cumul de Faire des Ravages, jusqu'à @. Vous perdez ensuite tous vos cumuls de Faire des Ravages si vous atteignez la limite de cumuls.*Pour chaque cumul de Faire des Ravages, l'Onde de Choc gagne :#+@ de Taille#+@ de £.*Les effets de ce Slorm Reaper sont affectés par votre plus haute Affinité de Slorm Reaper au lieu de celle de base, en dehors des bonus d'équipement./\n|*Les effets de ce Slorm Reaper sont désormais affectés par la somme de toutes vos Affinités de Slorm Reapers au lieu de celle de base, en dehors des bonus d'équipement./\n|*Lorsque la Faire des Ravages est active, vous avez un Multiplicateur de Vitesse d'Attaque de @. Ce multiplicateur est réduit de @ pour chaque cumul de Faire des Ravages que vous avez.",
    "FR_LORE": "Danser au rythme de l'anarchie, et transformer chaque coup en une symphonie de désordre.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 90,
    "SPRITE": 90,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "7|7|15|4|0.3|0.3|\n-100|25",
    "VALUE_TYPE": "||%|||%|\n%|%",
    "VALUE_LEVEL": "rl|rl|||rl|rl|\n|",
    "VALUE_STAT": "the_max_health_add|the_max_mana_add|chance|physical_damage|chance|increased_damage",
    "VALUE_REAL": "damage:physical_damage:150",
    "BASE_DMG_MIN": 24,
    "BASE_DMG_MAX": 36,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1663 - 2197"
  },
  {
    "REF": 91,
    "EN_NAME": "$ of the Giant Slayer",
    "EN_DESC": "*+@ £*+@ £|*Whenever you deal damage to an Elite enemy with your Primary or Secondary Skill, the same damage is also dealt to a random enemy nearby.*Whenever you deal damage, you apply Enfeeble to enemies hit.#Enfeeble increases incoming Damage by @ and can stack up to @ times./\n|*You have +@ Cooldown Reduction for every Enfeeble stack within a @ yard radius.*Whenever you kill an Elite enemy, your current Cooldown Reduction bonus becomes permanent for the next @ seconds./\n|*You cannot gain Attack Speed from Normal, Magic, Rare or Epic Equipment stats.",
    "EN_LORE": "\"What doesn't glow in the dark and doesn't have interesting loot?\"...",
    "EN": "$ of the Giant Slayer//*+@ £*+@ £|*Whenever you deal damage to an Elite enemy with your Primary or Secondary Skill, the same damage is also dealt to a random enemy nearby.*Whenever you deal damage, you apply Enfeeble to enemies hit.#Enfeeble increases incoming Damage by @ and can stack up to @ times./\n|*You have +@ Cooldown Reduction for every Enfeeble stack within a @ yard radius.*Whenever you kill an Elite enemy, your current Cooldown Reduction bonus becomes permanent for the next @ seconds./\n|*You cannot gain Attack Speed from Normal, Magic, Rare or Epic Equipment stats.//\"What doesn't glow in the dark and doesn't have interesting loot?\"...",
    "FR": "$ du Tueur de Géants//*+@ £*+@ £|*Lorsque vous infligez des Dégâts à un ennemi Élite avec une Compétence Primaire ou Secondaire, ces mêmes Dégâts sont infligés à un ennemi aléatoire proche.*Lorsque vous infligez des Dégats, vous appliquez Ramolli aux ennemis touchés.#Rammoli augmente les Dégâts reçus de @ et peut se cumuler jusqu'à @ fois./\n|*Vous avez +@ de Réduction de Temps de Recharge pour chaque cumul de Ramolli dans un rayon de @ mètres.*Lorsque vous tuez un ennemi Élite, votre bonus actuel de Réduction de Temps de Recharge devient permanent pour les @ prochaines secondes./\n|*Vous ne pouvez plus gagner de Vitesse d'Attaque grâce à des stats d'Équipement Normales, Magiques, Rares ou Épiques.//\"Qu'est-ce qui ne brille pas dans la nuit et n'a pas de butin intéressant ?\"...",
    "FR_NAME": "$ du Tueur de Géants",
    "FR_DESC": "*+@ £*+@ £|*Lorsque vous infligez des Dégâts à un ennemi Élite avec une Compétence Primaire ou Secondaire, ces mêmes Dégâts sont infligés à un ennemi aléatoire proche.*Lorsque vous infligez des Dégats, vous appliquez Ramolli aux ennemis touchés.#Rammoli augmente les Dégâts reçus de @ et peut se cumuler jusqu'à @ fois./\n|*Vous avez +@ de Réduction de Temps de Recharge pour chaque cumul de Ramolli dans un rayon de @ mètres.*Lorsque vous tuez un ennemi Élite, votre bonus actuel de Réduction de Temps de Recharge devient permanent pour les @ prochaines secondes./\n|*Vous ne pouvez plus gagner de Vitesse d'Attaque grâce à des stats d'Équipement Normales, Magiques, Rares ou Épiques.",
    "FR_LORE": "\"Qu'est-ce qui ne brille pas dans la nuit et n'a pas de butin intéressant ?\"...",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 91,
    "SPRITE": 91,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.25|0.2|10|\n1|5|12|",
    "VALUE_TYPE": "%|%|%||\n%||",
    "VALUE_LEVEL": "rl|rl|rl||\n|||",
    "VALUE_STAT": "increased_damage_on_elite_percent|reduced_damage_on_elite_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 25,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 2,
    "MAX_LVL": 68,
    "EVOLVE_IN": 92,
    "100": "1184 - 1695"
  },
  {
    "REF": 92,
    "EN_NAME": "Giant $ of the Giant Slayer",
    "EN_DESC": "|*Whenever you kill a Non-Elite enemy, you gain an Apex Predator stack granting you +@ £ for the next @ seconds.#Apex Predator can stack up to @ times.*Whenever you kill an Elite enemy, you lose your Apex Predator stacks and destroy every Non-Elite enemy affected by Enfeeble.",
    "EN_LORE": "... \"A regular enemy.\"#- From Elite to Elite, a compilation of Elite jokes for exceptional enemies.",
    "EN": "Giant $ of the Giant Slayer//|*Whenever you kill a Non-Elite enemy, you gain an Apex Predator stack granting you +@ £ for the next @ seconds.#Apex Predator can stack up to @ times.*Whenever you kill an Elite enemy, you lose your Apex Predator stacks and destroy every Non-Elite enemy affected by Enfeeble.//... \"A regular enemy.\"#- From Elite to Elite, a compilation of Elite jokes for exceptional enemies.",
    "FR": "$ Gigantesque du Tueur de Géants//|*Chaque fois que vous tuez un ennemi non-Elite, vous gagnez un cumul de Prédateur Alpha vous conférant +@ de £ pour les @ prochaines secondes.#Prédateur Alpha peut se cumuler jusqu'à @ fois.*Chaque fois que vous tuez un ennemi Elite, vous perdez vos cumuls de Prédateur Alpha et détruisez chaque ennemi affecté par Ramolli.//... \"Un ennemi normal.\"#- D'Élite à Élite, recueil de blagues d'Élite pour ennemis exceptionnels.",
    "FR_NAME": "$ Gigantesque du Tueur de Géants",
    "FR_DESC": "|*Chaque fois que vous tuez un ennemi non-Elite, vous gagnez un cumul de Prédateur Alpha vous conférant +@ de £ pour les @ prochaines secondes.#Prédateur Alpha peut se cumuler jusqu'à @ fois.*Chaque fois que vous tuez un ennemi Elite, vous perdez vos cumuls de Prédateur Alpha et détruisez chaque ennemi affecté par Ramolli.",
    "FR_LORE": "... \"Un ennemi normal.\"#- D'Élite à Élite, recueil de blagues d'Élite pour ennemis exceptionnels.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 92,
    "SPRITE": 92,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|10|1",
    "VALUE_TYPE": "%||",
    "VALUE_LEVEL": "||rl",
    "VALUE_STAT": "increased_damage_on_elite_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 26,
    "BASE_DMG_MAX": 31,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1196 - 1696"
  },
  {
    "REF": 93,
    "EN_NAME": "$ of the Mathematician",
    "EN_DESC": "|*Probability:#If a Chance-related stat is equal to @, it is equal to @ instead. Those are Inner Fire Chance, Overdrive Chance, Recast Chance, Pierce Chance, Fork Chance, Rebound Chance, Critical Strike Chance, Ancestral Strike Chance.*Stats Affected by Probability then gain a +@ Multiplier.*act:253/\n|*Function:#Whenever you cast Thalagoras Triangle, the Max Life of each Non-Elite enemy inside the Triangle is lowered to the lowest Max Life./\n|*Miscalculation:#Percentage Stats below @ are rounded down to @.#This effect occurs before the calculation of the Probability effect.",
    "EN_LORE": "a²+b² = c²",
    "EN": "$ of the Mathematician//|*Probability:#If a Chance-related stat is equal to @, it is equal to @ instead. Those are Inner Fire Chance, Overdrive Chance, Recast Chance, Pierce Chance, Fork Chance, Rebound Chance, Critical Strike Chance, Ancestral Strike Chance.*Stats Affected by Probability then gain a +@ Multiplier.*act:253/\n|*Function:#Whenever you cast Thalagoras Triangle, the Max Life of each Non-Elite enemy inside the Triangle is lowered to the lowest Max Life./\n|*Miscalculation:#Percentage Stats below @ are rounded down to @.#This effect occurs before the calculation of the Probability effect.//a²+b² = c²",
    "FR": "$ du Mathématicien//|*Probabilité :#Si une statistique de Chance est égale à @, elle est égale à @ à la place. Il s'agit de la Chance de Feu Intérieur, de la Chance de Surcharge, de la Chance de Relance, de la Chance de Percer, de la Chance de Fourcher, de la Chance de Rebond, de la Chance de Coup Critique, de la Chance de Coup Ancestral.*Les statistiques affectées par Probabilité bénéficient ensuite d'un Multiplicateur de +@.*act:253/\n|*Fonction :#Chaque fois que vous lancez le Triangle de Thalagore, la Vie Max de chaque ennemi non-Élite à l'intérieur du Triangle est réduite à la Vie Max la plus basse parmis les ennemis touchés./\n|*Erreur de Calcul :#Les statistiques en pourcentage inférieures à @ sont arrondies à @.#Cet effet se produit avant le calcul de l'effet de Probabilité.//a²+b² = c²",
    "FR_NAME": "$ du Mathématicien",
    "FR_DESC": "|*Probabilité :#Si une statistique de Chance est égale à @, elle est égale à @ à la place. Il s'agit de la Chance de Feu Intérieur, de la Chance de Surcharge, de la Chance de Relance, de la Chance de Percer, de la Chance de Fourcher, de la Chance de Rebond, de la Chance de Coup Critique, de la Chance de Coup Ancestral.*Les statistiques affectées par Probabilité bénéficient ensuite d'un Multiplicateur de +@.*act:253/\n|*Fonction :#Chaque fois que vous lancez le Triangle de Thalagore, la Vie Max de chaque ennemi non-Élite à l'intérieur du Triangle est réduite à la Vie Max la plus basse parmis les ennemis touchés./\n|*Erreur de Calcul :#Les statistiques en pourcentage inférieures à @ sont arrondies à @.#Cet effet se produit avant le calcul de l'effet de Probabilité.",
    "FR_LORE": "a²+b² = c²",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 93,
    "SPRITE": 50,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0|5|1|\n25|0",
    "VALUE_TYPE": "%|%|%|\n%|%",
    "VALUE_LEVEL": "||rl|\n|",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 16,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 4,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 2,
    "MAX_LVL": 16,
    "EVOLVE_IN": 94,
    "100": "1150 - 2170"
  },
  {
    "REF": 94,
    "EN_NAME": "$ of the Statistician",
    "EN_DESC": "|*Algebra:#If your Critical Strike Chance is equal to your Ancestral Strike Chance, your Critical Strike Damage becomes equal to your Ancestral Strike Damage.*Isoperimetry:#Enemies hit by the sides of Thalagoras Triangle are Stunned for the next @ seconds.",
    "EN_LORE": "syn ² x + cos² x = 1",
    "EN": "$ of the Statistician//|*Algebra:#If your Critical Strike Chance is equal to your Ancestral Strike Chance, your Critical Strike Damage becomes equal to your Ancestral Strike Damage.*Isoperimetry:#Enemies hit by the sides of Thalagoras Triangle are Stunned for the next @ seconds.//syn ² x + cos² x = 1",
    "FR": "$ du Statisticien//|*Algèbre :#Si votre Chance de Coup Critique est égale à votre Chance de Coup Ancestral, vos Dégâts de Coup Critique deviennent égaux à vos Dégâts de Coup Ancestral.*Isopérimétrie :#Les ennemis touchés par les côtés du Triangle de Thalagore sont Étourdis pour les @ prochaines secondes.//syn ² x + cos² x = 1",
    "FR_NAME": "$ du Statisticien",
    "FR_DESC": "|*Algèbre :#Si votre Chance de Coup Critique est égale à votre Chance de Coup Ancestral, vos Dégâts de Coup Critique deviennent égaux à vos Dégâts de Coup Ancestral.*Isopérimétrie :#Les ennemis touchés par les côtés du Triangle de Thalagore sont Étourdis pour les @ prochaines secondes.",
    "FR_LORE": "syn ² x + cos² x = 1",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 94,
    "SPRITE": 28,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "5",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 16,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 5,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 28,
    "EVOLVE_IN": 95,
    "100": "1390 - 2403"
  },
  {
    "REF": 95,
    "EN_NAME": "$ of the Academician",
    "EN_DESC": "|*Trigonometry:#Geometry deals @ £ to enemies hit by an Angle of Thalagoras Triangle.",
    "EN_LORE": "U = R x I",
    "EN": "$ of the Academician//|*Trigonometry:#Geometry deals @ £ to enemies hit by an Angle of Thalagoras Triangle.//U = R x I",
    "FR": "$ de l'Académicien//|*Trigonométrie :#Géométrie inflige @ de £ aux ennemis touchés par un Angle du Triangle de Thalagore.//U = R x I",
    "FR_NAME": "$ de l'Académicien",
    "FR_DESC": "|*Trigonométrie :#Géométrie inflige @ de £ aux ennemis touchés par un Angle du Triangle de Thalagore.",
    "FR_LORE": "U = R x I",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 95,
    "SPRITE": 29,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "rl",
    "VALUE_STAT": "increased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 16,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1629 - 2645"
  },
  {
    "REF": 96,
    "EN_NAME": "$ of Sun and Moon",
    "EN_DESC": "*+@ £*+@ £|*Every @ seconds, Active Effects switch between Sun and Moon effects.*While Sun Effects are active:#You have +@ £#Whenever you should gain Life from Life on Hit, you gain a Sunlight stack instead, up to your total Moonlight stacks.#You trigger Righteous Sunlight, dealing _ £ every second to enemies around you within a ~ yard radius. Righteous Sunlight deals _ Additional Damage. Additional Damage is equal to @ of your Life Regeneration for every Moonlight stack you have.##Whenever Sun Effects ends, if you have less Sunlight stacks than Moonlight stacks, you lose both Sunlight and Moonlight stacks.*While Moon Effects are active:#You have +@ £#You have @ £.#Whenever you're dealt damage, you gain a Moonlight stack.##Whenever you deal damage with your Primary or Secondary Skill, it deals _ Additional Damage. Additional Damage is equal to @ of your Life on Hit for every Sunlight stack you have.##Whenever Moon Effects ends, you lose your Sunlight stacks./\n|*While Moon Effects are active:#You gain @ Moonlight stacks every second./\n|*While Sun Effects are active:#You can only restore Life with Life Regeneration.*While Moon Effects are active:#You can only restore Life with Life on Hit.",
    "EN_LORE": "\"Sun and the Moon together in a single weapon? It's like having a pizza with all the toppings, but better!\" #- A hungry adventurer",
    "EN": "$ of Sun and Moon//*+@ £*+@ £|*Every @ seconds, Active Effects switch between Sun and Moon effects.*While Sun Effects are active:#You have +@ £#Whenever you should gain Life from Life on Hit, you gain a Sunlight stack instead, up to your total Moonlight stacks.#You trigger Righteous Sunlight, dealing _ £ every second to enemies around you within a ~ yard radius. Righteous Sunlight deals _ Additional Damage. Additional Damage is equal to @ of your Life Regeneration for every Moonlight stack you have.##Whenever Sun Effects ends, if you have less Sunlight stacks than Moonlight stacks, you lose both Sunlight and Moonlight stacks.*While Moon Effects are active:#You have +@ £#You have @ £.#Whenever you're dealt damage, you gain a Moonlight stack.##Whenever you deal damage with your Primary or Secondary Skill, it deals _ Additional Damage. Additional Damage is equal to @ of your Life on Hit for every Sunlight stack you have.##Whenever Moon Effects ends, you lose your Sunlight stacks./\n|*While Moon Effects are active:#You gain @ Moonlight stacks every second./\n|*While Sun Effects are active:#You can only restore Life with Life Regeneration.*While Moon Effects are active:#You can only restore Life with Life on Hit.//\"Sun and the Moon together in a single weapon? It's like having a pizza with all the toppings, but better!\" #- A hungry adventurer",
    "FR": "$ de Soleil et de Lune//*+@ £*+@ £|*Toutes les @ secondes, les Effets Actifs alternent entre les effets du Soleil et de la Lune.*Lorsque les effets du Soleil sont actifs :#Vous avez +@ de £#Chaque fois que vous devriez gagner de la Vie grâce à la Vie par Coup, vous gagnez un cumul de Rayonnement Solaire, jusqu'à votre total de cumuls de Rayonnement Lunaire.#Vous déclenchez le Rayonnement du Vertueux, infligeant _ de £ toutes les secondes aux ennemis autour de vous dans un rayon de ~ mètres. Le Rayonnement du Vertueux inflige _ de Dégâts Supplémentaires. Ces Dégâts Supplémentaires sont équivalents à @ de votre Régénération de Vie pour chaque cumul de Rayonnement Lunaire que vous avez.##Lorsque les effets du Soleil se terminent, si vous avez moins de cumuls de Rayonnement Solaire que de cumuls de Rayonnement Lunaire, vous perdez à la fois les cumuls de Rayonnement Solaire et de Rayonnement Lunaire.*Lorsque les effets de la Lune sont actifs :#Vous avez +@ de £#Vous avez @ de £.#Chaque fois que vous subissez des dégâts, vous gagnez un cumul de Rayonnement Lunaire.##Chaque fois que vous infligez des dégâts avec une Compétence Primaire ou Secondaire, elle inflige _ de Dégâts Supplémentaires. Ces Dégâts Supplémentaires sont équivalents à @ de votre Vie par Coup pour chaque pile de Rayonnement Solaire que vous avez.##Lorsque les effets de la Lune se terminent, vous perdez vos cumuls de Rayonnement Solaire./\n|*Lorsque les effets de la Lune sont actifs :#Vous gagnez @ cumuls de Rayonnement Lunaire chaque seconde./\n|*Lorsque les effets du Soleil sont actifs :#Vous ne pouvez restaurer la Vie qu'avec la Régénération de Vie.*Lorsque les effets de la Lune sont actifs :#Vous ne pouvez restaurer la Vie qu'avec la Vie par Coup.//\"Soleil et Lune dans une seule arme ? C'est comme avoir une pizza avec tous les garnitures, mais en mieux !\"#- Un aventurier gourmand",
    "FR_NAME": "$ de Soleil et de Lune",
    "FR_DESC": "*+@ £*+@ £|*Toutes les @ secondes, les Effets Actifs alternent entre les effets du Soleil et de la Lune.*Lorsque les effets du Soleil sont actifs :#Vous avez +@ de £#Chaque fois que vous devriez gagner de la Vie grâce à la Vie par Coup, vous gagnez un cumul de Rayonnement Solaire, jusqu'à votre total de cumuls de Rayonnement Lunaire.#Vous déclenchez le Rayonnement du Vertueux, infligeant _ de £ toutes les secondes aux ennemis autour de vous dans un rayon de ~ mètres. Le Rayonnement du Vertueux inflige _ de Dégâts Supplémentaires. Ces Dégâts Supplémentaires sont équivalents à @ de votre Régénération de Vie pour chaque cumul de Rayonnement Lunaire que vous avez.##Lorsque les effets du Soleil se terminent, si vous avez moins de cumuls de Rayonnement Solaire que de cumuls de Rayonnement Lunaire, vous perdez à la fois les cumuls de Rayonnement Solaire et de Rayonnement Lunaire.*Lorsque les effets de la Lune sont actifs :#Vous avez +@ de £#Vous avez @ de £.#Chaque fois que vous subissez des dégâts, vous gagnez un cumul de Rayonnement Lunaire.##Chaque fois que vous infligez des dégâts avec une Compétence Primaire ou Secondaire, elle inflige _ de Dégâts Supplémentaires. Ces Dégâts Supplémentaires sont équivalents à @ de votre Vie par Coup pour chaque pile de Rayonnement Solaire que vous avez.##Lorsque les effets de la Lune se terminent, vous perdez vos cumuls de Rayonnement Solaire./\n|*Lorsque les effets de la Lune sont actifs :#Vous gagnez @ cumuls de Rayonnement Lunaire chaque seconde./\n|*Lorsque les effets du Soleil sont actifs :#Vous ne pouvez restaurer la Vie qu'avec la Régénération de Vie.*Lorsque les effets de la Lune sont actifs :#Vous ne pouvez restaurer la Vie qu'avec la Vie par Coup.",
    "FR_LORE": "\"Soleil et Lune dans une seule arme ? C'est comme avoir une pizza avec tous les garnitures, mais en mieux !\"#- Un aventurier gourmand",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 47,
    "SPRITE": 26,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|12|1|4|1|-100|8|\n3",
    "VALUE_TYPE": "|||%|%|%|%|%|\n",
    "VALUE_LEVEL": "rl|rl||rl||rl|||\n",
    "VALUE_STAT": "health_on_hit_add|health_regen_add|health_regen_global_mult|physical_damage|health_on_hit_global_mult|health_regen_global_mult",
    "VALUE_REAL": "damage:physical_damage:1*rl|synergy:health_regen_moonlight:4|synergy:health_on_hit_sunlight:8",
    "BASE_DMG_MIN": 28,
    "BASE_DMG_MAX": 34,
    "MIN_DMG_LVL": 7,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1331 - 1517"
  },
  {
    "REF": 97,
    "EN_NAME": "$ of the Ungifted",
    "EN_DESC": "|*Your Primary Skill deals _% £.#Increased Damage is equal to your Percent of Mana Locked increased by @.*If the Mana Cost of a Skill is less than _, its Mana Cost is reduced to @.#This being equal to your Mana Locked.*act:254*act:255/ \n|*Mana Locked can now exceed @./\n|*Mana Locked by Mana is Overrated is now @.",
    "EN_LORE": "You know what's not overatted? The fact that you can use this weapon as an umbrella on rainy days.",
    "EN": "$ of the Ungifted//|*Your Primary Skill deals _% £.#Increased Damage is equal to your Percent of Mana Locked increased by @.*If the Mana Cost of a Skill is less than _, its Mana Cost is reduced to @.#This being equal to your Mana Locked.*act:254*act:255/ \n|*Mana Locked can now exceed @./\n|*Mana Locked by Mana is Overrated is now @.//You know what's not overatted? The fact that you can use this weapon as an umbrella on rainy days.",
    "FR": "$ du Sans-Don//|*Votre Compétence Primaire inflige _% de £.#Ces Dégâts Augmentés sont égaux à votre Pourcentage de Mana Verrouillé augmenté de @.*Si le Coût en Mana d'une Compétence est inférieur à _, son Coût en Mana est réduit à @.#Ceci étant égal à votre Mana Verrouillé.*act:254*act:255/\n|*Le Mana Verrouillé peut désormais dépasser @./\n|*Le Mana Verrouillé par Mana is Overrated est désormais @.//Vous savez ce qui n'est pas surcôté ? Le fait que vous puissiez utiliser cette arme comme un parapluie les jours de pluie.",
    "FR_NAME": "$ du Sans-Don",
    "FR_DESC": "|*Votre Compétence Primaire inflige _% de £.#Ces Dégâts Augmentés sont égaux à votre Pourcentage de Mana Verrouillé augmenté de @.*Si le Coût en Mana d'une Compétence est inférieur à _, son Coût en Mana est réduit à @.#Ceci étant égal à votre Mana Verrouillé.*act:254*act:255/\n|*Le Mana Verrouillé peut désormais dépasser @./\n|*Le Mana Verrouillé par Mana is Overrated est désormais @.",
    "FR_LORE": "Vous savez ce qui n'est pas surcôté ? Le fait que vous puissiez utiliser cette arme comme un parapluie les jours de pluie.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 51,
    "SPRITE": 77,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1||\n100|\n100",
    "VALUE_TYPE": "%||\n%|\n%",
    "VALUE_LEVEL": "rl||\n|",
    "VALUE_STAT": "increased_damage|aura_increased_effect_percent",
    "VALUE_REAL": "synergy:mana_lock_percent_ungift:100|synergy:mana_lock_flat:100",
    "BASE_DMG_MIN": 17,
    "BASE_DMG_MAX": 24,
    "MIN_DMG_LVL": 12,
    "MAX_DMG_LVL": 16,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2141 - 2830"
  },
  {
    "REF": 98,
    "EN_NAME": "Overdriving $",
    "EN_DESC": "*+@ £*+@ £*+@ £|*Critical Strikes have @ £./\n*+@ £|*Overdrive Damage is now based on your Skill Damage instead of your Raw Damage./\n|*Melee Skill now deal @ Decreased Damage",
    "EN_LORE": "For simple Champions, with simple tastes.",
    "EN": "Overdriving $//*+@ £*+@ £*+@ £|*Critical Strikes have @ £./\n*+@ £|*Overdrive Damage is now based on your Skill Damage instead of your Raw Damage./\n|*Melee Skill now deal @ Decreased Damage//For simple Champions, with simple tastes.",
    "FR": "$ de Surpuissance//*+@ £*+@ £*+@ £|*Les Coups Critiques ont @ de £./\n*+@ £|*Les Dégats d'Overdrive sont désormais basés sur vos Dégats Physiques au lieu de vos Dégâts Bruts./\n|*Les Compétences de Mêlée infligent @ de Dégâts Réduits.//Pour des Champions simples, avec des goûts simples.",
    "FR_NAME": "$ de Surpuissance",
    "FR_DESC": "*+@ £*+@ £*+@ £|*Les Coups Critiques ont @ de £./\n*+@ £|*Les Dégats d'Overdrive sont désormais basés sur vos Dégats Physiques au lieu de vos Dégâts Bruts./\n|*Les Compétences de Mêlée infligent @ de Dégâts Réduits.",
    "FR_LORE": "Pour des Champions simples, avec des goûts simples.",
    "CH": "过载$",
    "CH_NAME": "过载$",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "過載$",
    "TW_NAME": "過載$",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 103,
    "SPRITE": 98,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3|0.4|1|100|\n0.5|\n50",
    "VALUE_TYPE": "|%|%|%|\n%|\n%",
    "VALUE_LEVEL": "|rl|rl||\nrl|",
    "VALUE_STAT": "overdrive_bounce_number_add|overdrive_chance_percent|overdrive_damage_percent|overdrive_chance_percent|\noverdrive_damage_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 28,
    "BASE_DMG_MAX": 35,
    "MIN_DMG_LVL": 8,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1497 - 1688"
  },
  {
    "REF": 99,
    "EN_NAME": "Altior, Arah's Perfect $",
    "EN_DESC": "|*You have +_% £. Raw Damage Multiplier is equal to @ for each Slorm Reaper you own across all Characters.*You have +_% £. Elemental Damage Multiplier is equal to @ for each Slorm Reaper you own across all Characters.*Reaper Storm deals @ £.*act:256/\n|*The cycle time of Reaper Storm is reduced by @.*The cycled Slorm Reaper deals @ £ for every Primordial version of it you own across all Characters./\n|*Altior is Perfect.#There is no Malediction.",
    "EN_LORE": "Wrought by Astorias Both a Reapersmith and Champion of Arah, he gave a part of his Blessing to this Reaper.#He never used it.",
    "EN": "Altior, Arah's Perfect $//|*You have +_% £. Raw Damage Multiplier is equal to @ for each Slorm Reaper you own across all Characters.*You have +_% £. Elemental Damage Multiplier is equal to @ for each Slorm Reaper you own across all Characters.*Reaper Storm deals @ £.*act:256/\n|*The cycle time of Reaper Storm is reduced by @.*The cycled Slorm Reaper deals @ £ for every Primordial version of it you own across all Characters./\n|*Altior is Perfect.#There is no Malediction.//Wrought by Astorias Both a Reapersmith and Champion of Arah, he gave a part of his Blessing to this Reaper.#He never used it.",
    "FR": "Altior, $ Béni d'Arah/Altior, $ Bénie d'Arah//|*Vous avez +_% de £. Ce Multiplicateur de Dégâts Brut est équivalent à @ pour chaque Slorm Reaper que vous possédez à travers tous vos personnages.*Vous avez +_% de £. Ce Multiplicateur de Dégâts Élémentaires est équivalent à @ pour chaque Slorm Reaper que vous possédez à travers tous vos personnages.*La Tempête de Reapers inflige @ de £.*act:256/\n|*Le temps de complétion de la Tempête de Reapers est réduit de @.*Le Slorm Reaper lancé inflige @ de £ pour chaque version Primordiale de celui-ci que vous possédez à travers tous vos personnages./\n|*Altior est Parfait.#Il n'y a aucune Malédiction.//Forgé par Astorias. A la fois Reapergeron et Champion d'Arah, il insuffla une partie de sa bénédiction dans son Reaper.#Il ne s'en est jamais servi.",
    "FR_NAME": "Altior, $ Béni d'Arah/Altior, $ Bénie d'Arah",
    "FR_DESC": "|*Vous avez +_% de £. Ce Multiplicateur de Dégâts Brut est équivalent à @ pour chaque Slorm Reaper que vous possédez à travers tous vos personnages.*Vous avez +_% de £. Ce Multiplicateur de Dégâts Élémentaires est équivalent à @ pour chaque Slorm Reaper que vous possédez à travers tous vos personnages.*La Tempête de Reapers inflige @ de £.*act:256/\n|*Le temps de complétion de la Tempête de Reapers est réduit de @.*Le Slorm Reaper lancé inflige @ de £ pour chaque version Primordiale de celui-ci que vous possédez à travers tous vos personnages./\n|*Altior est Parfait.#Il n'y a aucune Malédiction.",
    "FR_LORE": "Forgé par Astorias. A la fois Reapergeron et Champion d'Arah, il insuffla une partie de sa bénédiction dans son Reaper.#Il ne s'en est jamais servi.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 119,
    "SPRITE": 114,
    "BLACKSMITH": 0,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|0.5|1|\n0.5|40",
    "VALUE_TYPE": "%|%|%|\n%|%",
    "VALUE_LEVEL": "||rl|\nrl|",
    "VALUE_STAT": "basic_damage_global_mult|elemental_damage_global_mult|increased_damage|\nincreased_damage",
    "VALUE_REAL": "synergy:reaper_owned:0.5|synergy:reaper_owned:0.5",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 18,
    "MIN_DMG_LVL": 10,
    "MAX_DMG_LVL": 12,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2561 - 3069"
  },
  {
    "REF": 100,
    "EN_NAME": "Arah's Revenge, Liege-$",
    "EN_DESC": "*+@ £|*Whenever you deal damage to an enemy with a Primary or Secondary Projectile Skill, you have a @ £ to trigger Arah's Revenge, a Piercing projectile coming from a random direction toward the enemy hit, dealing _ £ and _ £ to enemies hit.*Arah's Revenge can both Fork and Pierce.*Arah's Revenge deals up to @ £./\n|*Whenever an enemy is killed by Arah's Revenge, it triggers an Explosion of @ Arah's Revenge.*Arah's Revenge triggered this way deal up to @ £./\n|*Damage you deal from Non-Projectile sources are reduced by @.",
    "EN_LORE": "\"Forged in the Wrath of the Ancient Champions, this legendary sword is revenge incarnate...\"",
    "EN": "Arah's Revenge, Liege-$//*+@ £|*Whenever you deal damage to an enemy with a Primary or Secondary Projectile Skill, you have a @ £ to trigger Arah's Revenge, a Piercing projectile coming from a random direction toward the enemy hit, dealing _ £ and _ £ to enemies hit.*Arah's Revenge can both Fork and Pierce.*Arah's Revenge deals up to @ £./\n|*Whenever an enemy is killed by Arah's Revenge, it triggers an Explosion of @ Arah's Revenge.*Arah's Revenge triggered this way deal up to @ £./\n|*Damage you deal from Non-Projectile sources are reduced by @.//\"Forged in the Wrath of the Ancient Champions, this legendary sword is revenge incarnate...\"",
    "FR": "Revanche d'Arah, $-Lige//*+@ £|*Chaque fois que vous infligez des dégâts à un ennemi avec une Compétence de Projectile Primaire ou Secondaire, vous avez @ de £ de déclencher la Vengeance d'Arah, un projectile Perçant provenant d'une direction aléatoire en direction de l'ennemi touché, infligeant _ de £ et _ de £ aux ennemis touchés.*La Vengeance d'Arah peut à la fois Foucher et Percer.*La Vengeance d'Arah inflige jusqu'à @ de £./\n|*Chaque fois qu'un ennemi est tué par la Vengeance d'Arah, cela déclenche une explosion de @ Vengeances d'Arah.*Les Vengeances d'Arah déclenchées de cette manière infligent jusqu'à @ de £./\n|*Les dégâts que vous infligez à partir de sources autres que les Projectiles sont réduits de @.//\"Forgée dans la colère des Anciens Champions, ce Reaper Légendaire est la revanche incarnée...\"",
    "FR_NAME": "Revanche d'Arah, $-Lige",
    "FR_DESC": "*+@ £|*Chaque fois que vous infligez des dégâts à un ennemi avec une Compétence de Projectile Primaire ou Secondaire, vous avez @ de £ de déclencher la Vengeance d'Arah, un projectile Perçant provenant d'une direction aléatoire en direction de l'ennemi touché, infligeant _ de £ et _ de £ aux ennemis touchés.*La Vengeance d'Arah peut à la fois Foucher et Percer.*La Vengeance d'Arah inflige jusqu'à @ de £./\n|*Chaque fois qu'un ennemi est tué par la Vengeance d'Arah, cela déclenche une explosion de @ Vengeances d'Arah.*Les Vengeances d'Arah déclenchées de cette manière infligent jusqu'à @ de £./\n|*Les dégâts que vous infligez à partir de sources autres que les Projectiles sont réduits de @.",
    "FR_LORE": "\"Forgée dans la colère des Anciens Champions, ce Reaper Légendaire est la revanche incarnée...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 48,
    "SPRITE": 37,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.25|0.5|3|\n5|2|\n90",
    "VALUE_TYPE": "%|%|%|\n|%|\n%",
    "VALUE_LEVEL": "rl|rl|rl|\n|rl|\n",
    "VALUE_STAT": "fork_chance_percent|chance|physical_damage|elemental_damage|increased_damage|\nincreased_damage",
    "VALUE_REAL": "damage:physical_damage:100|damage:elemental_damage:100",
    "BASE_DMG_MIN": 10,
    "BASE_DMG_MAX": 20,
    "MIN_DMG_LVL": 7,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 3,
    "MAX_LVL": 20,
    "EVOLVE_IN": 101,
    "100": "1287 - 1478"
  },
  {
    "REF": 101,
    "EN_NAME": "Arah's Revenge, Enduring Liege-$",
    "EN_DESC": "|*Whenever Arah's Revenge triggers, it has a @ £ to trigger another Arah's Revenge.",
    "EN_LORE": "\"Each strike unleashes divine fury...\"",
    "EN": "Arah's Revenge, Enduring Liege-$//|*Whenever Arah's Revenge triggers, it has a @ £ to trigger another Arah's Revenge.//\"Each strike unleashes divine fury...\"",
    "FR": "Revanche d'Arah, $-Lige Tenace//|*Chaque fois que la Vengeance d'Arah est déclenchée, elle a @ de £ de déclencher une autre Vengeance d'Arah.//\"Chaque coup déchaîne la fureur divine...\"",
    "FR_NAME": "Revanche d'Arah, $-Lige Tenace",
    "FR_DESC": "|*Chaque fois que la Vengeance d'Arah est déclenchée, elle a @ de £ de déclencher une autre Vengeance d'Arah.",
    "FR_LORE": "\"Chaque coup déchaîne la fureur divine...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 49,
    "SPRITE": 38,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "50",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "chance",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 10,
    "BASE_DMG_MAX": 20,
    "MIN_DMG_LVL": 8,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 40,
    "EVOLVE_IN": 102,
    "100": "1451 - 1643"
  },
  {
    "REF": 102,
    "EN_NAME": "Arah's Revenge, True Liege-$",
    "EN_DESC": "|*Whenever Arah's Revenge triggers twice, it has a @ £ to trigger up to @ additional Arah's Revenge.",
    "EN_LORE": "\"... turning enemies into cosmic confetti.\"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "EN": "Arah's Revenge, True Liege-$//|*Whenever Arah's Revenge triggers twice, it has a @ £ to trigger up to @ additional Arah's Revenge.//\"... turning enemies into cosmic confetti.\"#- Champions of Arah, Heroes or Impostors, by Adam Nostrus",
    "FR": "Revanche d'Arah, $-Lige Véritable//|*Chaque fois que la Vengeance d'Arah est déclenchée deux fois, elle a @ de £ de déclencher jusqu'à @ Vengeances d'Arah supplémentaires.//\"... transformant les ennemis en confettis cosmiques.\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "FR_NAME": "Revanche d'Arah, $-Lige Véritable",
    "FR_DESC": "|*Chaque fois que la Vengeance d'Arah est déclenchée deux fois, elle a @ de £ de déclencher jusqu'à @ Vengeances d'Arah supplémentaires.",
    "FR_LORE": "\"... transformant les ennemis en confettis cosmiques.\"#- Champions d'Arah, Héros ou Imposteurs, par Adam Nostrus",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 50,
    "SPRITE": 39,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "50|3",
    "VALUE_TYPE": "%|",
    "VALUE_LEVEL": "|",
    "VALUE_STAT": "chance",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 10,
    "BASE_DMG_MAX": 20,
    "MIN_DMG_LVL": 9,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1617 - 1813"
  },
  {
    "REF": 103,
    "EN_NAME": "$ of The Many",
    "EN_DESC": "*+@ £*+@ £|*Every @ seconds, you gain an Orb of Life, rotating around you, up to @.*Whenever you should receive damage, if you have an Orb of Life, you ignore Incoming Damage, and lose an Orb of Life instead.*For each Orb of Life you have:#+@ £.*For each Orb of Life missing:#+@ £*Whenever you deal damage with your Primary or Secondary Skill:#- You have a @ £ per Orb of Life to apply Life Odyssey to enemies hit, dealing _ £ over @ seconds. Skill Damage is equal to @ of your Life Regeneration.#- You have a @ £ per missing Orb of Life to deal _ Additional Damage. Additional Damage is equal to @ of your Life Regeneration for every enemy affected by Life Odyssey within a @ yard radius./\n|*You can now have up to @ Orbs of Life.*Every second, Orbs of Life now cast a Life Projectile toward a random enemy nearby piercing and dealing _ £ to enemies hit.*Life Projectiles deal @ £ per missing Orb of Life./\n|*Whenever an Orb of Life spawns, @ of your Life is consumed.",
    "EN_LORE": "Comes with a cup holder.",
    "EN": "$ of The Many//*+@ £*+@ £|*Every @ seconds, you gain an Orb of Life, rotating around you, up to @.*Whenever you should receive damage, if you have an Orb of Life, you ignore Incoming Damage, and lose an Orb of Life instead.*For each Orb of Life you have:#+@ £.*For each Orb of Life missing:#+@ £*Whenever you deal damage with your Primary or Secondary Skill:#- You have a @ £ per Orb of Life to apply Life Odyssey to enemies hit, dealing _ £ over @ seconds. Skill Damage is equal to @ of your Life Regeneration.#- You have a @ £ per missing Orb of Life to deal _ Additional Damage. Additional Damage is equal to @ of your Life Regeneration for every enemy affected by Life Odyssey within a @ yard radius./\n|*You can now have up to @ Orbs of Life.*Every second, Orbs of Life now cast a Life Projectile toward a random enemy nearby piercing and dealing _ £ to enemies hit.*Life Projectiles deal @ £ per missing Orb of Life./\n|*Whenever an Orb of Life spawns, @ of your Life is consumed.//Comes with a cup holder.",
    "FR": "$ de Multitude//*+@ £*+@ £|*Toutes les @ secondes, vous gagnez un Orbe de Vie qui tourne autour de vous, jusqu'à un maximum de @.*Chaque fois que vous devriez subir des dégâts, si vous avez un Orbe de Vie, vous ignorez ces dégâts et perdez un Orbe de Vie à la place.*Pour chaque Orbe de Vie que vous avez :#+@ de £.*Pour chaque Orbe de Vie manquant :#+@ de £*Chaque fois que vous infligez des Dégâts avec une Compétence Primaire ou Secondaire :#- Vous avez @ de £ par Orbe de Vie d'appliquer Odyssée de Vie aux ennemis touchés, infligeant _ de £ sur @ secondes. Ces Dégâts Physiques sont équivalents à @ de votre Régénération de Vie.#- Vous avez @ de £ par Orbe de Vie manquant d'infliger _ de Dégâts Supplémentaires. Ces Dégâts Supplémentaires sont équivalents à @ de votre Régénération de Vie pour chaque ennemi affecté par Odyssée de Vie dans un rayon de @ mètres./\n|*Vous pouvez maintenant avoir jusqu'à @ Orbes de Vie.*Chaque seconde, les Orbes de Vie lancent maintenant un Projectile de Vie vers un ennemi aléatoire à proximité, Perçant et infligeant _ de £ aux ennemis touchés.*Les Projectiles de Vie infligent @ de £ par Orbe de Vie manquant./\n|*À chaque apparition d'un Orbe de Vie, @ de votre Vie est consumée.//Est egalement equippé d'un porte-gobelet.",
    "FR_NAME": "$ de Multitude",
    "FR_DESC": "*+@ £*+@ £|*Toutes les @ secondes, vous gagnez un Orbe de Vie qui tourne autour de vous, jusqu'à un maximum de @.*Chaque fois que vous devriez subir des dégâts, si vous avez un Orbe de Vie, vous ignorez ces dégâts et perdez un Orbe de Vie à la place.*Pour chaque Orbe de Vie que vous avez :#+@ de £.*Pour chaque Orbe de Vie manquant :#+@ de £*Chaque fois que vous infligez des Dégâts avec une Compétence Primaire ou Secondaire :#- Vous avez @ de £ par Orbe de Vie d'appliquer Odyssée de Vie aux ennemis touchés, infligeant _ de £ sur @ secondes. Ces Dégâts Physiques sont équivalents à @ de votre Régénération de Vie.#- Vous avez @ de £ par Orbe de Vie manquant d'infliger _ de Dégâts Supplémentaires. Ces Dégâts Supplémentaires sont équivalents à @ de votre Régénération de Vie pour chaque ennemi affecté par Odyssée de Vie dans un rayon de @ mètres./\n|*Vous pouvez maintenant avoir jusqu'à @ Orbes de Vie.*Chaque seconde, les Orbes de Vie lancent maintenant un Projectile de Vie vers un ennemi aléatoire à proximité, Perçant et infligeant _ de £ aux ennemis touchés.*Les Projectiles de Vie infligent @ de £ par Orbe de Vie manquant./\n|*À chaque apparition d'un Orbe de Vie, @ de votre Vie est consumée.",
    "FR_LORE": "Est egalement equippé d'un porte-gobelet.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 52,
    "SPRITE": 86,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.5|0.5|3|5|10|15|10|7|15|10|5|7|\n10|1|\n10",
    "VALUE_TYPE": "|%|||%|%|%||%|%|%||\n|%|\n%",
    "VALUE_LEVEL": "rl|rl|||||||rl||rl||\n|rl|\n",
    "VALUE_STAT": "health_regen_add|health_regen_percent|the_speed_global_mult|health_regen_global_mult|chance|physical_damage|chance|\nphysical_damage|increased_damage",
    "VALUE_REAL": "synergy:life_regeneration:0.15*rl|synergy:life_regeneration:0.05*rl|\ndamage:physical_damage:100|\nsynergy:max_health:0.1",
    "BASE_DMG_MIN": 32,
    "BASE_DMG_MAX": 46,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 6,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 1,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "984 - 1768"
  },
  {
    "REF": 104,
    "EN_NAME": "$ of the Goldfish",
    "EN_DESC": "|*act:203/\n|*act:206*Cooldowns are no longer shared./\n|*You can no longer benefit from Greater Traits on Attributes.",
    "EN_LORE": "\"Forged by Smaloron as he stood by a lake. He would have been inspired by the shape of fishes...\"",
    "EN": "$ of the Goldfish//|*act:203/\n|*act:206*Cooldowns are no longer shared./\n|*You can no longer benefit from Greater Traits on Attributes.//\"Forged by Smaloron as he stood by a lake. He would have been inspired by the shape of fishes...\"",
    "FR": "$ du Poisson Rouge//|*act:203/\n|*act:206*Les Temps de Recharge ne sont plus partagés./\n|*Vous ne pouvez plus bénéficier des Traits Supérieurs d'Attributs.//\"Forgé par Smaloron alors qu'il se tenait au bord d'un Lac.#Il aurait été inspiré par la forme des poissons...\"",
    "FR_NAME": "$ du Poisson Rouge",
    "FR_DESC": "|*act:203/\n|*act:206*Les Temps de Recharge ne sont plus partagés./\n|*Vous ne pouvez plus bénéficier des Traits Supérieurs d'Attributs.",
    "FR_LORE": "\"Forgé par Smaloron alors qu'il se tenait au bord d'un Lac.#Il aurait été inspiré par la forme des poissons...\"",
    "CH": "金鱼$////“斯莫洛伦站在湖面上打造了它。他的灵感肯定来自于鱼儿……”",
    "CH_NAME": "金鱼$",
    "CH_DESCRIPTION": "“斯莫洛伦站在湖面上打造了它。他的灵感肯定来自于鱼儿……”",
    "CH_LORE": "",
    "TW": "金魚$//|*act:203/\n|*act:206*冷卻時間不再共享。 /\n|*不再受益於角色屬性。//“斯莫洛倫站在湖面上打造了它。他的靈感肯定來自於魚兒……”",
    "TW_NAME": "金魚$",
    "TW_DESCRIPTION": "|*act:203/\n|*act:206*冷卻時間不再共享。 /\n|*不再受益於角色屬性。",
    "TW_LORE": "“斯莫洛倫站在湖面上打造了它。他的靈感肯定來自於魚兒……”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 108,
    "SPRITE": 104,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 0,
    "BASE_DMG_MAX": 0,
    "MIN_DMG_LVL": 0,
    "MAX_DMG_LVL": 0,
    "DMG_MULTIPLIER": 0,
    "LOOTABLE": 2,
    "MAX_LVL": 58,
    "EVOLVE_IN": 105,
    "100": "0 - 0"
  },
  {
    "REF": 105,
    "EN_NAME": "$ of the Magnificent Leviathan",
    "EN_DESC": "|*act:204*act:205*Active Skills from this Reaper share the same Cooldown.",
    "EN_LORE": "\"... and apparently, fishes were not the only thing in the lake... \"#- Slorm Reapers of Legend, by Pr. Geranium",
    "EN": "$ of the Magnificent Leviathan//|*act:204*act:205*Active Skills from this Reaper share the same Cooldown.//\"... and apparently, fishes were not the only thing in the lake... \"#- Slorm Reapers of Legend, by Pr. Geranium",
    "FR": "$ du Léviathan Magnifique//|*act:204*act:205*Les Compétences Actives de ce Reaper partagent le même Temps de Recharge.//\"... et visiblement, il n'y avait pas que des poissons dans ce lac...\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "$ du Léviathan Magnifique",
    "FR_DESC": "|*act:204*act:205*Les Compétences Actives de ce Reaper partagent le même Temps de Recharge.",
    "FR_LORE": "\"... et visiblement, il n'y avait pas que des poissons dans ce lac...\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "宏伟利维坦$//|*act:204*act:205*该武器的主动技能共享冷却时间。//“……显然，湖里不只有鱼儿……”#- 猎魂传奇，杰兰涅姆教授",
    "CH_NAME": "宏伟利维坦$",
    "CH_DESCRIPTION": "|*act:204*act:205*该武器的主动技能共享冷却时间。",
    "CH_LORE": "“……显然，湖里不只有鱼儿……”#- 猎魂传奇，杰兰涅姆教授",
    "TW": "宏偉利維坦$//|*act:204*act:205*該武器的主動技能共享冷卻時間。//“……顯然，湖里不只有魚兒……”#- 獵魂傳奇，傑蘭涅姆教授",
    "TW_NAME": "宏偉利維坦$",
    "TW_DESCRIPTION": "|*act:204*act:205*該武器的主動技能共享冷卻時間。",
    "TW_LORE": "“……顯然，湖里不只有魚兒……”#- 獵魂傳奇，傑蘭涅姆教授",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 109,
    "SPRITE": 105,
    "BLACKSMITH": 4,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 24,
    "BASE_DMG_MAX": 31,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1663 - 2893"
  },
  {
    "REF": 106,
    "EN_NAME": "$ of the Conniving Warlock",
    "EN_DESC": "|*Whenever you spend Life to cast Skills, you trigger Life Drain, dealing Skill Damage equal to the Percent of Life spent to enemies within a @ yard radius and apply up to @ Life Link stack to enemies hit, up to @.*Life Drain deals @ £.*Life Drain then deals @ £ for each stack of Life Link.*act:257/\n|*The Cooldown Time of Skills costing Life to cast is reduced by @./\n|*Life Costs are doubled.",
    "EN_LORE": "\"This cursed Reaper is the perfect accomplice to sow chaos...\"",
    "EN": "$ of the Conniving Warlock//|*Whenever you spend Life to cast Skills, you trigger Life Drain, dealing Skill Damage equal to the Percent of Life spent to enemies within a @ yard radius and apply up to @ Life Link stack to enemies hit, up to @.*Life Drain deals @ £.*Life Drain then deals @ £ for each stack of Life Link.*act:257/\n|*The Cooldown Time of Skills costing Life to cast is reduced by @./\n|*Life Costs are doubled.//\"This cursed Reaper is the perfect accomplice to sow chaos...\"",
    "FR": "$ du Démoniste Complice//|*Chaque fois que vous dépensez de la Vie pour lancer des Compétences, vous déclenchez Drain de Vie, infligeant des Dégâts Physiques équivalents au Pourcentage de Vie dépensé aux ennemis dans un rayon de @ mètres et appliquant jusqu'à @ cumuls de Lien de Vie aux ennemis touchés, jusqu'à @.*Drain de Vie inflige @ de £.*Drain de Vie inflige ensuite @ de £ pour chaque cumul de Lien de Vie.*act:257/\n|*Le Temps de Recharge des Compétences coûtant de la Vie à lancer est réduit de @./\n|*Les Coûts en Vie sont doublés.//\"Ce Reaper maudit est le complice parfait pour semer le chaos...\"",
    "FR_NAME": "$ du Démoniste Complice",
    "FR_DESC": "|*Chaque fois que vous dépensez de la Vie pour lancer des Compétences, vous déclenchez Drain de Vie, infligeant des Dégâts Physiques équivalents au Pourcentage de Vie dépensé aux ennemis dans un rayon de @ mètres et appliquant jusqu'à @ cumuls de Lien de Vie aux ennemis touchés, jusqu'à @.*Drain de Vie inflige @ de £.*Drain de Vie inflige ensuite @ de £ pour chaque cumul de Lien de Vie.*act:257/\n|*Le Temps de Recharge des Compétences coûtant de la Vie à lancer est réduit de @./\n|*Les Coûts en Vie sont doublés.",
    "FR_LORE": "\"Ce Reaper maudit est le complice parfait pour semer le chaos...\"",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 104,
    "SPRITE": 48,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "3|20|1|1|8|\n70",
    "VALUE_TYPE": "|||%|%|\n%",
    "VALUE_LEVEL": "||rl|rl||\n",
    "VALUE_STAT": "increased_damage|increased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 7,
    "BASE_DMG_MAX": 18,
    "MIN_DMG_LVL": 9,
    "MAX_DMG_LVL": 14,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 1,
    "MAX_LVL": 46,
    "EVOLVE_IN": 107,
    "100": "1367 - 2098"
  },
  {
    "REF": 107,
    "EN_NAME": "$ of the Treacherous Warlock",
    "EN_DESC": "|*Life Drain can Leech Life.*You have +@ £ on Life Drain.",
    "EN_LORE": "\"It whispers wicked secrets to its wielder, turning every strike into a sinister prank.\"#- Slorm Reapers of Legend, by Pr. Geranium ",
    "EN": "$ of the Treacherous Warlock//|*Life Drain can Leech Life.*You have +@ £ on Life Drain.//\"It whispers wicked secrets to its wielder, turning every strike into a sinister prank.\"#- Slorm Reapers of Legend, by Pr. Geranium ",
    "FR": "$ du Démoniste Traître//|*Drain de Vie peut Voler de la Vie.*Vous avez +@ de £  avec Drain de Vie.//\"Il murmure des secrets maléfiques à son porteur, transformant chaque coup en une farce sinistre.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "$ du Démoniste Traître",
    "FR_DESC": "|*Drain de Vie peut Voler de la Vie.*Vous avez +@ de £  avec Drain de Vie.",
    "FR_LORE": "\"Il murmure des secrets maléfiques à son porteur, transformant chaque coup en une farce sinistre.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 105,
    "SPRITE": 49,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "5",
    "VALUE_TYPE": "%",
    "VALUE_LEVEL": "rl",
    "VALUE_STAT": "health_leech_global_mult",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 7,
    "BASE_DMG_MAX": 18,
    "MIN_DMG_LVL": 10,
    "MAX_DMG_LVL": 15,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1506 - 2239"
  },
  {
    "REF": 108,
    "EN_NAME": "Aurelon's $ of Light",
    "EN_DESC": "*+@ £*+@ £|*The Judge of Light is Equipped and at Max Rank.*Whenever you kill an enemy while The Judge of Light is active, you have a @ £ to create a Blorm of Light, for every Negative Effect on the enemy.*A Blorm of Light lasts @ seconds and relentlessly attacks nearby enemies, dealing _ £ per attack.*Blorms of Light deal _% Increased Damage.#Increased Damage is equal to your Percent of Missing Mana./\n|*Whenever you receive Damage, if you have Shield, Damage is reduced to @.*Shield stops decreasing while The Judge of Light is active./\n|*Value of Shields is @.",
    "EN_LORE": "Reaper created by Fulgurorn, who tried to reproduce Aïondee, The Rising Sun, with little success.",
    "EN": "Aurelon's $ of Light//*+@ £*+@ £|*The Judge of Light is Equipped and at Max Rank.*Whenever you kill an enemy while The Judge of Light is active, you have a @ £ to create a Blorm of Light, for every Negative Effect on the enemy.*A Blorm of Light lasts @ seconds and relentlessly attacks nearby enemies, dealing _ £ per attack.*Blorms of Light deal _% Increased Damage.#Increased Damage is equal to your Percent of Missing Mana./\n|*Whenever you receive Damage, if you have Shield, Damage is reduced to @.*Shield stops decreasing while The Judge of Light is active./\n|*Value of Shields is @.//Reaper created by Fulgurorn, who tried to reproduce Aïondee, The Rising Sun, with little success.",
    "FR": "$ de Lumière d'Aurelon//*+@ £*+@ £|*Le Juge de Lumière est Équipé et au Rang Max*Chaque fois que vous tuez un ennemi alors que Juge de Lumière est actif, vous avez @ de £ de créer un Blorm de Lumière pour chaque Effet Négatif sur l'ennemi.*Un Blorm de Lumière dure @ secondes et attaque sans relâche les ennemis à proximité, infligeant _ de £ par attaque.*Les Blorms de Lumière infligent _% de Dégâts Supplémentaires.#Ces Dégâts Supplémentaires sont équivalents à votre Pourcentage de Mana Manquant./\n|*Chaque fois que vous subissez des dégâts, si vous avez du Bouclier, les dégâts sont réduits à @.*Le Bouclier cesse de diminuer tant que Juge de Lumière est actif./\n|*La valeur des Boucliers est de @.//Reaper crée par Fulgurorn, qui tenta de reproduire Aïondee, le Soleil Levant, sans grand succès.",
    "FR_NAME": "$ de Lumière d'Aurelon",
    "FR_DESC": "*+@ £*+@ £|*Le Juge de Lumière est Équipé et au Rang Max*Chaque fois que vous tuez un ennemi alors que Juge de Lumière est actif, vous avez @ de £ de créer un Blorm de Lumière pour chaque Effet Négatif sur l'ennemi.*Un Blorm de Lumière dure @ secondes et attaque sans relâche les ennemis à proximité, infligeant _ de £ par attaque.*Les Blorms de Lumière infligent _% de Dégâts Supplémentaires.#Ces Dégâts Supplémentaires sont équivalents à votre Pourcentage de Mana Manquant./\n|*Chaque fois que vous subissez des dégâts, si vous avez du Bouclier, les dégâts sont réduits à @.*Le Bouclier cesse de diminuer tant que Juge de Lumière est actif./\n|*La valeur des Boucliers est de @.",
    "FR_LORE": "Reaper crée par Fulgurorn, qui tenta de reproduire Aïondee, le Soleil Levant, sans grand succès.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 106,
    "SPRITE": 103,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.5|0.3|7|\n1|\n1",
    "VALUE_TYPE": "%|%|%||\n|",
    "VALUE_LEVEL": "rl|rl|rl||\n|",
    "VALUE_STAT": "aura_increased_effect_percent|aura_increased_effect_global_mult|chance|physical_damage",
    "VALUE_REAL": "damage:physical_damage:70|synergy:percent_missing_mana:1",
    "BASE_DMG_MIN": 38,
    "BASE_DMG_MAX": 46,
    "MIN_DMG_LVL": 2,
    "MAX_DMG_LVL": 3,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "767 - 1048"
  },
  {
    "REF": 109,
    "EN_NAME": "Jerelzor's $ of Shadow Pact",
    "EN_DESC": "*+@ £*+@ £|*Black Pact is equipped and at Max Rank.*Damage diverted by Black Pact is also increased by _% and dealt to the source of that Damage and enemies surrounding him within a @ yard radius.#This Increase is equal to your Aura Increased Effect./\n|*While Black Pact is active, if the damage dealt to you is less than @ of your Current Mana, then @ of that damage is dealt to your Mana instead of your Life.*Damage diverted by Black Pact deals _% £. Increased Damage is equal to your Percent of Missing Mana./\n|*While Black Pact is active, you cannot restore Mana.",
    "EN_LORE": "Be careful what you wish for.",
    "EN": "Jerelzor's $ of Shadow Pact//*+@ £*+@ £|*Black Pact is equipped and at Max Rank.*Damage diverted by Black Pact is also increased by _% and dealt to the source of that Damage and enemies surrounding him within a @ yard radius.#This Increase is equal to your Aura Increased Effect./\n|*While Black Pact is active, if the damage dealt to you is less than @ of your Current Mana, then @ of that damage is dealt to your Mana instead of your Life.*Damage diverted by Black Pact deals _% £. Increased Damage is equal to your Percent of Missing Mana./\n|*While Black Pact is active, you cannot restore Mana.//Be careful what you wish for.",
    "FR": "$ du Pacte d'Ombre de Jerelzor//*+@ £*+@ £|*Pacte Noir est Équipé et au Rang Max.*Les dégâts détournés par Pacte Noir sontaugmentés de _% et infligés à la source de ces dégâts ainsi qu'aux ennemis qui l'entourent dans un rayon de @ mètres.#Cette augmentation est égale à votre Effet d'Aura Amélioré./\n|*Tant que Pacte Noir est actif, si les dégâts qui vous sont infligés sont inférieurs à @ de votre Mana actuel, alors @ de ces dégâts sont infligés à votre Mana au lieu de votre Vie.*Les dégâts détournés par Pacte Noir infligent _% de £. Ces Dégâts Supplémentaires sont équivalents à votre Pourcentage de Mana Manquant./\n|*Tant que Pacte Noir est actif, vous ne pouvez pas restaurer de Mana.//Faites attention à ce que vous souhaitez.",
    "FR_NAME": "$ du Pacte d'Ombre de Jerelzor",
    "FR_DESC": "*+@ £*+@ £|*Pacte Noir est Équipé et au Rang Max.*Les dégâts détournés par Pacte Noir sontaugmentés de _% et infligés à la source de ces dégâts ainsi qu'aux ennemis qui l'entourent dans un rayon de @ mètres.#Cette augmentation est égale à votre Effet d'Aura Amélioré./\n|*Tant que Pacte Noir est actif, si les dégâts qui vous sont infligés sont inférieurs à @ de votre Mana actuel, alors @ de ces dégâts sont infligés à votre Mana au lieu de votre Vie.*Les dégâts détournés par Pacte Noir infligent _% de £. Ces Dégâts Supplémentaires sont équivalents à votre Pourcentage de Mana Manquant./\n|*Tant que Pacte Noir est actif, vous ne pouvez pas restaurer de Mana.",
    "FR_LORE": "Faites attention à ce que vous souhaitez.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 107,
    "SPRITE": 106,
    "BLACKSMITH": 5,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.5|2|\n5|100",
    "VALUE_TYPE": "%|%||\n%|%",
    "VALUE_LEVEL": "rl|rl||\n|",
    "VALUE_STAT": "aura_increased_effect_percent|aura_increased_effect_global_mult|\nincreased_damage",
    "VALUE_REAL": "synergy:aura_increased_effect:1|\nsynergy:percent_missing_mana:1",
    "BASE_DMG_MIN": 31,
    "BASE_DMG_MAX": 60,
    "MIN_DMG_LVL": 1,
    "MAX_DMG_LVL": 2,
    "DMG_MULTIPLIER": 4,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "619 - 1106"
  },
  {
    "REF": 110,
    "EN_NAME": "$ of Retaliating Foresight",
    "EN_DESC": "*+@ £*+@ £|*The first time an enemy should apply a debuff to you, ignore it.* _ Damage is added to your Retaliation Damage.#Added Damage is equal to @ of your Armor.*Whenever you get within a @ yard radius of an enemy for the first time, you trigger a Sight of Vengeance targetting them.*act:225/\n|*Sight of Vengeance applies Foresight of Vengeance.*You now deal Retaliation Damage to enemies affected by Foresight of Vengeance whenever they start their attack./\n|*You no longer deal Retaliation Damage when being hit.",
    "EN_LORE": "Revenge is a dish best served before it's ready.",
    "EN": "$ of Retaliating Foresight//*+@ £*+@ £|*The first time an enemy should apply a debuff to you, ignore it.* _ Damage is added to your Retaliation Damage.#Added Damage is equal to @ of your Armor.*Whenever you get within a @ yard radius of an enemy for the first time, you trigger a Sight of Vengeance targetting them.*act:225/\n|*Sight of Vengeance applies Foresight of Vengeance.*You now deal Retaliation Damage to enemies affected by Foresight of Vengeance whenever they start their attack./\n|*You no longer deal Retaliation Damage when being hit.//Revenge is a dish best served before it's ready.",
    "FR": "$ de Revanche Anticipée//*+@ £*+@ £|*La première fois qu'un ennemi devrait vous appliquer un Effet Négatif, ignorez le.*_  Dégâts sont ajoutés à vos Dégâts de Représailles.#Les Dégâts ajoutés sont équivalents à @ de votre Armure.*Lorsque vous êtes à moins de @ mètres d'un ennemi pour la première fois, vous déclenchez une Vision de Vengeance, le ciblant.*act:225/\n|*Vision de Vengeance applique Prescience de Vengeance.*Vous infligez désormais vos Dégâts de Représailles au moment où les ennemis affectés par Prescience de Vengeance commencent leurs attaques./\n|*Vous n'infligez plus vos Dégâts de Représailles lorsque vous êtes touchés.//La vengeance est un plat qui se mange avant qu'il ne soit prêt.",
    "FR_NAME": "$ de Revanche Anticipée",
    "FR_DESC": "*+@ £*+@ £|*La première fois qu'un ennemi devrait vous appliquer un Effet Négatif, ignorez le.*_  Dégâts sont ajoutés à vos Dégâts de Représailles.#Les Dégâts ajoutés sont équivalents à @ de votre Armure.*Lorsque vous êtes à moins de @ mètres d'un ennemi pour la première fois, vous déclenchez une Vision de Vengeance, le ciblant.*act:225/\n|*Vision de Vengeance applique Prescience de Vengeance.*Vous infligez désormais vos Dégâts de Représailles au moment où les ennemis affectés par Prescience de Vengeance commencent leurs attaques./\n|*Vous n'infligez plus vos Dégâts de Représailles lorsque vous êtes touchés.",
    "FR_LORE": "La vengeance est un plat qui se mange avant qu'il ne soit prêt.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 102,
    "SPRITE": 110,
    "BLACKSMITH": 2,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "2|0.5|0.5|2",
    "VALUE_TYPE": "%|%|%|",
    "VALUE_LEVEL": "rl|rl|rl|",
    "VALUE_STAT": "retaliate_percent|retaliate_global_mult",
    "VALUE_REAL": "synergy:armor:0.5*rl",
    "BASE_DMG_MIN": 36,
    "BASE_DMG_MAX": 48,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 2,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "993 - 1444"
  },
  {
    "REF": 111,
    "EN_NAME": "Slormbane, Twin-$",
    "EN_DESC": "*+@ £*+@ £|*Your Primary and Secondary Skills deal _ £.#Additional Damage is equal to the amount of Slorm found in the last @ seconds./\n|*Whenever you reap Slorm, you gain Life equal to the value of the Slorm reaped./\n|*Life Leech, Life on Hit, Life on Kill and Life Regeneration are reduced to 0.",
    "EN_LORE": "\"An old man once told me that mastering both Goldscourge...\"",
    "EN": "Slormbane, Twin-$//*+@ £*+@ £|*Your Primary and Secondary Skills deal _ £.#Additional Damage is equal to the amount of Slorm found in the last @ seconds./\n|*Whenever you reap Slorm, you gain Life equal to the value of the Slorm reaped./\n|*Life Leech, Life on Hit, Life on Kill and Life Regeneration are reduced to 0.//\"An old man once told me that mastering both Goldscourge...\"",
    "FR": "Slormbane, $-Jumeau/Slormbane, $-Jumelle//*+@ £*+@ £|*Vos Compétences Primaires et Secondaires infligent _ de £.#Ces Dégâts Additionnels sont équivalents au montant de Slorm trouvé ces @ dernières secondes./\n|*Lorsque vous récoltez du Slorm, vous récupérez de la Vie équivalente à la valeur du Slorm récolté./\n|*Vol de Vie, Vie par Coup, Vie par Victime et Régénération de Vie sont réduites à 0.//\"Un vieil homme m'a dit un jour que maîtriser à la fois Goldscourge...\"\n",
    "FR_NAME": "Slormbane, $-Jumeau/Slormbane, $-Jumelle",
    "FR_DESC": "*+@ £*+@ £|*Vos Compétences Primaires et Secondaires infligent _ de £.#Ces Dégâts Additionnels sont équivalents au montant de Slorm trouvé ces @ dernières secondes./\n|*Lorsque vous récoltez du Slorm, vous récupérez de la Vie équivalente à la valeur du Slorm récolté./\n|*Vol de Vie, Vie par Coup, Vie par Victime et Régénération de Vie sont réduites à 0.",
    "FR_LORE": "\"Un vieil homme m'a dit un jour que maîtriser à la fois Goldscourge...\"\n",
    "CH": "魂灵祸患，双子$//*+@£*+@£|*主技能和副技能造成_£。#额外伤害等于最近@秒内收集到的魂灵。/\n|*获取魂灵时，获得等量的生命值。/\n|*生命吸收、击中生命恢复、击杀生命恢复、生命再生减少为0。//“一个老人告诉过我，只要掌握了黄金灾厄……",
    "CH_NAME": "魂灵祸患，双子$",
    "CH_DESCRIPTION": "*+@£*+@£|*主技能和副技能造成_£。#额外伤害等于最近@秒内收集到的魂灵。/\n|*获取魂灵时，获得等量的生命值。/\n|*生命吸收、击中生命恢复、击杀生命恢复、生命再生减少为0。",
    "CH_LORE": "“一个老人告诉过我，只要掌握了黄金灾厄……",
    "TW": "魂靈禍患，雙子$//*+@£*+@£|*主技能和副技能造成_£。 #額外傷害等於最近@秒內收集到的魂靈。 /\n|*獲取魂靈時，獲得等量的生命值。 /\n|*生命吸收、擊中生命恢復、擊殺生命恢復、生命再生減少為0。//“一個老人告訴過我，只要掌握了黃金災厄……",
    "TW_NAME": "魂靈禍患，雙子$",
    "TW_DESCRIPTION": "*+@£*+@£|*主技能和副技能造成_£。 #額外傷害等於最近@秒內收集到的魂靈。 /\n|*獲取魂靈時，獲得等量的生命值。 /\n|*生命吸收、擊中生命恢復、擊殺生命恢復、生命再生減少為0。",
    "TW_LORE": "“一個老人告訴過我，只要掌握了黃金災厄……",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 110,
    "SPRITE": 111,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.5|20|",
    "VALUE_TYPE": "%|%|",
    "VALUE_LEVEL": "rl|rl||",
    "VALUE_STAT": "essence_find_percent|essence_find_global_mult|additional_damage",
    "VALUE_REAL": "synergy:slormocide_60:1",
    "BASE_DMG_MIN": 12,
    "BASE_DMG_MAX": 15,
    "MIN_DMG_LVL": 2,
    "MAX_DMG_LVL": 3,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 50,
    "EVOLVE_IN": 113,
    "100": "453 - 626"
  },
  {
    "REF": 112,
    "EN_NAME": "Goldscourge, Twin-$",
    "EN_DESC": "*+@ £*+@ £|*Your Primary and Secondary Skills deal _ £.#Additional Damage is equal to the amount of Goldus found in the last @ seconds./\n|*Every @ second, you trigger a Golden Overdrive dealing _% £ and _ £.#Increased Damage is equal to your Goldus Find.#Additional Damage is equal to the amount of Goldus found in the past @ seconds./\n|*Your Overdrive Chance is reduced to 0.",
    "EN_LORE": "\"... and his Twin-Reaper Slormbane would make them fusion into a single Slorm Reaper : the Mythical Aurifurious.\"#- Slorm Reapers of Legend, by Pr. Geranium",
    "EN": "Goldscourge, Twin-$//*+@ £*+@ £|*Your Primary and Secondary Skills deal _ £.#Additional Damage is equal to the amount of Goldus found in the last @ seconds./\n|*Every @ second, you trigger a Golden Overdrive dealing _% £ and _ £.#Increased Damage is equal to your Goldus Find.#Additional Damage is equal to the amount of Goldus found in the past @ seconds./\n|*Your Overdrive Chance is reduced to 0.//\"... and his Twin-Reaper Slormbane would make them fusion into a single Slorm Reaper : the Mythical Aurifurious.\"#- Slorm Reapers of Legend, by Pr. Geranium",
    "FR": "Goldscourge, $-Jumeau/Goldscourge, $-Jumelle//*+@ £*+@ £|*Vos Compétences Primaires et Secondaires infligent _ de £.#Ces Dégâts Additionnels sont équivalents au montant de Goldus ramassé ces @ dernières secondes./\n|*Toutes les @ secondes, vous déclenchez une Surpuissance Dorée, infligeant _% de £ et _ de £.#Ces Dégâts Supplémentaires sont équivalents à votre Découverte de Goldus.#Ces Dégâts Additionnels sont équivalent au montant de Goldus ramassé ces @ dernières secondes./\n|*Votre Chance de Surpuissance est réduite à 0.//\"... et son Reaper-Jumeau Slormbane les unirait en un seul Slorm Reaper : Aurifurious.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "FR_NAME": "Goldscourge, $-Jumeau/Goldscourge, $-Jumelle",
    "FR_DESC": "*+@ £*+@ £|*Vos Compétences Primaires et Secondaires infligent _ de £.#Ces Dégâts Additionnels sont équivalents au montant de Goldus ramassé ces @ dernières secondes./\n|*Toutes les @ secondes, vous déclenchez une Surpuissance Dorée, infligeant _% de £ et _ de £.#Ces Dégâts Supplémentaires sont équivalents à votre Découverte de Goldus.#Ces Dégâts Additionnels sont équivalent au montant de Goldus ramassé ces @ dernières secondes./\n|*Votre Chance de Surpuissance est réduite à 0.",
    "FR_LORE": "\"... et son Reaper-Jumeau Slormbane les unirait en un seul Slorm Reaper : Aurifurious.\"#- Slorm Reapers de Légende, par le Pr. Geranium",
    "CH": "黄金灾厄，双子$",
    "CH_NAME": "黄金灾厄，双子$",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "黃金災厄，雙子$",
    "TW_NAME": "黃金災厄，雙子$",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 111,
    "SPRITE": 112,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|0.5|5|\n0.5|5",
    "VALUE_TYPE": "%|%||\n|",
    "VALUE_LEVEL": "rl|rl||\n|",
    "VALUE_STAT": "gold_find_percent|gold_find_global_mult|additional_damage|\nincreased_damage|additional_damage",
    "VALUE_REAL": "synergy:goldbane_5:1|\nsynergy:gold_find_percent:1|synergy:goldbane_5:1",
    "BASE_DMG_MIN": 10,
    "BASE_DMG_MAX": 13,
    "MIN_DMG_LVL": 2,
    "MAX_DMG_LVL": 3,
    "DMG_MULTIPLIER": 2,
    "LOOTABLE": 2,
    "MAX_LVL": 50,
    "EVOLVE_IN": 113,
    "100": "447 - 624"
  },
  {
    "REF": 113,
    "EN_NAME": "Aurifurious, $ born of Fusion",
    "EN_DESC": "|/\n|/\n|",
    "EN_LORE": "On the handle, it reads: \"We do not sow. We take.\"",
    "EN": "Aurifurious, $ born of Fusion//|/\n|/\n|//On the handle, it reads: \"We do not sow. We take.\"",
    "FR": "Aurifurious, $ né de Fusion/Aurifurious, $ née de Fusion//|/\n|/\n|//Sur le manche, on peut lire : \"Nous ne semons pas. Nous prenons\".",
    "FR_NAME": "Aurifurious, $ né de Fusion/Aurifurious, $ née de Fusion",
    "FR_DESC": "|/\n|/\n|",
    "FR_LORE": "Sur le manche, on peut lire : \"Nous ne semons pas. Nous prenons\".",
    "CH": "曜金神话，融合$//|/\n|/\n|//握把上写着：“不播种，只收割。”",
    "CH_NAME": "曜金神话，融合$",
    "CH_DESCRIPTION": "|/\n|/\n|",
    "CH_LORE": "握把上写着：“不播种，只收割。”",
    "TW": "曜金神話，融合$//|/\n|/\n|//握把上寫著：“不播種，只收割。”",
    "TW_NAME": "曜金神話，融合$",
    "TW_DESCRIPTION": "|/\n|/\n|",
    "TW_LORE": "握把上寫著：“不播種，只收割。”",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 112,
    "SPRITE": 113,
    "BLACKSMITH": 3,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "",
    "VALUE_TYPE": "",
    "VALUE_LEVEL": "",
    "VALUE_STAT": "",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 11,
    "BASE_DMG_MAX": 14,
    "MIN_DMG_LVL": 3,
    "MAX_DMG_LVL": 4,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": null,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "891 - 1135"
  },
  {
    "REF": 114,
    "EN_NAME": "Vengeance, Ohm Agad's Cursed $",
    "EN_DESC": "|*Whenever you deal damage to an enemy with your Primary or Secondary Skills, you have a Chance equal to their Percent of Missing Life to deal _ Additional Damage.#Additional Damage is equal to the number of Victims made by Vengeance.*You have +_ £.#Max Life is equal to @ for each Victim made by Vengeance./\n|*You have +_% £.#Raw Damage is equal to @ for each Victim made by Vengeance, across all Characters.*You have +_% £.#Elemental Damage is equal to @ for each Victim made by Vengeance, across all Characters./\n|*You can no longer gain Slorm and Experience.",
    "EN_LORE": "Only as a Primordial, does Vengeance reveal its true potential.",
    "EN": "Vengeance, Ohm Agad's Cursed $//|*Whenever you deal damage to an enemy with your Primary or Secondary Skills, you have a Chance equal to their Percent of Missing Life to deal _ Additional Damage.#Additional Damage is equal to the number of Victims made by Vengeance.*You have +_ £.#Max Life is equal to @ for each Victim made by Vengeance./\n|*You have +_% £.#Raw Damage is equal to @ for each Victim made by Vengeance, across all Characters.*You have +_% £.#Elemental Damage is equal to @ for each Victim made by Vengeance, across all Characters./\n|*You can no longer gain Slorm and Experience.//Only as a Primordial, does Vengeance reveal its true potential.",
    "FR": "Vengeance, $ Maudit d'Ohm Agad/Vengeance, $ Maudite d'Ohm Agad//|*À chaque fois que vous infligez des dégâts à un ennemi avec une Compétence Primaire ou Secondaire, vous avez une chance égale à leur Pourcentage de Vie Manquant d'infliger _ de Dégâts Additionnels .#Ces Dégâts Additionnels sont équivalents au nombre de Victimes faites par Vengeance.*Vous avez +_ de £.#Cette Vie Max est équivalente à @ pour chaque Victime faite par Vengeance./\n|*Vous avez +_% £.#Ces Dégâts Bruts sont équivalents à @ pour chaque Victime faite par Vengeance, à travers tous vos personnages.*Vous avez +_% £.#Ces Dégâts Élémentaires sont équivalents à @ pour chaque Victime faite par Vengeance, à travers tous vos personnages./\n|*Vous ne pouvez plus gagner de Slorm et d'Experience.//Ce n'est qu'en tant que Primordial que Vengeance révèle son véritable potentiel.",
    "FR_NAME": "Vengeance, $ Maudit d'Ohm Agad/Vengeance, $ Maudite d'Ohm Agad",
    "FR_DESC": "|*À chaque fois que vous infligez des dégâts à un ennemi avec une Compétence Primaire ou Secondaire, vous avez une chance égale à leur Pourcentage de Vie Manquant d'infliger _ de Dégâts Additionnels .#Ces Dégâts Additionnels sont équivalents au nombre de Victimes faites par Vengeance.*Vous avez +_ de £.#Cette Vie Max est équivalente à @ pour chaque Victime faite par Vengeance./\n|*Vous avez +_% £.#Ces Dégâts Bruts sont équivalents à @ pour chaque Victime faite par Vengeance, à travers tous vos personnages.*Vous avez +_% £.#Ces Dégâts Élémentaires sont équivalents à @ pour chaque Victime faite par Vengeance, à travers tous vos personnages./\n|*Vous ne pouvez plus gagner de Slorm et d'Experience.",
    "FR_LORE": "Ce n'est qu'en tant que Primordial que Vengeance révèle son véritable potentiel.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 118,
    "SPRITE": 99,
    "BLACKSMITH": 7,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "0.1|\n0.01|0.01|\n-100|-100",
    "VALUE_TYPE": "|\n||\n%|%",
    "VALUE_LEVEL": "|\n||\n|",
    "VALUE_STAT": "the_max_health_add|\nbasic_damage_global_mult|elemental_damage_global_mult|\nessence_find_global_mult|xp_find_global_mult",
    "VALUE_REAL": "synergy:victims_114:1|synergy:victims_114:0.1|\nsynergy:victims_114_all:0.01|synergy:victims_114_all:0.01",
    "BASE_DMG_MIN": 0,
    "BASE_DMG_MAX": 0,
    "MIN_DMG_LVL": 0,
    "MAX_DMG_LVL": 0,
    "DMG_MULTIPLIER": 0,
    "LOOTABLE": null,
    "MAX_LVL": 1,
    "EVOLVE_IN": null,
    "100": "0 - 0"
  },
  {
    "REF": 115,
    "EN_NAME": "Farosh, The Burning Lava $",
    "EN_DESC": "*+@ £|*Fire-Imbued Skills deal @ £.*+@ Increased Burn Damage.*While Lowey's Creation is toggled on, a second Fire Elemental now fights by your side.*Living Inferno now lasts @ seconds./\n|*Fiery Weapons and More Fire! are automatically Equipped and at Max Rank./\n|*You can only use Fire and Neutral Skills and benefit from Fire and Neutral Upgrades from the Ancestral Legacy.",
    "EN_LORE": "To eradicate twice as many insects!",
    "EN": "Farosh, The Burning Lava $//*+@ £|*Fire-Imbued Skills deal @ £.*+@ Increased Burn Damage.*While Lowey's Creation is toggled on, a second Fire Elemental now fights by your side.*Living Inferno now lasts @ seconds./\n|*Fiery Weapons and More Fire! are automatically Equipped and at Max Rank./\n|*You can only use Fire and Neutral Skills and benefit from Fire and Neutral Upgrades from the Ancestral Legacy.//To eradicate twice as many insects!",
    "FR": "Farosh, $ de Lave Brûlante//*+@ £|*Les compétences Imprégnées de Feu infligent @ de £.+*@ de Dégâts Supplémentaires de Brûlure.*Lorsque Création de Lowey est active, un deuxième Élémentaire du Feu se bat maintenant à vos côtés.*Enfer Vivant dure désormais @ secondes./\n|*Armes Flambantes et Davantage de Flammes ! sont automatiquement Équipées et au Rang Max./\n|*Vous ne pouvez utiliser que des Compétences de Feu et Neutres, et bénéficier des Améliorations de Feu et Neutres de 'Héritage Ancestral.//Pour éradiquer deux fois plus d'insectes !",
    "FR_NAME": "Farosh, $ de Lave Brûlante",
    "FR_DESC": "*+@ £|*Les compétences Imprégnées de Feu infligent @ de £.+*@ de Dégâts Supplémentaires de Brûlure.*Lorsque Création de Lowey est active, un deuxième Élémentaire du Feu se bat maintenant à vos côtés.*Enfer Vivant dure désormais @ secondes./\n|*Armes Flambantes et Davantage de Flammes ! sont automatiquement Équipées et au Rang Max./\n|*Vous ne pouvez utiliser que des Compétences de Feu et Neutres, et bénéficier des Améliorations de Feu et Neutres de 'Héritage Ancestral.",
    "FR_LORE": "Pour éradiquer deux fois plus d'insectes !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 113,
    "SPRITE": 115,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|1|30",
    "VALUE_TYPE": "%|%|%|",
    "VALUE_LEVEL": "rl|rl|rl|",
    "VALUE_STAT": "fire_resistance_percent|increased_damage",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 32,
    "BASE_DMG_MAX": 40,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 8,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 3,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1700 - 2221"
  },
  {
    "REF": 116,
    "EN_NAME": "Slasha, The Shivering $ of Winter",
    "EN_DESC": "*+@ £|*Ice-Imbued Skills deal @ £.*Chill now lasts indefinitely.#Chill now applies @ £.*Frozen now breaks after @ hits instead of @.*Ice-Imbued Skills and Ice Effects deal @ £ to enemies affected by Chill or Frozen./\n|*Winds of Winter is Equipped and at Max Rank.*The colder it gets, the stronger Slasha is:#Ice-Imbued Skills and Ice Effects deal Increased Damage based on Temperature, up to @./\n|*You can no longer apply Negative Effects to enemies besides Chill and Frozen.",
    "EN_LORE": "This Reaper was wrought with the eternal ice of the Northern Lands. It never melts. # Convenient to keep all types of meat or to refresh a drink!",
    "EN": "Slasha, The Shivering $ of Winter//*+@ £|*Ice-Imbued Skills deal @ £.*Chill now lasts indefinitely.#Chill now applies @ £.*Frozen now breaks after @ hits instead of @.*Ice-Imbued Skills and Ice Effects deal @ £ to enemies affected by Chill or Frozen./\n|*Winds of Winter is Equipped and at Max Rank.*The colder it gets, the stronger Slasha is:#Ice-Imbued Skills and Ice Effects deal Increased Damage based on Temperature, up to @./\n|*You can no longer apply Negative Effects to enemies besides Chill and Frozen.//This Reaper was wrought with the eternal ice of the Northern Lands. It never melts. # Convenient to keep all types of meat or to refresh a drink!",
    "FR": "Slasha, $ de l'Hiver Mordant//*+@ £|*Les compétences Imprégnées de Glace infligent @ de £.*Glacé dure désormais indéfiniment.#Glacé applique désormais @ £.*Congelé se brise désormais après @ coups au lieu de @.*Les Compétences Imprégnées de Glace et les Effets de Glace infligent @ de  £ aux ennemis affectés par Glacé ou Congelé./\n|*Les Vents de l'Hiver est automatiquement Équipé et au Rang Max.*Plus il fait froid, plus Slasha est puissante :#Les Compétences Imprégnées de Glace et les Effets de Glace infligent des Dégâts Supplémentaires en fonction de la température, jusqu'à @./\n|*Vous ne pouvez plus appliquer d'Effets Négatifs aux ennemis, sauf Glacé ou Congelé.//Ce Reaper fut forgé avec la glace éternelle des Terres du Nord. Il ne fond jamais.#Pratique pour conserver tous types de viande ou rafraichir une boisson !",
    "FR_NAME": "Slasha, $ de l'Hiver Mordant",
    "FR_DESC": "*+@ £|*Les compétences Imprégnées de Glace infligent @ de £.*Glacé dure désormais indéfiniment.#Glacé applique désormais @ £.*Congelé se brise désormais après @ coups au lieu de @.*Les Compétences Imprégnées de Glace et les Effets de Glace infligent @ de  £ aux ennemis affectés par Glacé ou Congelé./\n|*Les Vents de l'Hiver est automatiquement Équipé et au Rang Max.*Plus il fait froid, plus Slasha est puissante :#Les Compétences Imprégnées de Glace et les Effets de Glace infligent des Dégâts Supplémentaires en fonction de la température, jusqu'à @./\n|*Vous ne pouvez plus appliquer d'Effets Négatifs aux ennemis, sauf Glacé ou Congelé.",
    "FR_LORE": "Ce Reaper fut forgé avec la glace éternelle des Terres du Nord. Il ne fond jamais.#Pratique pour conserver tous types de viande ou rafraichir une boisson !",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 114,
    "SPRITE": 116,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|-95|3|1|1|\n2",
    "VALUE_TYPE": "%|%|%|||%|\n%",
    "VALUE_LEVEL": "rl|rl||||rl|\nrl",
    "VALUE_STAT": "ice_resistance_percent|increased_damage|the_speed_percent|increased_damage|",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 38,
    "BASE_DMG_MAX": 44,
    "MIN_DMG_LVL": 9,
    "MAX_DMG_LVL": 11,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 3,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "2444 - 2952"
  },
  {
    "REF": 117,
    "EN_NAME": "Kah Veer, The Crackling Thunder-$",
    "EN_DESC": "*+@ £*+@ £*+@ £|*Lightning-Imbued Skills deal @ £.*The Cooldown of your Skills is reduced by @ every time you walk @ yard./\n|*Thunderstruck is automatically Equipped and at Max Rank.#Critical Strikes trigger Thunderstruck as if they were Ancestral Strikes./\n*Critical Strikes no longer deal Critical Strike Damage.",
    "EN_LORE": "Death comes from above!",
    "EN": "Kah Veer, The Crackling Thunder-$//*+@ £*+@ £*+@ £|*Lightning-Imbued Skills deal @ £.*The Cooldown of your Skills is reduced by @ every time you walk @ yard./\n|*Thunderstruck is automatically Equipped and at Max Rank.#Critical Strikes trigger Thunderstruck as if they were Ancestral Strikes./\n*Critical Strikes no longer deal Critical Strike Damage.//Death comes from above!",
    "FR": "Kah Veer, $-Tonnerre Crépitant/Kah Veer, $-Tonnerre Crépitante//*+@ £*+@ £*+@ £|*Les Compétences Imprégnées de Foudre infligent @ de £.*Le Temps de Recharge de vos Compétences est réduit de @ chaque fois que vous parcourez @ mètres./\n|*Éclair est automatiquement Équipé et au Rang Max.#Les Coups Critiques déclenchent Tonnerre comme si il s'agissait de Coups Ancestraux./\n*Les Coups Critiques n'infligent plus de Dégâts Critiques.//La mort vient du ciel !",
    "FR_NAME": "Kah Veer, $-Tonnerre Crépitant/Kah Veer, $-Tonnerre Crépitante",
    "FR_DESC": "*+@ £*+@ £*+@ £|*Les Compétences Imprégnées de Foudre infligent @ de £.*Le Temps de Recharge de vos Compétences est réduit de @ chaque fois que vous parcourez @ mètres./\n|*Éclair est automatiquement Équipé et au Rang Max.#Les Coups Critiques déclenchent Tonnerre comme si il s'agissait de Coups Ancestraux./\n*Les Coups Critiques n'infligent plus de Dégâts Critiques.",
    "FR_LORE": "La mort vient du ciel !",
    "CH": "卡赫·维尔，霹雳惊雷$//*+@£*+@£*+@£|*光明附魔技能造成@£。*每移动@码，技能冷却时间减少@。/\n|*雷击自动装备，并视为最高等级。#暴击触发雷击，以先祖攻击计算。/\n*暴击不再造成暴击伤害。//死从天降！",
    "CH_NAME": "卡赫·维尔，霹雳惊雷$",
    "CH_DESCRIPTION": "*+@£*+@£*+@£|*光明附魔技能造成@£。*每移动@码，技能冷却时间减少@。/\n|*雷击自动装备，并视为最高等级。#暴击触发雷击，以先祖攻击计算。/\n*暴击不再造成暴击伤害。",
    "CH_LORE": "死从天降！",
    "TW": "卡赫·維爾，霹靂驚雷$//*+@£*+@£*+@£|*光明附魔技能造成@£。 *每移動@碼，技能冷卻時間減少@。 /\n|*雷擊自動裝備，並視為最高等級。 #暴擊觸發雷擊，以先祖攻擊計算。 /\n*暴擊不再造成暴擊傷害。//死從天降！",
    "TW_NAME": "卡赫·維爾，霹靂驚雷$",
    "TW_DESCRIPTION": "*+@£*+@£*+@£|*光明附魔技能造成@£。 *每移動@碼，技能冷卻時間減少@。 /\n|*雷擊自動裝備，並視為最高等級。 #暴擊觸發雷擊，以先祖攻擊計算。 /\n*暴擊不再造成暴擊傷害。",
    "TW_LORE": "死從天降！",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 115,
    "SPRITE": 117,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|0.5|1|2|1|\n\n-100",
    "VALUE_TYPE": "%|%|%|%|%||\n\n%",
    "VALUE_LEVEL": "rl|rl|rl|rl|||",
    "VALUE_STAT": "lightning_resistance_percent|elemental_damage_percent|the_speed_percent|increased_damage|\ncrit_damage_global_mult",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 1,
    "BASE_DMG_MAX": 46,
    "MIN_DMG_LVL": 0,
    "MAX_DMG_LVL": 9,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 3,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "142 - 2488"
  },
  {
    "REF": 118,
    "EN_NAME": "Aïondee, The $ of the Rising Sun",
    "EN_DESC": "*+@ £*+@ £|*Light-Imbued Skills deal @ £.*The Shield Globes' value is also increased by _. The increase is equal to @ of your $.*Shield stops decreasing below @ Shield./\n|*Whenever you gain Life from Life Leech, Life on Hit or Life on Kill, if your Life is full, Life gain is converted to Shield./\n|*Your Max Life is @.",
    "EN_LORE": "The First of the five Legendary Reapers created by Hagan, the Forsaken Exiled.#Wrought for Aurelon, the first Champion of Arah.",
    "EN": "Aïondee, The $ of the Rising Sun//*+@ £*+@ £|*Light-Imbued Skills deal @ £.*The Shield Globes' value is also increased by _. The increase is equal to @ of your $.*Shield stops decreasing below @ Shield./\n|*Whenever you gain Life from Life Leech, Life on Hit or Life on Kill, if your Life is full, Life gain is converted to Shield./\n|*Your Max Life is @.//The First of the five Legendary Reapers created by Hagan, the Forsaken Exiled.#Wrought for Aurelon, the first Champion of Arah.",
    "FR": "Aïondee, $ du Soleil Levant//*+@ £*+@ £|*Les Compétences Imprégnées de Lumière infligent @ de £.*La valeur des Globes de Bouclier est augmentée de _. Cette augmentation est équivalente à @ de votre $.*Le Bouclier cesse de diminuer en dessous de @ de Bouclier restant./\n|*Lorsque vous gagnez de la Vie avec Vol de Vie, Vie par Coup ou Vie par Victime, si votre Vie est pleine, la Vie gagnée est convertie en Bouclier./\n|*Votre Vie Max est de @.//Premier des cinq Reapers Légendaires créés par Hagan, l'Exilé Oublié.#Forgé pour Aurelon, le Premier Champion d'Arah.",
    "FR_NAME": "Aïondee, $ du Soleil Levant",
    "FR_DESC": "*+@ £*+@ £|*Les Compétences Imprégnées de Lumière infligent @ de £.*La valeur des Globes de Bouclier est augmentée de _. Cette augmentation est équivalente à @ de votre $.*Le Bouclier cesse de diminuer en dessous de @ de Bouclier restant./\n|*Lorsque vous gagnez de la Vie avec Vol de Vie, Vie par Coup ou Vie par Victime, si votre Vie est pleine, la Vie gagnée est convertie en Bouclier./\n|*Votre Vie Max est de @.",
    "FR_LORE": "Premier des cinq Reapers Légendaires créés par Hagan, l'Exilé Oublié.#Forgé pour Aurelon, le Premier Champion d'Arah.",
    "CH": "艾昂迪，旭日$//*+@£*+@£|*光明附魔技能造成@£。*护盾球的数值增加_。增长量等于你的$的@。*护盾值在低于@时停止减少。/\n|*通过生命吸收、击中生命恢复、击杀生命恢复、生命再生获得生命值时，均转化为等量护盾。/\n|*最大生命值为@。//最初的五件传奇猎魂武装，由“被遗忘的流放者”哈甘打造。#由奥伦铸造，最初的亚拉勇者。",
    "CH_NAME": "艾昂迪，旭日$",
    "CH_DESCRIPTION": "*+@£*+@£|*光明附魔技能造成@£。*护盾球的数值增加_。增长量等于你的$的@。*护盾值在低于@时停止减少。/\n|*通过生命吸收、击中生命恢复、击杀生命恢复、生命再生获得生命值时，均转化为等量护盾。/\n|*最大生命值为@。",
    "CH_LORE": "最初的五件传奇猎魂武装，由“被遗忘的流放者”哈甘打造。#由奥伦铸造，最初的亚拉勇者。",
    "TW": "艾昂迪，旭日$//*+@£*+@£|*光明附魔技能造成@£。 *護盾球的數值增加_。增長量等於你的$的@。 *護盾值在低於@時停止減少。 /\n|*通過生命吸收、擊中生命恢復、擊殺生命恢復、生命再生獲得生命值時，均轉化為等量護盾。 /\n|*最大生命值為@。//最初的五件傳奇獵魂武裝，由“被遺忘的流放者”哈甘打造。 #由奧倫鑄造，最初的亞拉勇者。",
    "TW_NAME": "艾昂迪，旭日$",
    "TW_DESCRIPTION": "*+@£*+@£|*光明附魔技能造成@£。 *護盾球的數值增加_。增長量等於你的$的@。 *護盾值在低於@時停止減少。 /\n|*通過生命吸收、擊中生命恢復、擊殺生命恢復、生命再生獲得生命值時，均轉化為等量護盾。 /\n|*最大生命值為@。",
    "TW_LORE": "最初的五件傳奇獵魂武裝，由“被遺忘的流放者”哈甘打造。 #由奧倫鑄造，最初的亞拉勇者。",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 116,
    "SPRITE": 118,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|1|1|\n26|\n1",
    "VALUE_TYPE": "%|%|%|%|\n|",
    "VALUE_LEVEL": "rl|rl|rl|rl|\nrl|",
    "VALUE_STAT": "light_resistance_percent|shield_globe_value|increased_damage|\n|",
    "VALUE_REAL": "synergy:elemental_damage:1*rl",
    "BASE_DMG_MIN": 52,
    "BASE_DMG_MAX": 64,
    "MIN_DMG_LVL": 10,
    "MAX_DMG_LVL": 14,
    "DMG_MULTIPLIER": 1,
    "LOOTABLE": 3,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1591 - 2187"
  },
  {
    "REF": 119,
    "EN_NAME": "Ashes, $ of the Blorm Chieftain",
    "EN_DESC": "*+@ £|*Shadow-Imbued Skills deal @ £.*Blorm Up! is automatically Equipped and at Max Rank.*While being Soul Bound to an enemy, Blorms you control have +@ £.*While they stand on a Shadow Seal, Blorms you control have +@ £.*While being the Avatar of Shadow, Blorms you control have +@ £./\n|*Blorms you control deal @ £.*Whenever you should create a Blorm, if you've reached the Maximum number of Blorms, you instead grant +@ £ and +@ £ to other Blorms for the next @ seconds./\n|*You cannot control more than @ Blorms at once.",
    "EN_LORE": "The Blorms are a creation of Jerelzor. They are an alteration of Slorm. A unique form of Slormancy and Necromancy.",
    "EN": "Ashes, $ of the Blorm Chieftain//*+@ £|*Shadow-Imbued Skills deal @ £.*Blorm Up! is automatically Equipped and at Max Rank.*While being Soul Bound to an enemy, Blorms you control have +@ £.*While they stand on a Shadow Seal, Blorms you control have +@ £.*While being the Avatar of Shadow, Blorms you control have +@ £./\n|*Blorms you control deal @ £.*Whenever you should create a Blorm, if you've reached the Maximum number of Blorms, you instead grant +@ £ and +@ £ to other Blorms for the next @ seconds./\n|*You cannot control more than @ Blorms at once.//The Blorms are a creation of Jerelzor. They are an alteration of Slorm. A unique form of Slormancy and Necromancy.",
    "FR": "Ashes, $ du Chef de Tribu Blorms//*+@ £|*Les Compétences Imprégnées d'Ombre infligent @ de £.*Appel du Blorm ! est automatiquement Équipé et au Rang Max*En étant Lié à  à un ennemi, les Blorms que vous contrôlez ont +@ £.*Tant qu'ils se tiennent sur un Sceau d'ombre, les Blorms que vous contrôlez ont +@ £.*En étant l'Avatar de l'Ombre, les Blorms que vous contrôlez ont +@ £./\n|*Les Blorms que vous contrôlez infligent @ £.*Chaque fois que vous devriez créer un Blorm, si vous avez atteint le nombre maximum de Blorms, vous accordez plutôt +@ £ et +@ £ aux autres Blorms pour les @ prochaines secondes./\n|*Vous ne pouvez pas contrôler plus de @ Blorms à la fois.//Les Blorms sont une création de Jerelzor. Ils sont une altération du Slorm. Une forme unique de Slormancie et de Nécromancie.",
    "FR_NAME": "Ashes, $ du Chef de Tribu Blorms",
    "FR_DESC": "*+@ £|*Les Compétences Imprégnées d'Ombre infligent @ de £.*Appel du Blorm ! est automatiquement Équipé et au Rang Max*En étant Lié à  à un ennemi, les Blorms que vous contrôlez ont +@ £.*Tant qu'ils se tiennent sur un Sceau d'ombre, les Blorms que vous contrôlez ont +@ £.*En étant l'Avatar de l'Ombre, les Blorms que vous contrôlez ont +@ £./\n|*Les Blorms que vous contrôlez infligent @ £.*Chaque fois que vous devriez créer un Blorm, si vous avez atteint le nombre maximum de Blorms, vous accordez plutôt +@ £ et +@ £ aux autres Blorms pour les @ prochaines secondes./\n|*Vous ne pouvez pas contrôler plus de @ Blorms à la fois.",
    "FR_LORE": "Les Blorms sont une création de Jerelzor. Ils sont une altération du Slorm. Une forme unique de Slormancie et de Nécromancie.",
    "CH": "",
    "CH_NAME": "",
    "CH_DESCRIPTION": "",
    "CH_LORE": "",
    "TW": "",
    "TW_NAME": "",
    "TW_DESCRIPTION": "",
    "TW_LORE": "",
    "JP": "",
    "JP_NAME": "",
    "JP_DESCRIPTION": "",
    "JP_LORE": "",
    "DE": "",
    "DE_NAME": "",
    "DE_DESCRIPTION": "",
    "DE_LORE": "",
    "ES": "",
    "ES_NAME": "",
    "ES_DESCRIPTION": "",
    "ES_LORE": "",
    "IT": "",
    "IT_NAME": "",
    "IT_DESCRIPTION": "",
    "IT_LORE": "",
    "RU": "",
    "RU_NAME": "",
    "RU_DESCRIPTION": "",
    "RU_LORE": "",
    "PT": "",
    "PT_NAME": "",
    "PT_DESCRIPTION": "",
    "PT_LORE": "",
    "PO": "",
    "PO_NAME": "",
    "PO_DESCRIPTION": "",
    "PO_LORE": "",
    "KR": "",
    "KR_NAME": "",
    "KR_DESCRIPTION": "",
    "KR_LORE": "",
    "TUR": "",
    "TUR_NAME": "",
    "TUR_DESCRIPTION": "",
    "TUR_LORE": "",
    "ORDER": 117,
    "SPRITE": 119,
    "BLACKSMITH": 6,
    "CONSTANT_CHECK": "",
    "VALUE_BASE": "1|1|1|1|5|\n3|5|10|10|\n5",
    "VALUE_TYPE": "%|%|%|%|%|\n%|%|%||\n",
    "VALUE_LEVEL": "rl|rl|rl|rl|rl|\nrl||||",
    "VALUE_STAT": "shadow_resistance_percent|increased_damage|recast_chance_percent|cooldown_reduction_global_mult|crit_damage_percent|\nincreased_damage|cooldown_reduction_global_mult|the_speed_percent",
    "VALUE_REAL": "",
    "BASE_DMG_MIN": 19,
    "BASE_DMG_MAX": 30,
    "MIN_DMG_LVL": 6,
    "MAX_DMG_LVL": 10,
    "DMG_MULTIPLIER": 3,
    "LOOTABLE": 3,
    "MAX_LVL": 100,
    "EVOLVE_IN": null,
    "100": "1635 - 2645"
  }
]